...
function typeToFormattedString(type, checker, indent = 0, visited = new Set()) {
	...
	if (checker.isArrayType(type)) {
		...
		if (typeArgs && typeArgs.length > 0) {
			const elementType = typeToFormattedString(
				typeArgs[0],
				checker,
				indent,
				new Set(visited),
	...
	if (type.isUnion()) {
		...
		const allBooleanLiterals = nonUndefinedTypes.every(
		...
		const typeStrs = types.map((t) =>
			typeToFormattedString(t, checker, indent, new Set(visited)),
		);
	...
	if (type.flags & ts.TypeFlags.Object) {
		...
		for (const prop of properties) {
			...
			const propTypeStr = typeToFormattedString(
				propType,
				checker,
				indent + 1,
				new Set(visited),
		...
		if (stringIndexType) {
			const indexTypeStr = typeToFormattedString(
				stringIndexType,
				checker,
				indent + 1,
				new Set(visited),
