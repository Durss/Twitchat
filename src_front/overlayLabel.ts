...
async function broadcast<Event extends keyof TwitchatEventMap>(
	type: Event,
	...args: TwitchatEventMap[Event] extends undefined ? [] : [data: TwitchatEventMap[Event]]
): Promise<void> {
	const [data] =
	...
	if (obsConnected) {
		...
		void obsSocket.call("BroadcastCustomEvent", {
			eventData: eventData as unknown as JsonObject,
		});
	}
}
...
function onMessage(message: IEnvelope): void {
	...
	if (message.type == "ON_TWITCHAT_READY" || message.type == "ON_OBS_WEBSOCKET_CONNECTED") {
		...
		renderValue();
	} else if (message.type == "ON_LABEL_OVERLAY_CONFIGS") {
...
function setScrollSpeed(attempts = 0) {
	...
	scrollables.forEach((node) => {
		...
		if (bounds.width === 0) {
			...
			const phType: LabelItemPlaceholder["type"] =
				parameters!.mode == "placeholder"
					? placeholders[parameters!.placeholder]!.type
					: "string";
			const ratio = phType == "image" ? 0.5 : 20 / parameters!.fontSize;
