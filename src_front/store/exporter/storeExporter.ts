...
export const storeExporter = defineStore("Exporter", {
	...
	actions: {
		async exportSelectedSettings(
			...
			if (json) {
				const creditsData = JSON.parse(json) as
					| TwitchatDataTypes.EndingCreditsParams
					| undefined;
			...
			try {
				...
				const query = await ApiHelper.call(
					"user/settingsPreset",
					"POST",
					{ name: exportName, data: encrypted ? undefined : data, encrypted },
					false,
				);
			...
			try {
				...
				if (data.triggers.length > 0) {
					data.triggers.forEach((t) => {
						...
						const existsAt = StoreProxy.triggers.triggerList.findIndex(
							(v) => v.id == t.id,
						);
						if (existsAt !== -1) {
				...
				if (data.endingCreditsSlots.length > 0) {
					...
					void StoreProxy.endingCredits.saveParams();
				}
			} catch (e) {
				console.error("Error importing triggers:", e);
				Sentry.captureException("Failed importing triggers", {
					attachments: [{ filename: "imported_data.json", data: JSON.stringify(data) }],
				});
			}
		},
	} as IExporterActions &
