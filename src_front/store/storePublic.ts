...
export const storePublic = defineStore("public", {
	...
	actions: {
		...
		async twitchAuth(code?: string): Promise<boolean> {
			...
			let twitchAuthResult: TwitchDataTypes.AuthTokenResult | null = storeValue
			...
			if (code) {
				...
				TwitchUtils.updateAuthInfo(
					twitchAuthResult.access_token,
					twitchAuthResult.scope,
					(_scopes: TwitchScopesString[]) => {},
					async (): Promise<false | TwitchDataTypes.AuthTokenResult> => {
						return false;
					},
					this.twitchUid,
				);
				...
				refreshTokenTO = window.setTimeout(
					() => {
						void this.twitchTokenRefresh(true);
					},
		...
		async twitchTokenRefresh(): Promise<false | TwitchDataTypes.AuthTokenResult> {
			let twitchAuthResult: TwitchDataTypes.AuthTokenResult = JSON.parse(
				DataStoreCommon.get(DataStoreCommon.TWITCH_AUTH_TOKEN),
			);
