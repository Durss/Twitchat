...
import StoreProxy, {
	...
	type IDonationGoalGetters,
	type IDonationGoalState,
} from "../StoreProxy";
...
export const storeDonationGoals = defineStore("donationGoals", {
	...
	actions: {
		...
		removeOverlay(id: string): void {
			...
			StoreProxy.main
				.confirm(
					t("donation_goals.delete_confirm.title"),
					t("donation_goals.delete_confirm.description"),
				)
				.then(() => {
		...
		broadcastData(overlayId?: string): void {
			for (const overlay of this.overlayList) {
				...
				if (overlay.dataSource == "streamlabs_charity") {
					...
					raisedPersonnal =
						StoreProxy.streamlabs.charityTeam.amountRaisedPersonnal_cents / 100;
				}

				if (overlay.dataSource == "tiltify") {
					if (!StoreProxy.tiltify.campaignList) continue;
					const campaign = StoreProxy.tiltify.campaignList.find(
						(v) => v.id == overlay.campaignId,
					);
				...
				if (overlay.dataSource == "twitch_charity") {
					...
					goal =
						campaign.target_amount.value /
						Math.pow(10, campaign.target_amount.decimal_places);
					raisedTotal = raisedPersonnal =
						campaign.current_amount.value /
						Math.pow(10, campaign.current_amount.decimal_places);
				}

				if (overlay.dataSource == "counter") {
					const counter = StoreProxy.counters.counterList.find(
						(v) => v.id == overlay.counterId,
					);
					if (!counter) continue;
					goal = counter.max || 0;
					raisedTotal = raisedPersonnal = counter.value;
				}
				...
				if (overlay.dataSource == "twitch_subs") {
					...
					goal = subs;
					raisedTotal = raisedPersonnal = subs;
				}

				if (overlay.dataSource == "twitch_followers") {
					const followers =
						(StoreProxy.labels.getLabelByKey("FOLLOWER_COUNT") as number) || 0;
					if (isNaN(followers)) continue;
					goal = followers;
					raisedTotal = raisedPersonnal = followers;
				}

				const currentStepIndex = overlay.goalList
					.concat()
					.sort((a, b) => a.amount - b.amount)
					.findLastIndex((v) => v.amount <= raisedTotal);
				if (donationGoalStatesCache[overlay.id] != currentStepIndex) {
					if (donationGoalStatesCache[overlay.id]! < currentStepIndex) {
						...
						void TriggerActionHandler.instance.execute(triggerEvent);
					}
					donationGoalStatesCache[overlay.id] = currentStepIndex;
				}

				PublicAPI.instance.broadcast("ON_DONATION_GOALS_OVERLAY_CONFIGS", {
					params: overlay,
					goal,
					raisedTotal,
					raisedPersonnal,
