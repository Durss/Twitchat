...
export const storeMain: StoreDefinition<"main", IMainState, IMainGetters, IMainActions> =
	defineStore("main", {
		state: (): IMainState =>
			({
				latestUpdateIndex: 20 as IMainState["latestUpdateIndex"],
				initComplete: false as IMainState["initComplete"],
				devmode: false as IMainState["devmode"],
				messageExportState: null as IMainState["messageExportState"],
				ahsInstaller: null as IMainState["ahsInstaller"],
				tooltip: "" as IMainState["tooltip"],
				cypherKey: "" as IMainState["cypherKey"],
				cypherEnabled: false as IMainState["cypherEnabled"],
				tempStoreValue: null as IMainState["tempStoreValue"],
				confirmData: null as IMainState["confirmData"],
				chatAlertParams: {
					chatCmd: "!alert",
					message: true,
					shake: true,
				...
				} as IMainState["chatAlertParams"],
				chatAlert: null as IMainState["chatAlert"],
				iconCache: {} as IMainState["iconCache"],
				outdatedDataVersion: false as IMainState["outdatedDataVersion"],
				offlineMode: false as IMainState["offlineMode"],
		...
		getters: {
			nonPremiumLimitExceeded: () => {
				...
				const triggersLength = StoreProxy.triggers.triggerList.filter(
					(v) =>
						v.enabled !== false &&
						StoreProxy.triggers.triggerIdToFolderEnabled[v.id] !== false,
				).length;

				return (
					StoreProxy.counters.counterList.filter((v) => v.enabled != false).length >
						Config.instance.MAX_COUNTERS ||
					StoreProxy.values.valueList.filter((v) => v.enabled != false).length >
						Config.instance.MAX_VALUES ||
					triggersLength > Config.instance.MAX_TRIGGERS ||
					StoreProxy.heat.screenList.filter((v) => v.enabled != false).length >
						Config.instance.MAX_CUSTOM_HEAT_SCREENS ||
					StoreProxy.users.customBadgeList.filter((v) => v.enabled != false).length >
						Config.instance.MAX_CUSTOM_BADGES ||
					Object.keys(StoreProxy.users.customUserBadges).length >
						Config.instance.MAX_CUSTOM_BADGES_ATTRIBUTION ||
					Object.keys(StoreProxy.users.customUsernames).length >
						Config.instance.MAX_CUSTOM_USERNAMES ||
					StoreProxy.heat.distortionList.filter((v) => v.enabled).length >
						Config.instance.MAX_DISTORTION_OVERLAYS ||
					StoreProxy.bingoGrid.gridList.filter((v) => v.enabled).length >
						Config.instance.MAX_BINGO_GRIDS ||
					StoreProxy.labels.labelList.filter((v) => v.enabled).length >
						Config.instance.MAX_LABELS ||
					StoreProxy.timers.timerList.filter((v) => v.enabled && !v.isDefault).length >
		...
		actions: {
			...
			async startApp(authenticate: boolean, callback: (value: unknown) => void) {
				...
				for (const key in TriggerTypes) {
					...
					if (uniqueIdsCheck[v] === true) {
						StoreProxy.common.alert(
							"Duplicate trigger type id (" + v + ') found for trigger "' + key + '"',
						);
						break;
					}
			...
			onAuthenticated(): void {
				...
				} else //Warn the user about the new ad break capabilities
				if (
					DataStore.get(DataStore.AD_BREAK_SCOPES_REQUEST) !== "true" &&
				...
				} else //Ask the user if they want to make their donation public
				if (
					!DataStore.get(DataStore.TWITCHAT_SPONSOR_PUBLIC_PROMPT) &&
					sAuth.donorLevel > -1
				) {
					window.setTimeout(() => {
						sChat.sendTwitchatAd(
				...
				} else //Show "right click message" hint
				...
				const sParams = StoreProxy.params;
				if (
					sParams.appearance.bttvEmotes.value === true &&
					sParams.appearance.showEmotes.value === true
				) {
					void BTTVUtils.instance.enable();
				} else {
					void BTTVUtils.instance.disable();
				}
				if (
					sParams.appearance.ffzEmotes.value === true &&
					sParams.appearance.showEmotes.value === true
				) {
					void FFZUtils.instance.enable();
				} else {
					void FFZUtils.instance.disable();
				}
				if (
					sParams.appearance.sevenTVEmotes.value === true &&
					sParams.appearance.showEmotes.value === true
				) {
					void SevenTVUtils.instance.enable();
				} else {
					void SevenTVUtils.instance.disable();
				}
				...
				PublicAPI.instance.addEventListener("SET_SHOUTOUT_LAST_RAIDER", () => {
					...
					if (raider) {
						...
						void sUsers.shoutout(me.id, raider);
					} else {
				...
				PublicAPI.instance.addEventListener("SET_RAFFLE_PICK_WINNER", () => {
					...
					void StoreProxy.raffle.pickWinner(list[0]!.sessionId || "");
				});
				 ...
				 */
				PublicAPI.instance.addEventListener("SET_CHAT_HIGHLIGHT_OVERLAY_MESSAGE", (e) => {
				 ...
				 */
				PublicAPI.instance.addEventListener("GET_GLOBAL_STATES", () =>
				...
				GoXLRSocket.instance.addEventListener(
					GoXLRSocketEvent.BUTTON_PRESSED,
					onGoXLRButton,
				);
				GoXLRSocket.instance.addEventListener(
					GoXLRSocketEvent.BUTTON_RELEASED,
					onGoXLRButton,
				);
				 ...
				 */
				GoXLRSocket.instance.addEventListener(
					GoXLRSocketEvent.SAMPLE_PLAYBACK_COMPLETE,
					(e: GoXLRSocketEvent) => {
				 ...
				 */
				GoXLRSocket.instance.addEventListener(
					GoXLRSocketEvent.ENCODER,
					(e: GoXLRSocketEvent) => {
						const configs = [
							StoreProxy.params.goxlrConfig.chatScrollSources,
							StoreProxy.params.goxlrConfig.chatReadMarkSources,
						];
						for (let j = 0; j < configs.length; j++) {
							...
							for (let i = 0; i < config.length; i++) {
								...
								if (!column) continue;
								if (
									column[0] == indexToButtonId[e.fxIndex!] &&
									column[1] == e.encoderId!
								) {
									const id = e.encoderId!;
									const value = e.encoderValue!;
									const encoderParams =
										GoXLRSocket.instance.getEncoderPossibleValues(id);
									const resetValue =
										encoderParams.values[
											Math.round(encoderParams.values.length / 2)
										];
									if (value !== resetValue && resetValue) {
										...
										const delta = Math.round(
											(index2 - index1) / encoderParams.step,
										);
										//Scroll chat column
										if (j == 0) {
											PublicAPI.instance.broadcast(
												"SET_CHAT_FEED_SCROLL",
												{ colIndex: i, scrollBy: delta, mode: "pixels" },
												true,
											);
										} else {
											PublicAPI.instance.broadcast(
												"SET_CHAT_FEED_READ",
												{ colIndex: i, count: delta },
												true,
											);
										}
										...
										//Reset encoder value
										void GoXLRSocket.instance.setEncoderValue(id, resetValue);
									}
								}
							}
						}
				...
				if (
					DataStore.get(DataStore.HEAT_ENABLED) === "true" &&
					StoreProxy.auth.twitch.user
				) {
					void HeatSocket.instance.connect(StoreProxy.auth.twitch.user.id);
				}
				 ...
				 */
				void StoreProxy.common.initialize(true);

				/**
				 * Execute twitchat start trigger
				 */
				void TriggerActionHandler.instance.execute(
					{
						id: Utils.getUUID(),
						channel_id: StoreProxy.auth.twitch.user.id,
						date: Date.now(),
			...
			loadDataFromStorage() {
				...
				void StoreProxy.mixitup.populateData();
				...
				if (alert) {
					Utils.mergeRemoteObject(
						JSON.parse(alert),
						this.chatAlertParams as unknown as JsonObject,
					);
				}

				void Database.instance.connect().then(async () => {
			...
			setCypherEnabled(payload: boolean) {
				this.cypherEnabled = payload;
			},
			...
			setAhsInstaller(value: TwitchatDataTypes.InstallHandler) {
				this.$state.ahsInstaller = value;
			},
			...
			ThisType<
				IMainActions &
					UnwrapRef<IMainState> &
					_StoreWithState<"main", IMainState, IMainGetters, IMainActions> &
					_StoreWithGetters<IMainGetters> &
