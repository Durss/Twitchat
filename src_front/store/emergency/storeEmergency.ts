	...
	actions: {
		populateData() {
			...
			PublicAPI.instance.addEventListener("SET_EMERGENCY_MODE", (e) => {
				...
				if (e.data?.promptConfirmation === true) {
					StoreProxy.main
						.confirm(
							StoreProxy.i18n.t("emergency.enable_confirm"),
							undefined,
							undefined,
							undefined,
							undefined,
						...
						.then(() => {
							void this.setEmergencyMode(true);
						})
						.catch(() => {});
				} else {
					void this.setEmergencyMode(enabled);
				}
			});
		},
		...
		async setEmergencyMode(enable: boolean): Promise<void> {
			...
			if (enable) {
				...
				if (this.params.enableShieldMode) {
					void TwitchUtils.setShieldMode(channelId, true);
				} else {
					...
					if (this.params.followOnly)
						roomSettings.followOnly = this.params.followOnlyDuration;
				...
				if (this.params.toUsers) {
					...
					for (let i = 0; i < usersNames.length; i++) {
						StoreProxy.users.getUserFrom(
							...
							undefined,
							(u) => {
								userToPrevModState[channelId]![u.id] =
									u.channelInfo[channelId]!.is_moderator === true;
								void TwitchUtils.banUser(
									u,
									channelId,
									30 * 60,
									"Timed out because the emergency mode has been triggered on Twitchat",
								);
							},
							undefined,
							undefined,
				...
				if (this.params.obsScene)
					void OBSWebsocket.instance.setCurrentScene(this.params.obsScene);
				...
				//Unset all changes
				if (this.params.enableShieldMode) {
					void TwitchUtils.setShieldMode(channelId, false);
				} else {
					const roomSettings: TwitchatDataTypes.IRoomSettings = {};
		...
		reloadFollowbotList(json: any) {
			...
			this.follows = json
				...
				.map(
					(
						entry: TwitchatDataTypes.EmergencyFollowEntryData,
		...
		saveFollowbotList() {
			const saved: TwitchatDataTypes.EmergencyFollowEntryData[] = this.follows.map(
				(entry) => {
					return {
