	...
	actions: {
		...
		createCustomTrain(): void {
			this.customTrainList.push({
				...
				approachingLabel: StoreProxy.i18n.t(
					"overlay.customTrain.param_approachingLabel_default",
				),
				approachingEmote:
					"https://static-cdn.jtvnw.net/emoticons/v2/emotesv2_c1f4899e65cf4f53b2fd98e15733973a/animated/dark/2.0",
				successLabel: StoreProxy.i18n.t("overlay.customTrain.param_successLabel_default"),
				successLabelSummary: StoreProxy.i18n.t(
					"overlay.customTrain.param_successLabel_summary",
				),
				successEmote:
					"https://static-cdn.jtvnw.net/emoticons/v2/emotesv2_bf888b2af57b4abd80653dff26768ae5/animated/dark/2.0",
				...
				failedEmote:
					"https://static-cdn.jtvnw.net/emoticons/v2/emotesv2_db3cd788399347a8b2ebfb8a85f5badb/static/dark/2.0",
				recordLabel: StoreProxy.i18n.t("overlay.customTrain.param_recordLabel_default"),
				recordEmote:
					"https://static-cdn.jtvnw.net/emoticons/v2/emotesv2_dee4ecfb7f0940bead9765da02c57ca9/static/dark/2.0",
				...
				expires_at: 0,
				coolDownEnd_at: 0,
				postLevelUpChatMessage: StoreProxy.i18n.t(
					"overlay.customTrain.param_postLevelUpChatMessage",
				),
				postSuccessChatMessage: StoreProxy.i18n.t(
					"overlay.customTrain.param_postSuccessChatMessage",
				),
				postSuccessOnChat: false,
				levelAmounts: [
					20, 50, 75, 100, 130, 170, 200, 250, 300, 350, 400, 500, 600, 700, 800, 900,
				],
				platforms: {
					kofi: true,
					streamelements: true,
		...
		deleteCustomTrain(id: string): void {
			...
			StoreProxy.main
				.confirm(
					StoreProxy.i18n.t("overlay.customTrain.delete_confirm.title"),
					StoreProxy.i18n.t("overlay.customTrain.delete_confirm.message"),
				)
				.then(() => {
		...
		registerActivity(
			...
			if (typeof amount !== "number" || isNaN(amount)) {
				console.warn(
					"CustomTrain: Ignoring invalid activity amount",
					amount,
					"from platform",
					platform,
				);
			...
			for (const train of this.customTrainList) {
				...
				const scheduleEnd = (isMissedTrain: boolean = false) => {
					...
					state.timeoutRef = SetTimeoutWorker.instance.create(() => {
						...
						train.coolDownEnd_at = isMissedTrain
							? Date.now() + 10_000
							: Date.now() + train.cooldownDuration_s * 1000;
						train.expires_at = 0;
						delete this.customTrainStates[train.id];
				...
				const currentLevel = () => {
					...
					if (state.activities.length <= train.triggerEventCount)
						return { index: 0, offset: 0, goal: 1 };
				...
				if (
					trainState.activities.length == 0 ||
				...
				// New all time record ?
				if (
					!train.testing && // Don't register if testing a fake train
					train.allTimeRecord &&
				...
				if (newLevel.index !== prevLevel.index) {
					...
					scheduleEnd();
					const amountLeft = Math.ceil(
						train.levelAmounts[newLevel.index - 1]! - trainState.amount,
					);
					...
					const message: TwitchatDataTypes.MessageCustomTrainLevelUpData = {
						...
						percent: Math.floor(
							((trainState.amount - newLevel.offset) / newLevel.goal) * 100,
						),
		...
		async simulateTrain(overlayId: string): Promise<void> {
			...
			for (let i = 1; i < Math.min(levels.length, 3); i++) {
				...
				const goal = levels[i]! - levels[i - 1]! * (Math.random() * 0.75 + 0.25);
