	...
	actions: {
		...
		async doRefreshToken(): Promise<void> {
			...
			if (result.json.success) {
				...
				void this.connect(this.accessToken, false);
			} else {
		...
		async connect(token: string, isReconnect: boolean = false): Promise<boolean> {
			...
			//Get socket path
			const socketPropsQuery = await fetch("https://api.tipeeestream.com/v2.0/site/socket", {
				method: "GET",
			});
			const socketProps = (await socketPropsQuery.json()) as {
				code: number;
				datas: { port: string; host: string };
			};

			let socketPath = socketProps.datas.host.replace(/https?:\/\//, "");
			if (socketProps.datas.port && socketProps.datas.port != "443")
				socketPath += ":" + socketProps.datas.port;

			socket = new WebSocket(
				`wss://${socketPath}/socket.io/?EIO=3&transport=websocket&access_token=${accessTokenJSON.apiKey}`,
			);
			...
			return new Promise<boolean>((resolve, _reject) => {
				socket = socket!; //Enforce non-null for typescript
				socket.onmessage = (event: MessageEvent<string>) => {
					...
					try {
						...
						const json = JSON.parse(
							event.data.replace(new RegExp("^" + code, ""), "") || "{}",
						);
						//Welcome message
						if (code == "0") {
							//Join room to get events
							socket?.send(
								`42["join-room",{"room":"${accessTokenJSON.apiKey}","username":"${meJSON.username}"}]`,
							);
						...
						//Authentication failed
							...
							if (isAutoInit) {
								StoreProxy.common.alert(
									StoreProxy.i18n.t("error.tipeee_connect_failed"),
								);
							}
							this.disconnect(false);
							resolve(false);
											...
											const data: TwitchatDataTypes.MessageTipeeeDonationData =
												{
													...
													amountFormatted:
														message.parameters.amount +
														message.parameters.currency,
													...
													recurringCount:
														message.parameters.recurring_counter || 0,
												};
