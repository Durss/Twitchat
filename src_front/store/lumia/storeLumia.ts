	...
	actions: {
		populateData(): void {
			...
			if (data) {
				...
				if (this.socketToken) {
					void this.connect(this.socketToken).then((success) => {
						if (!success) {
							StoreProxy.common.alert(
								StoreProxy.i18n.t("error.lumia_connect_failed"),
							);
						}
					});
				}
		...
		async connect(token: string, isReconnect: boolean = false): Promise<boolean> {
			...
			return new Promise<boolean>((resolve, _reject) => {
				socket = new WebSocket(
					`ws://127.0.0.1:39231/api?token=${token}&origin=Twitchat&name=Twitchat`,
				);
...
// oxlint-disable-next-line no-unused-vars
type PlatformCode = "twitch" | "youtube" | "facebook" | "trovo" | "glimesh";
