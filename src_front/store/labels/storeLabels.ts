...
import {
	...
	type LabelItemData,
	type LabelItemPlaceholder,
	type LabelItemPlaceholderTag,
	...
	actions: {
		populateData(): void {
			...
			this.placeholders["DATE"] = {
				value: Date.now(),
				category: "date",
				placeholder: LabelItemPlaceholderList.find((v) => v.tag == "DATE")!,
			};
			this.placeholders["TIME"] = {
				value: Date.now(),
				category: "date",
				placeholder: LabelItemPlaceholderList.find((v) => v.tag == "TIME")!,
			};
			this.placeholders["DATE_TIME"] = {
				value: Date.now(),
				category: "date",
				placeholder: LabelItemPlaceholderList.find((v) => v.tag == "DATE_TIME")!,
			};
		...
		removeLabel(labelId: string): void {
			...
			StoreProxy.main
				.confirm(
					t("overlay.labels.delete_confirm.title"),
					t("overlay.labels.delete_confirm.description"),
				)
				.then(() => {
		...
		async updateLabelValue(
			...
			if (this.placeholders[key] && this.placeholders[key].value != value) {
				...
				Sentry.captureMessage(
					"[LABELS] Trying to update unknown label placeholder: " + key,
				);
			}
		},
			...
			for (const key of userIdKeyList) {
				if (this.placeholders[key] && this.placeholders[key].value === userId) {
					...
					for (const labelKey in this.placeholders) {
						...
						if (labelKey.startsWith(prefix) && this.placeholders[typedKey]) {
				...
				for (const key in this.allPlaceholders) {
					...
					if (list[key].value === undefined) {
						if (
							...
							ph.placeholder.type == "datetime" ||
							ph.placeholder.type == "time"
						) {
							list[key].value = Date.now();
						} else if (
							ph.placeholder.type == "image" ||
							ph.placeholder.type == "string"
						) {
							list[key].value = "";
						} else if (
							ph.placeholder.type == "number" ||
							ph.placeholder.type == "duration"
						) {
							list[key].value = 0;
						}
					}
