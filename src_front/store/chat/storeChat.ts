...
export const storeChat = defineStore('chat', {
	...
	actions: {
		...
		async addMessage(message:TwitchatDataTypes.ChatMessageTypes) {
			...
			/*
			//Live translation. Disabled as it may cost me a lot even if limited to premium members
			if(sAuth.isPremium && TwitchatDataTypes.TranslatableMessageTypesString.hasOwnProperty(message.type)) {
				...
				if(text.length >= 4) {
					...
					const lang = res[0][0]
					if(lang != "fra" && Iso3To1Table[lang]) {
						console.log("Translate",res[0]);
						ApiController.call("google/translate", "POST", {langSource:Iso3To1Table[lang], langTarget:"fr", text:text}, false)
						.then(res=>{
