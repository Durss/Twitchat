	...
	actions: {
		populateData(): void {
			...
			if (data) {
				...
				if (this.userId) {
					void this.connect(this.userId).then((success) => {
						if (!success) {
							StoreProxy.common.alert(
								StoreProxy.i18n.t("error.streamfog_connect_failed"),
							);
						}
					});
				}
		...
		async listAllLenses(): Promise<Lense[] | false> {
			try {
				const res = await fetch(
					"https://api.streamfog.com/api/v2/camerakit-lenses/external",
					{
						headers: {
		...
		async listUserLenses(): Promise<boolean> {
			try {
				const res = await fetch(
					"https://api.streamfog.com/api/v2/user-content/inventory/" +
				...
				this.userLensesList = this.lensesList.filter((lense) =>
					lenseIds.includes(lense.id),
				);
		...
		async activateLense(lenseId: string, duration_ms?: number): Promise<boolean> {
			try {
				const res = await fetch(
					"https://api.streamfog.com/api/v2/camControl/activateLens",
					{
						method: "POST",
						headers: {
							"Content-Type": "application/json",
		...
		async playVideoAnimation(videoId: string): Promise<boolean> {
			try {
				const res = await fetch(
					"https://api.streamfog.com/api/v2/camControl/playVideoAnim",
					{
						method: "POST",
						headers: {
							"Content-Type": "application/json",
