		...
		populateData() {
			...
			if (obsMuteUnmuteCommands) {
				Utils.mergeRemoteObject(
					JSON.parse(obsMuteUnmuteCommands),
					this.muteUnmuteCommands as unknown as JsonObject,
				);
			...
			if (obsCommandsPermissions) {
				Utils.mergeRemoteObject(
					JSON.parse(obsCommandsPermissions),
					this.commandsPermissions as unknown as JsonObject,
				);
			...
			if (this.connectionEnabled && (port != null || pass != null || ip != null)) {
				this.connectionEnabled = true;
				void OBSWebsocket.instance.connect(
					port || "4455",
					pass || "",
					true,
					ip || "127.0.0.1",
			...
			OBSWebsocket.instance.addEventListener("ON_OBS_WEBSOCKET_CONNECTED", async () => {
				...
				filteredSources.forEach((browserSource) => {
					void OBSWebsocket.instance
						.getSourceSettings<{
							is_local_file: boolean;
							url: string;
							local_file: string;
						}>(browserSource.inputName)
						.then((browserSourceSettings) => {
							const urlRaw = (
								(browserSourceSettings.inputSettings.url as string) || ""
							).trim();
							if (!urlRaw) return;
							...
							try {
								...
								if (
									/.*twitchat.fr$/gi.test(url.hostname) ||
									(/^\/overlay\/.+$/gi.test(url.pathname) &&
										url.port == "8081" &&
									...
									if (url.toString() != prevURL) {
										...
										console.log(
											'ℹ️ Updating browser source URL from "' +
												prevURL +
												'" to "' +
												url.toString() +
												'"',
										);
										void OBSWebsocket.instance.setBrowserSourceURL(
											browserSource.inputName,
											url.toString(),
										);
									}
			...
			OBSWebsocket.instance.addEventListener("ON_OBS_STREAM_STATE", (event): void => {
				let m:
					| TwitchatDataTypes.MessageOBSStartStreamData
					| TwitchatDataTypes.MessageOBSStopStreamData = {
					id: Utils.getUUID(),
					date: Date.now(),
					platform: "twitchat",
				...
				if (!event.data.outputActive) {
					m = { ...m, type: TwitchatDataTypes.TwitchatMessageType.OBS_STOP_STREAM };
				}
				void TriggerActionHandler.instance.execute(m);
			});
			...
			OBSWebsocket.instance.addEventListener("ON_OBS_RECORD_STATE", (event): void => {
				const type = event.data.outputActive
					? TwitchatDataTypes.TwitchatMessageType.OBS_RECORDING_START
					: TwitchatDataTypes.TwitchatMessageType.OBS_RECORDING_STOP;
			...
			function onPlayBackStateChanged(
				event:
					| {
							type: "ON_OBS_PLAYBACK_ENDED";
							data: TwitchatEventMap["ON_OBS_PLAYBACK_ENDED"];
					  }
					| {
							type: "ON_OBS_PLAYBACK_STARTED";
							data: TwitchatEventMap["ON_OBS_PLAYBACK_STARTED"];
					  }
					| {
							type: "ON_OBS_PLAYBACK_PAUSED";
							data: TwitchatEventMap["ON_OBS_PLAYBACK_PAUSED"];
					  }
					| {
							type: "ON_OBS_PLAYBACK_NEXT";
							data: TwitchatEventMap["ON_OBS_PLAYBACK_NEXT"];
					  }
					| {
							type: "ON_OBS_PLAYBACK_PREVIOUS";
							data: TwitchatEventMap["ON_OBS_PLAYBACK_PREVIOUS"];
					  }
					| {
							type: "ON_OBS_PLAYBACK_RESTARTED";
							data: TwitchatEventMap["ON_OBS_PLAYBACK_RESTARTED"];
					  },
			): void {
				const data = event.data;
				const typeToState: Partial<{
					[key in keyof TwitchatEventMap]: TwitchatDataTypes.MessageOBSPlaybackStateValue;
				}> = {};
			...
			OBSWebsocket.instance.addEventListener("ON_OBS_FILTER_NAME_CHANGED", (event): void => {
				...
				StoreProxy.triggers.renameOBSFilter(
					data.sourceName,
					data.oldFilterName,
					data.filterName,
				);
			...
			OBSWebsocket.instance.addEventListener("ON_OBS_PLAYBACK_ENDED", (e) =>
				onPlayBackStateChanged(e as any),
			);
			OBSWebsocket.instance.addEventListener("ON_OBS_PLAYBACK_STARTED", (e) =>
				onPlayBackStateChanged(e as any),
			);
			OBSWebsocket.instance.addEventListener("ON_OBS_PLAYBACK_PAUSED", (e) =>
				onPlayBackStateChanged(e as any),
			);
			OBSWebsocket.instance.addEventListener("ON_OBS_PLAYBACK_NEXT", (e) =>
				onPlayBackStateChanged(e as any),
			);
			OBSWebsocket.instance.addEventListener("ON_OBS_PLAYBACK_PREVIOUS", (e) =>
				onPlayBackStateChanged(e as any),
			);
			OBSWebsocket.instance.addEventListener("ON_OBS_PLAYBACK_RESTARTED", (e) =>
				onPlayBackStateChanged(e as any),
			);
		},
			...
			if (
				...
				for (const scene of this.sceneCommands) {
					if (scene.command.trim().toLowerCase() == cmd) {
						void OBSWebsocket.instance.setCurrentScene(scene.scene.sceneName);
					}
				}
				...
				if (audioSourceName) {
					...
					if (cmd == this.muteUnmuteCommands.muteCommand) {
						void OBSWebsocket.instance.setMuteState(audioSourceName, true);
					}
					if (cmd == this.muteUnmuteCommands.unmuteCommand) {
						void OBSWebsocket.instance.setMuteState(audioSourceName, false);
					}
				}
			}
		},
