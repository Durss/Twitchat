...
export const storeStreamelements = defineStore("streamelements", {
	...
	actions: {
		...
		async getOAuthURL(): Promise<string> {
			...
			const redirectURI =
				"https://twitchat.fr" +
				StoreProxy.router.resolve({ name: "streamelements/auth" }).href;
		...
		async connect(token: string, isReconnect: boolean = false): Promise<boolean> {
			...
			return new Promise<boolean>((resolve, _reject) => {
				...
				socket.onopen = async () => {
					...
					socket!.send(
						'42["authenticate",' +
							JSON.stringify({ method: "oauth2", token: token }) +
							"]",
					);
				};
				...
				socket.onmessage = (event: MessageEvent<string>) => {
					try {
						...
						//All other messages
							...
							void Utils.sha256(StoreProxy.auth.twitch.user.id).then((hash) => {
								if (
									hash ===
									"2a08ed3165c25849d65504d4f49042560bf2a62bef318cbc5627aee94aebe0fc"
								) {
									void ApiHelper.call("log", "POST", {
										cat: "random",
										log: { type: "STREAMELEMENTS_EVENT", data: json },
									});
								}
							});
							switch (action) {
