		...
		updateCounter(data: TwitchatDataTypes.CounterData): void {
			for (let i = 0; i < this.counterList.length; i++) {
				...
				if (counter.id == data.id) {
					...
					if (
						data.placeholderKey &&
						data.placeholderKey.toLowerCase() !=
							(counter.placeholderKey ?? "").toLowerCase()
					) {
						StoreProxy.triggers.renameCounterPlaceholder(
							counter.placeholderKey,
							data.placeholderKey,
						);
					}
					//Delete old, add new
							...
							StoreProxy.users.getUserFrom(
								...
								async (res) => {
									...
									if (loadedUsers == users.length) {
										...
										PublicAPI.instance.broadcast("ON_COUNTER_UPDATE", {
											counter: counter!,
										});
									}
								},
		...
		increment(
			...
			if (c.perUser) {
				...
				if (user) {
					...
					if (!c.users[user.id])
						c.users[user.id] = { login: user.login, value: 0, platform: user.platform };
			...
			switch (action) {
				case "DEL":
					counterValue -= value;
					value = -value;
					break; //Invert value so proper triggers are executed
				case "SET":
					counterValue = value;
					break;
				case "ADD":
