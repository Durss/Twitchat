...
export const storeRaffle = defineStore("raffle", {
	...
	actions: {
		populateData() {
			const rafflesRunning = JSON.parse(
				DataStore.get(DataStore.RAFFLES_RUNNING) || "[]",
			) as TwitchatDataTypes.RaffleData[];
		...
		async startRaffle(payload: TwitchatDataTypes.RaffleData) {
			...
			switch (payload.mode) {
				...
				case "tips": {
					//Announce start on chat
					const messageParams =
						payload.messages?.raffleStart ||
						StoreProxy.chat.botMessages.raffleTipsStart;
					if (messageParams.enabled) {
						const platforms: string[] = [];
						const minSuffix = StoreProxy.i18n.t("raffle.tips.minAmount_suffix");
						if (payload.tip_kofi) {
							let label = "Ko-Fi";
							if ((payload.tip_kofi_minAmount || 0) > 0)
								label += " (" + payload.tip_kofi_minAmount + "🪙" + minSuffix + ")";
							platforms.push(label);
						}
						if (payload.tip_streamlabs) {
							let label = "Streamlabs";
							if ((payload.tip_streamlabs_minAmount || 0) > 0)
								label +=
									" (" +
									payload.tip_streamlabs_minAmount +
									"🪙" +
									minSuffix +
									")";
							platforms.push(label);
						...
						if (payload.tip_streamlabsCharity) {
							...
							if ((payload.tip_streamlabsCharity_minAmount || 0) > 0)
								label +=
									" (" +
									payload.tip_streamlabsCharity_minAmount +
									"🪙" +
									minSuffix +
									")";
							platforms.push(label);
						...
						if (payload.tip_streamelements) {
							...
							if ((payload.tip_streamelements_minAmount || 0) > 0)
								label +=
									" (" +
									payload.tip_streamelements_minAmount +
									"🪙" +
									minSuffix +
									")";
							platforms.push(label);
						...
						if (payload.tip_tipeee) {
							let label = "Tipeee";
							if ((payload.tip_tipeee_minAmount || 0) > 0)
								label +=
									" (" + payload.tip_tipeee_minAmount + "🪙" + minSuffix + ")";
							platforms.push(label);
						}
						if (payload.tip_tiltify) {
							let label = "Tiltify";
							if ((payload.tip_tiltify_minAmount || 0) > 0)
								label +=
					...
					if (winnerLoc.user) {
						if (StoreProxy.params.features.raffleHighlightUser.value) {
							const user = StoreProxy.users.getUserFrom(
								winnerLoc.user.platform,
								winnerLoc.user.channel_id,
								winnerLoc.user.id,
								undefined,
								undefined,
								undefined,
							...
							window.setTimeout(
								...
								((StoreProxy.params.features.raffleHighlightUserDuration
									.value as number) ?? 0) *
									1000 *
									60,
							);
					...
					if (winner.tip) {
						...
						switch (winner.tip?.source) {
							...
							case "streamlabs_charity":
								platform = "Streamlabs Charity";
								break;
							case "streamlements":
								platform = "Streamlements";
								break;
							case "tipeee":
								platform = "Tipeee";
								break;
			...
			this.raffleList.forEach((raffle) => {
				...
				if (raffle.mode == "chat") {
					...
					if (
						...
						const cmd = (typedMessage.message || "")
							.trim()
							.split(" ")[0]!
							.toLowerCase();
						if (cmd == raffle.command.trim().toLowerCase()) canJoin = true;
					}
				} else //Check if can join from tip
				if (
					raffle.mode == "tips" &&
					((raffle.tip_kofi === true &&
						message.type == TwitchatDataTypes.TwitchatMessageType.KOFI &&
						message.eventType == "donation" &&
						message.amount >= (raffle.tip_kofi_minAmount || 0)) ||
						(raffle.tip_streamlabs === true &&
							message.type == TwitchatDataTypes.TwitchatMessageType.STREAMLABS &&
							message.eventType == "donation" &&
							message.amount >= (raffle.tip_streamlabs_minAmount || 0)) ||
						(raffle.tip_streamlabsCharity === true &&
							message.type == TwitchatDataTypes.TwitchatMessageType.STREAMLABS &&
							message.eventType == "charity" &&
							message.amount >= (raffle.tip_streamlabsCharity_minAmount || 0)) ||
						(raffle.tip_streamelements === true &&
							message.type == TwitchatDataTypes.TwitchatMessageType.STREAMELEMENTS &&
							message.eventType == "donation" &&
							message.amount >= (raffle.tip_streamelements_minAmount || 0)) ||
						(raffle.tip_tipeee === true &&
							message.type == TwitchatDataTypes.TwitchatMessageType.TIPEEE &&
							message.eventType == "donation" &&
							message.amount >= (raffle.tip_tipeee_minAmount || 0)) ||
						(raffle.tip_tiltify === true &&
							message.type == TwitchatDataTypes.TwitchatMessageType.TILTIFY &&
							message.eventType == "donation" &&
							message.amount >= (raffle.tip_tiltify_minAmount || 0)) ||
						(raffle.tip_twitchCharity === true &&
							message.type ==
								TwitchatDataTypes.TwitchatMessageType.TWITCH_CHARITY_DONATION &&
							message.amount >= (raffle.tip_twitchCharity_minAmount || 0)))
				) {
					canJoin = true;
					username =
						message.type ==
						TwitchatDataTypes.TwitchatMessageType.TWITCH_CHARITY_DONATION
							...
							: message.userName;
					const existingEntry = username
						? raffle.entries.find(
								(v) =>
									v.label.toLowerCase().trim() == username.toLowerCase().trim(),
							)
						: false;
					//Refuse if username is already registered
					if (existingEntry && raffle.multipleJoin !== true) return;
					let tipPlatform: NonNullable<(typeof raffle.entries)[number]["tip"]>["source"] =
						"kofi";
				...
				if (
					elapsed <= raffle.duration_s * 1000 &&
					...
					if (messageParams.enabled) {
						...
						while (joinMessage.length < 500 && userCount < confirmSpool.length) {
							userCount++;
							joinMessage = messageParams.message.replace(
								/\{USER\}/gi,
								confirmSpool.slice(0, userCount).join(", @"),
							);
						}
		...
		async pickWinner(
			...
			if (!forcedWinner && userList.length > 0) {
				...
				if ((data.subT2Ratio || data.subT3Ratio) && userList.length > 0) {
					...
					const res = await TwitchUtils.getSubscriptionState(
						userList.map((v) => v.user.id),
					);
					res.forEach((v) => {
						...
						//Sub tier 3
						if (data.subT3Ratio > 0 && v.tier == "3000")
							entry.score += data.subT3Ratio ?? 0;
						//Sub tier 2
						else if (data.subT2Ratio > 0 && v.tier == "2000")
							entry.score += data.subT2Ratio ?? 0;
						//Sub tier 1 & prime
				...
				for (const v of userList) {
					...
					//Apply VIP ratio
					if (data.vipRatio > 0 && user.channelInfo[channel_id]!.is_vip)
						v.entry.score += data.vipRatio;
					//Apply sub gifter ratio
					if (data.subgiftRatio > 0 && user.channelInfo[channel_id]!.is_gifter)
						v.entry.score += data.subgiftRatio;
					//Apply follower ratio
					if (data.followRatio > 0) {
						//If user follow state isn't loaded yet, get it
						if (user.channelInfo[channel_id]!.is_following === null)
							await sUsers.checkFollowerState(user, channel_id);
						if (user.channelInfo[channel_id]!.is_following === true)
							v.entry.score += data.followRatio;
					}
					...
					if (
						...
						user.channelInfo[channel_id]!.is_subscriber
					)
						v.entry.score += data.subRatio;
				}
			}
				...
				if (data.mode == "manual") {
					...
					if (customEntriesStr?.length > 0) {
						...
						StoreProxy.common.alert(
							StoreProxy.i18n.t("error.raffle.pick_winner_no_entry"),
						);
						return;
					}
					...
					subs = subs.filter((v) => {
						...
						if (
							data.subMode_includeGifters == true &&
							subs.find((v2) => v2.gifter_id == v.user_id)
						)
							return true;
					...
					if (subs.length === 0) {
						StoreProxy.common.alert(
							StoreProxy.i18n.t("error.raffle.pick_winner_no_subs"),
						);
						return;
					}
			...
			const wheelOverlayPresenceHandler = () => {
				wheelOverlayExists = true;
			};
			PublicAPI.instance.addEventListener(
				"ON_WHEEL_OVERLAY_PRESENCE",
				wheelOverlayPresenceHandler,
			);
			...
			if (wheelOverlayExists) {
				const list: TwitchatDataTypes.EntryItem[] = data.entries.map(
					(v: TwitchatDataTypes.RaffleEntry): TwitchatDataTypes.EntryItem => {
						return {
							id: v.id,
							label: v.label,
						};
		...
		saveData(): void {
			DataStore.set(
				DataStore.RAFFLES_RUNNING,
				this.raffleList.filter((v) => {
					return (
						v.mode != "sub" &&
						v.mode != "manual" &&
						v.mode != "values" &&
