	...
	actions: {
		populateData(): void {
			...
			SSEHelper.instance.addEventListener(SSEEvent.QNA_ACTION, async (event) => {
				...
				switch (data.action) {
					case "add_message": {
						...
						if (
							session &&
							session.shareWithMods &&
							session.messages.findIndex(
								(v) => v.message.id == data.message.message.id,
							) == -1
						) {
			...
			PublicAPI.instance.addEventListener("SET_QNA_SKIP", (event) => {
				const session = this.activeSessions.find((v) => v.id == event.data?.id);
				if (session) {
					void this.removeMessageFromSession(session.messages[0]!, session);
				}
			});
		...
		async addMessageToSession(
			...
			let chunks = (JSON.parse(JSON.stringify(message.message_chunks)) || []) as NonNullable<
				typeof message.message_chunks
			>;
			...
			if (session.shareWithMods) {
				if (session.ownerId != StoreProxy.auth.twitch.user.id) {
					...
					ApiHelper.call("mod/qna/message", "PUT", {
						...
						const index = session.messages.findIndex(
							(v) => v.message.id == qnaMessage.message.id,
						);
						session.messages.splice(index, 1);
					});
				} else {
		...
		async removeMessageFromSession(
			...
			const messageIndex = session.messages.findIndex(
				(v) => v.message.id == message.message.id,
			);
		...
		async handleChatCommand(
			...
			if (session && session.open) {
				if (upvoteMode) {
					...
					if (
						session.shareWithMods &&
						session.ownerId == StoreProxy.auth.twitch.user.id
					) {
						this.shareSessionsWithMods();
					}
				} else {
					...
					const isHighlightReward =
						message.type == TwitchatDataTypes.TwitchatMessageType.REWARD &&
						(message as TwitchatDataTypes.MessageRewardRedeemData).reward.id ==
							Config.instance.highlightMyMessageReward.id;
					if (isHighlightReward) return;

					void this.addMessageToSession(message, session);
				}
			}
		},
		...
		shareSessionsWithMods(): void {
			...
			shareDebounce = window.setTimeout(() => {
				void ApiHelper.call("mod/qna", "POST", {
					sessions: this.activeSessions.filter((v) => v.shareWithMods == true),
				});
			}, 500);
		},
		...
		highlightEntry(entry: TwitchatDataTypes.QnaSession["messages"][0]): void {
			...
			void StoreProxy.chat.highlightChatMessageOverlay(fakeMessage);
		},

		broadcastQnaList(): void {
			PublicAPI.instance.broadcast("ON_QNA_SESSION_LIST", {
				sessionList: this.activeSessions.map((v) => ({
					id: v.id,
					command: v.command,
					open: v.open,
				})),
