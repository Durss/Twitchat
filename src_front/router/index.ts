	...
	},
	{
		...
		redirect: () => {
			...
			if (params.code && !Utils.getQueryParameterByName("error")) {
				...
				void StoreProxy.twitchBot.completeOAuthProcess(params.code, params.csrf);
			} else {
	...
	},
	{
		...
		redirect: () => {
			...
			if (Utils.getQueryParameterByName("code")) {
				...
				sParams.openParamsPage(
					TwitchatDataTypes.ParameterPages.CONNECTIONS,
					TwitchatDataTypes.ParamDeepSections.TIPEEE,
				);
			} else {
			...
			if (Utils.getQueryParameterByName("code")) {
				...
				if (/beta-/gi.test(params.csrf)) {
					...
					document.location.href =
						"https://beta.twitchat.fr/streamelements/auth?" +
						new URLSearchParams(params);
				} else if (/alpha-/gi.test(params.csrf)) {
					params.csrf = params.csrf.replace(/^.*?-/, "");
					document.location.href =
					...
					params.csrf = params.csrf.replace(/^.*?-/, "");
					document.location.href =
						"http://localhost:8081/streamelements/auth?" + new URLSearchParams(params);
				} else {
					sStreamelements.setAuthResult(params.code, params.csrf);
					sParams.openParamsPage(
						TwitchatDataTypes.ParameterPages.CONNECTIONS,
						TwitchatDataTypes.ParamDeepSections.STREAMELEMENTS,
					);
				}
			} else {
