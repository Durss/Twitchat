...
export default class MessengerProxy {
	...
	public async sendMessage(
		message: string,
		targetPlatforms?: TwitchatDataTypes.ChatPlatform[],
		channelId?: string,
		...
		if (!hasPlatform || targetPlatforms!.indexOf("twitch") > -1) {
			if (
				(await TwitchMessengerClient.instance.sendMessage(
					channelId,
					message,
					replyTo,
					noConfirm,
	...
	private initialize(): void {
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.MESSAGE,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.DELETE_MESSAGE,
			(e: MessengerClientEvent) => this.onDeleteMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.WHISPER,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.SUB,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.CHEER,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.RAID,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.DISCONNECT,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.CLEAR_CHAT,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.REWARD,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.NOTICE,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.JOIN,
			(e: MessengerClientEvent) => this.onJoinLeave(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.LEAVE,
			(e: MessengerClientEvent) => this.onJoinLeave(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.CONNECTED,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.DISCONNECTED,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.REFRESH_TOKEN,
			(e: MessengerClientEvent) => this.onRefreshToken(e),
		);
		TwitchMessengerClient.instance.addEventListener(
			MessengerClientEvent.WATCH_STREAK,
			(e: MessengerClientEvent) => this.onMessage(e),
		);
	}
	...
	private onMessage(e: MessengerClientEvent): void {
		if (e.data && typeof e.data != "string") {
			//Typeof check only here for linting
	...
	private async handleTwitchatCommands(
		message: string,
		_targetPlatforms?: TwitchatDataTypes.ChatPlatform[],
		channelId?: string,
	): Promise<boolean> {
		...
		const hasChannelPoints =
			StoreProxy.auth.twitch.user.is_affiliate || StoreProxy.auth.twitch.user.is_partner;
		...
		const triggerCommands = StoreProxy.triggers.triggerList.filter(
			(v) => v.type == TriggerTypes.SLASH_COMMAND && v.chatCommand,
		);
		...
		if (cmd == "/devmode") {
			...
			const entry = StoreProxy.timers.timerList.find(
			...
			return true;
		} else if (cmd == "/countdownpause") {
			const entry = StoreProxy.timers.timerList.find(
				(v) => v.type == "countdown" && v.isDefault,
			)!;
			StoreProxy.timers.timerPause(entry.id);
			return true;
		} else if (cmd == "/countdownunpause") {
			const entry = StoreProxy.timers.timerList.find(
				(v) => v.type == "countdown" && v.isDefault,
			)!;
			StoreProxy.timers.timerUnpause(entry.id);
			return true;
		} else if (cmd == "/countdownstop") {
			const entry = StoreProxy.timers.timerList.find(
				(v) => v.type == "countdown" && v.isDefault,
			)!;
			StoreProxy.timers.timerStop(entry.id);
			return true;
		} else if (cmd == "/timerstart") {
			const entry = StoreProxy.timers.timerList.find(
				(v) => v.type == "timer" && v.isDefault,
			)!;
			StoreProxy.timers.timerStart(entry.id);
			return true;
		} else if (cmd == "/timeradd") {
			const entry = StoreProxy.timers.timerList.find(
				(v) => v.type == "timer" && v.isDefault,
			)!;
			const duration = this.paramsToDuration(params[0]!);
			...
			StoreProxy.timers.timerPause(entry.id);
			return true;
		} else if (cmd == "/timerunpause") {
			const entry = StoreProxy.timers.timerList.find(
				(v) => v.type == "timer" && v.isDefault,
			)!;
			StoreProxy.timers.timerUnpause(entry.id);
			return true;
		} else if (cmd == "/timerstop") {
			const entry = StoreProxy.timers.timerList.find(
				(v) => v.type == "timer" && v.isDefault,
			)!;
			StoreProxy.timers.timerStop(entry.id);
			...
			return true;
		} else if (cmd == "/__logmessages__") {
			console.log(StoreProxy.chat.messages);
			return true;
		} else if (cmd == "/__stopobsstream__") {
			void OBSWebsocket.instance.stopStreaming();
			return true;
		} else if (cmd == "/raid" && (!params[0] || params[0] == "[user]")) {
			if (
			...
			void TwitchUtils.snoozeNextAd();
			return true;
		} else if (cmd == "/userinfo" || cmd == "/user") {
			if (!params[0]) {
				...
				let username = params[0]!
					...
					.trim();
				if (params.length > 1) {
					const channelRef = StoreProxy.users.getUserFrom(
						"twitch",
						channelId,
						undefined,
						params[1],
			...
			return true;
		} else if (cmd == "/userfromid") {
			const res = await TwitchUtils.getUserInfo([params[0]!]);
			if (res.length === 0) {
				StoreProxy.common.alert(
					StoreProxy.i18n.t("error.user_param_not_found", {
						USER: "<mark>" + params[0] + "</mark>",
					}),
				);
			} else {
		...
		if (cmd == "/logself") {
			...
			void TTSUtils.instance.readNow(m);
			return true;
		} else if (cmd == "/ttsuser") {
			...
			try {
				...
				if (res.length == 0) {
					const notice: TwitchatDataTypes.MessageNoticeData = {
						...
						platform: "twitchat",
						noticeId: TwitchatDataTypes.TwitchatNoticeType.ERROR,
						message: StoreProxy.i18n.t("error.user_param_not_found", {
							USER: "<mark>" + username + "</mark>",
						}),
						channel_id: channelId,
					};
					void StoreProxy.chat.addMessage(notice);
			...
			return true;
		} else if (isAdmin && cmd == "/logusers") {
			console.log(StoreProxy.users.users);
			return true;
		} else if (isAdmin && cmd == "/betaadd") {
			void StoreProxy.admin.addBetaUser(
				params[0]!
					.toLowerCase()
					.replace(/[^a-z0-9_]+/gi, "")
					.trim(),
			...
			return true;
		} else if (isAdmin && cmd == "/betadel") {
			void StoreProxy.admin.removeBetaUser(
				params[0]!
					.toLowerCase()
					.replace(/[^a-z0-9_]+/gi, "")
					.trim(),
			...
			return true;
		} else if (isAdmin && cmd == "/betaclose") {
			void StoreProxy.admin.removeAllBetaUser();
			return true;
		} else if (isAdmin && cmd == "/betamigrate") {
			void StoreProxy.admin.migrateUserDataToProd(
				params[0]!
					.toLowerCase()
					.replace(/[^a-z0-9_]+/gi, "")
					.trim(),
			...
			return true;
		} else if (isAdmin && cmd == "/giftpremium") {
			void StoreProxy.admin.giftPremium(
				params[0]!
					.toLowerCase()
					.replace(/[^a-z0-9_]+/gi, "")
					.trim(),
			...
			return true;
		} else if (isAdmin && cmd == "/ungiftpremium") {
			void StoreProxy.admin.ungiftPremium(
				params[0]!
					.toLowerCase()
					.replace(/[^a-z0-9_]+/gi, "")
					.trim(),
			...
			return true;
		} else if (cmd == "/setstreamtags") {
			const tags = params
				.map((v) =>
					Utils.replaceDiacritics(v)
						.replace(/[^a-z0-9]/gi, "")
						.substring(0, 25)
						.trim(),
				)
			...
			const [user] = await TwitchUtils.getUserInfo(undefined, [
				params[0]!.toLowerCase().replace(/[^a-z0-9_]+/gi, ""),
			]);
			if (user) {
				void TwitchUtils.setSuspiciousUser(channelId, user.id, "ACTIVE_MONITORING");
			}
			return true;
		} else if (cmd == "/restrict") {
			const [user] = await TwitchUtils.getUserInfo(undefined, [
				params[0]!.toLowerCase().replace(/[^a-z0-9_]+/gi, ""),
			]);
			if (user) {
				void TwitchUtils.setSuspiciousUser(channelId, user.id, "RESTRICTED");
			}
			return true;
		} else if (cmd == "/unmonitor" || cmd == "/unrestrict") {
			const [user] = await TwitchUtils.getUserInfo(undefined, [
				params[0]!.toLowerCase().replace(/[^a-z0-9_]+/gi, ""),
			]);
			if (user) {
				void TwitchUtils.unsetSuspiciousUser(channelId, user.id);
			}
			return true;
		} else if (cmd == "/bingo") {
			//Open bingo form
			...
			if (params[0] == "number") {
				...
				if (params.length > 2 && parseInt(params[1]!).toString() === params[1]) {
					payload.customValueTolerance =
						Math.min(5, Math.max(0, parseInt(params[1]))) ?? 3;
			...
			if (discordChan) {
				...
				if (messageToSend.length == 0) {
					...
					const res = await ApiHelper.call("discord/message", "POST", {
						...
						channelId: discordChan,
					});
					if (res.status != 200) {
						switch (res.json.errorCode) {
							case "POST_FAILED":
								error = StoreProxy.i18n.t("error.discord.POST_FAILED", {
									CHANNEL: res.json.channelName,
								});
								break;
							case "MISSING_ACCESS":
							...
							case "UNKNOWN_CHANNEL":
								error = StoreProxy.i18n.t("error.discord.UNKNOWN_CHANNEL", {
									CHANNEL: res.json.channelName,
								});
								break;
							default:
			...
			await StoreProxy.debug.sendRandomFakeMessage<TwitchatDataTypes.ChatMessageTypes>(
				true,
				forcedMessage,
				(m) => {
			...
			return true;
		} else if (cmd == "/simulatechat" || cmd == "/spam" || cmd == "/megaspam") {
			clearInterval(this.spamInterval);
			...
			let forcedType: TwitchatDataTypes.TwitchatMessageStringType | undefined = countMode
			...
			this.spamInterval = window.setInterval(() => {
				if (incMode) {
					...
					const lorem = new LoremIpsum({ wordsPerSentence: { max: 8, min: 1 } });
					const message = lorem.generateSentences(Math.round(Math.random() * 2) + 1);
					void StoreProxy.debug.sendRandomFakeMessage<TwitchatDataTypes.ChatMessageTypes>(
						true,
						forcedMessage.replace(/\{LOREM\}/gi, message),
						(m) => {
			...
			void StoreProxy.main.executeChatAlert(messageData);
			...
			(await Utils.sha256(cmd || "")) ==
			"bc65801f9bd87286a84409d0c8a33d1ada366e703bfc41c09e9f166f7905594e"
		) {
			StoreProxy.groq.enabled = !StoreProxy.groq.enabled;
			return true;
		} else if (isAdmin && (cmd == "/userdata" || cmd == "/loaduserdata")) {
			if (params.length == 0) {
				...
				if (users.length == 0) {
					StoreProxy.common.alert(
						StoreProxy.i18n.t("error.user_param_not_found", { USER: params[0] }),
					);
				} else {
					...
					if (res.status === 200) {
						if (cmd === "/loaduserdata") {
							void DataStore.loadFromJSON(res.json.data);
						} else {
