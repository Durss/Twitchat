...
export default class StreamdeckSocket extends EventDispatcher {
			...
			this._socket.onclose = (event) => {
				...
				if (!isManualConnect && event.code !== 1002) {
					...
					this._tryAgainTo = window.setTimeout(() => {
						void this.connect(secretKey, ip, this._isMainApp);
	...
	public broadcast<Event extends keyof TwitchatEventMap>(
		...
		byPassConnectCheck: boolean = false,
	): void {
...
export class StreamdeckSocketEvent<
	Event extends keyof TwitchatEventMap = keyof TwitchatEventMap,
> extends Event {
	public static readonly MESSAGE = "message";

	constructor(
		type: string,
		public data?: { action: Event; data?: TwitchatEventMap[Event] },
	) {
		super(type);
	}
}
