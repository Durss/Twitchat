...
export default class ContextMenuHelper {
	 ...
	 * @param e
	 */
	public async messageContextMenu(
		e: MouseEvent | TouchEvent,
		message: TwitchatDataTypes.ChatMessageTypes,
		canModerateMessage: boolean = false,
		canModerateUser: boolean = false,
		...
		if (TwitchatDataTypes.IsTranslatableMessage[message.type] === true) {
			...
			if (message.platform == "twitch") {
				...
				if (tMessage.type == TwitchatDataTypes.TwitchatMessageType.MESSAGE) {
					...
					if (
						tMessage.channel_id != StoreProxy.auth.twitch.user.id &&
					...
					if (allowed) {
						options.push({
							label:
								StoreProxy.chat.messageMode == "message"
									? t("chat.context_menu.answer")
									: t("chat.context_menu.quote"),
							icon:
								StoreProxy.chat.messageMode == "message"
									? this.getIcon("icons/reply.svg")
									: this.getIcon("icons/quote.svg"),
							onClick: () => {
								...
								StoreProxy.chat.replyTo =
									tMessage as TwitchatDataTypes.MessageChatData;
							},
						});
					}
				}
			...
			if (
				...
				StoreProxy.auth.twitch.user.channelInfo[message.channel_id]?.is_moderator &&
				message.type == TwitchatDataTypes.TwitchatMessageType.MESSAGE
			) {
				const isPinned =
					StoreProxy.chat.pinnedTwitchMessage[message.channel_id]?.message_id ==
					message.id;
				options.push({
					label:
						isPinned === true
							? t("chat.context_menu.unpin_twitch")
							: t("chat.context_menu.pin_twitch"),
					icon:
						isPinned === true
			...
			if (canModerateMessage) {
				...
				if (m.deleted !== true) {
					...
					if (
						message.platform == "twitch" &&
						!TwitchUtils.hasScopes([TwitchScopes.DELETE_MESSAGES])
					)
						classes += " disabled";
					if (
			...
			if (canModerateUser) {
				...
				if (
					message.platform == "twitch" &&
					!TwitchUtils.hasScopes([TwitchScopes.EDIT_BANNED])
				)
					classesMod += " disabled";
				if (
					message.platform == "youtube" &&
				...
				if (
					message.platform == "twitch" &&
					!TwitchUtils.hasScopes([TwitchScopes.EDIT_BLOCKED])
				)
					...
					if (channelInfo && (channelInfo.is_restricted || channelInfo.is_suspicious)) {
						moreModerationOptions.push({
							label: channelInfo.is_restricted
								...
								: t("chat.context_menu.unsuspicious"),
							icon: channelInfo.is_restricted
								? this.getIcon("icons/unlock.svg")
								: this.getIcon("icons/shield.svg"),
							customClass: classesMod,
							onClick: () => {
								if (
									!TwitchUtils.requestScopes([
										TwitchScopes.MANAGE_SUSPICIOUS_USERS,
									])
								)
									return;
						...
						moreModerationOptions.push({
							...
							onClick: () => {
								if (
									...
									return;
								if (message.fake === true) {
									StoreProxy.users.flagRestrictedUser(
										tMessage.channel_id,
										tMessage.user,
									);
								} else {
						...
						moreModerationOptions.push({
							...
							onClick: () => {
								if (
									...
									return;
								if (message.fake === true) {
									StoreProxy.users.flagSuspiciousUser(
										tMessage.channel_id,
										tMessage.user,
									);
								} else {
		...
		if (
			...
			if (!isSafari && !Config.instance.OBS_DOCK_CONTEXT) {
				optionAdded = true;
				options.push({
					label: Config.instance.OBS_DOCK_CONTEXT
						? t("chat.context_menu.export_clipboard")
						: t("chat.context_menu.export"),
					icon: Config.instance.OBS_DOCK_CONTEXT
						? this.getIcon("icons/copy.svg")
						: this.getIcon("icons/download.svg"),
	...
	private translate(
		...
		ApiHelper.call(
			"google/translate",
			"GET",
			{ langSource: langSource.iso1, langTarget, text: text },
			false,
		)
			.then((res) => {
				if (res.status == 401) {
	 ...
	 * @param message
	 */
	private addCustomTriggerEntries(
		options: MenuItem[],
		message: TwitchatDataTypes.TranslatableMessage,
	): void {
...
> ${message.message}`;
		try {
			...
			const result = await ApiHelper.call("discord/ticket", "POST", {
				message: text,
				channelId,
				threadName,
			});
			if (result.json.success && result.json.messageLink) {
				MessengerProxy.instance.sendMessage(
					"@" + message.user.login + " " + result.json.messageLink!,
					[message.platform],
					message.channel_id,
				);
				...
				if (result.json.errorCode == "POST_FAILED") {
					StoreProxy.common.alert(
						StoreProxy.i18n.t("error.discord.MISSING_ACCESS", {
							CHANNEL: result.json.channelName,
						}),
					);
					return;
