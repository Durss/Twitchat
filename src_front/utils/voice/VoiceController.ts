...
export default class VoiceController {
		...
		this.recognition.onresult = async (event) => {
			...
			for (let i = event.resultIndex; i < event.results.length; ++i) {
				if (event.results[i]!.isFinal) {
					const finalText = event.results[i]![0]!.transcript.replace(
						this.lastTriggerKey,
						"",
					);
					if (this.remoteControlMode) {
						this.finalText.value = tempText_loc;
						PublicAPI.instance.broadcast("ON_STT_REMOTE_FINAL_TEXT_EVENT", {
							text: finalText,
						});
					} else {
			...
			if (this.remoteControlMode) {
				...
				PublicAPI.instance.broadcast("ON_STT_REMOTE_TEMP_TEXT_EVENT", {
					text: tempText_loc,
				});
			} else {
	...
	private initialize(): void {
		...
		PublicAPI.instance.addEventListener("SET_VOICE_CONTROL_STATE", (data) => {
			...
			if (enable) {
				void this.start(this.remoteControlMode);
			} else {
				this.stop();
			}
		});
		...
		for (const key in this.hashmap) {
			...
			if (index > -1) {
				...
				if (action && this.lastTriggerAction?.id !== action.id) {
					...
					if (action.id) this.triggerAction(action.id);
				}
			}
		}
	...
	private buildHashmaps(): void {
		...
		this.splitRegGlobalActions = new RegExp(
			"(?:^|\\s)(" + regChunks.join("|") + ")(?:(?:[^\\s]{0,1}$)|(?:[^\\s]{1}\\s)?|\\s)",
			"gi",
		);
	}
		...
		if (tempText_loc.length > 0) {
			...
			for (const chunk of chunks) {
				...
				for (const key in this.hashmapGlobalActions) {
					const typedKey = key as keyof TwitchatEventMap;
					if (
						this.hashmapGlobalActions[typedKey] &&
