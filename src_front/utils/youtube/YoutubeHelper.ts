...
import type {
	YoutubeAuthToken,
	YoutubeChannelInfo,
	YoutubeFollowerResult,
	YoutubeLiveBroadcast,
	YoutubeMessages,
} from "@/types/youtube/YoutubeDataTypes";
import { ref } from "vue";
...
export default class YoutubeHelper {
	...
	public connect(): void {
		...
		if (token) {
			...
			void this.refreshToken().then(async () => {
				...
				window.setTimeout(() => {
					...
					void this.getCurrentLiveBroadcast();
					void this.getLastestFollowers();
				}, 5000);
	...
	public async startAuthFlow(grantModerate: boolean): Promise<boolean> {
		...
		const oauth = await ApiHelper.call("youtube/oauthURL", "GET", {
			redirectURI,
			grantModerate,
		});
	...
	public async authenticate(code: string): Promise<YoutubeAuthToken | null> {
		...
		if (res.status == 200 && res.json.data.token && res.json.data.token.access_token) {
			...
			Logger.instance.log("youtube", {
				log:
					"User authenticated. Schedule auth token refresh in " +
	...
	public async getCurrentUserInfo(): Promise<void> {
		...
		try {
			...
			if (res.status == 200) {
				...
				user.avatarPath =
					this._userData.snippet.thumbnails.default.url ||
					this._userData.snippet.thumbnails.medium.url;
				const chanInfos = user.channelInfo[this._userData.id]!;
				chanInfos.is_broadcaster = true;
				chanInfos.is_moderator = true;
				Logger.instance.log("youtube", {
					log:
						"User infos loaded successfully. " +
						user.displayName +
						" (#" +
						user.id +
						")",
			...
			if (res.status == 200) {
				...
				const items = json.items
					.sort((a, b) => {
						...
						if (
							...
							return 1;
						if (
							a.status.lifeCycleStatus == "liveStarting" &&
						...
						if (
							...
							return 1;
						if (
							a.status.lifeCycleStatus == "ready" &&
						...
						if (
							...
							return 1;
						if (
							a.status.lifeCycleStatus == "testStarting" &&
							b.status.lifeCycleStatus != "testStarting"
						)
							return -1;
						if (
							a.status.lifeCycleStatus != "testStarting" &&
							...
							return 1;
						if (
							a.status.lifeCycleStatus == "testing" &&
					...
					.filter(
						(v) =>
							v.status.recordingStatus == "recording" ||
							v.status.recordingStatus == "notRecording",
					);
				...
				if (item) {
					...
					const messageNotification: TwitchatDataTypes.MessageCustomData = {
						...
						message: StoreProxy.i18n.t("chat.youtube.connected_to", {
							TITLE: item.snippet.title,
						}),
						col: 0,
					};
					void StoreProxy.chat.addMessage(messageNotification);
					Logger.instance.log("youtube", {
				...
				if (res.status == 401) {
					...
					try {
						...
						Logger.instance.log("youtube", {
							log:
								"Failed loading current live broadcast (status: " +
								res.status +
								")",
							error: json,
							credits: this._creditsUsed,
						...
						if (reason === "liveStreamingNotEnabled") {
							StoreProxy.common.alert(
								StoreProxy.i18n.t("error.youtube_no_broadcast"),
							);
							return null;
						}
	...
	public async refreshViewerCount(): Promise<void> {
		...
		try {
			...
			if (res.status == 200) {
				...
				StoreProxy.stream.currentStreamInfo["youtube"] = {
					//Super dirty """user""" ID I know :3. It should be an actual ID instead of "youtube"
					category: "",
					lastSoDoneDate: 0,
					live: true,
					started_at: 0,
	...
	public async getMessages(): Promise<void> {
			...
			try {
				...
				if (res.status == 200) {
					...
					for (; i < json.items.length; i++) {
						...
						const data = await this.parseMessage(
							m,
							this._liveIdToChanId[liveId]!,
							liveId,
						);
						if (data) {
					...
					try {
						...
						Logger.instance.log("youtube", {
							log:
								"Failed decoding JSON returned by youtube API (status: " +
								res.status +
								")",
							error: text,
							credits: this._creditsUsed,
		...
		const closestDelayEntry =
			delays.find(({ delay }) => delay >= elapsedTime) || delays[delays.length - 1]!;
	 ...
	 */
	public async getLastestFollowers(
		isInit: boolean = true,
	): Promise<YoutubeFollowerResult["items"]> {
		this._creditsUsed++;
		...
		try {
			...
			if (res.status == 200) {
				...
				json.items.forEach((v) => {
					if (!isInit && this._lastFollowerList[v.id] !== true) {
						...
						const user = StoreProxy.users.getUserFrom(
							"youtube",
							channelId,
							v.subscriberSnippet.channelId,
							v.subscriberSnippet.title,
							v.subscriberSnippet.title,
						);
				...
				this._pollFollowersTimeout = window.setTimeout(
					() => this.getLastestFollowers(false),
					60000,
				);
				return newFollowers;
			} else {
	 ...
	 */
	public async getLastestSubscribers(
		isInit: boolean = true,
	): Promise<YoutubeFollowerResult["items"]> {
		this._creditsUsed++;
		...
		try {
			const res = await fetch(url, { method: "GET", headers: this.headers });
			if (res.status == 200) {
				this._pollSubscribersTimeout = window.setTimeout(
					() => this.getLastestFollowers(false),
					60000 * 5,
				);
				//TODO
	...
	public async connectToLiveByURL(videoUrl: string): Promise<boolean> {
		...
		const extractID = (url: string): string | false => {
			let regExp =
				/(?:(?:youtu\.be\/|v\/|vi\/|u\/\w\/|embed\/|shorts\/)|(?:(?:watch)?\?v(?:i)?=|&v(?:i)?=))([^#&?]*)/g;
			let matches = [...url.matchAll(regExp)];
			return matches.length > 0 && matches[0]!.length > 0 && matches[0]![1]!.length == 11
			...
			try {
				...
				if (res.status >= 200 && res.status <= 204) {
					...
					json.items.forEach((ytUser) => {
						const user = StoreProxy.users.getUserFrom(
							"youtube",
							this._userData?.id || ytUser.id,
							ytUser.id,
							ytUser.snippet.title,
							ytUser.snippet.title,
							undefined,
							false,
							false,
						...
						user.avatarPath =
							ytUser.snippet.thumbnails.default.url ||
							ytUser.snippet.thumbnails.medium.url;
						users.push(user);
					});
				}
			} catch (_error) {}
	...
	private async refreshToken(): Promise<boolean> {
		...
		if (res.status == 200 && res.json.data.token) {
			...
			Logger.instance.log("youtube", {
				log:
					"Auth token refreshed successfully. Schedule next refresh in " +
					Utils.formatDuration(refreshDelay) +
					"s",
				credits: this._creditsUsed,
				liveID: this._currentLiveChatIds,
	...
	private async parseMessage(
		m: YoutubeMessages["items"][number],
		channelId: string,
		liveId: string,
	): Promise<TwitchatDataTypes.ChatMessageTypes | null> {
		//Create message
		const message = m.snippet.displayMessage || "";
		const message_chunks = m.snippet.displayMessage
			? this.parseMessageChunks(m.snippet.displayMessage)
			: [];
		...
		} else //Add badge if not already specified
		...
		switch (m.snippet.type) {
			...
			case "superStickerDetails": {
				const data: TwitchatDataTypes.MessageYoutubeSuperStickerData = {
					...
					sticker_url:
						StickerList[
							m.snippet.superStickerDetails.superStickerMetadata
								.stickerId as keyof typeof StickerList
						] || "",
			...
			case "memberMilestoneChatEvent": {
				const data: TwitchatDataTypes.MessageYoutubeSubscriptionData = {
					...
					message_chunks: this.parseMessageChunks(
						m.snippet.memberMilestoneChatDetails.userComment || "",
					),
					message_html,
					youtube_liveId: liveId,
					is_resub: false,
			...
			case "userBannedEvent": {
				const bannedUser = StoreProxy.users.getUserFrom(
					"youtube",
					channelId,
					m.snippet.userBannedDetails.bannedUserDetails.channelId,
					m.snippet.userBannedDetails.bannedUserDetails.displayName,
					m.snippet.userBannedDetails.bannedUserDetails.displayName,
				);
				void StoreProxy.users.flagBanned(
					...
					channelId,
					bannedUser.id,
					m.snippet.userBannedDetails.banDurationSeconds,
					user,
				);
				break;
			}
