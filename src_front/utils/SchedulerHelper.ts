		...
		switch (trigger.scheduleParams.type) {
			...
			case TriggerScheduleTypes.SPECIFIC_DATES: {
				for (const d of trigger.scheduleParams.dates) {
					...
					if (d.daily === true || d.monthly === true || d.yearly === true)
						date.setDate(new Date().getDate());
					if (d.monthly === true || d.yearly === true)
						date.setMonth(new Date().getMonth());
					if (d.yearly === true) date.setFullYear(new Date().getFullYear());

					//Date has past and not daily,monthly or yearly, skip it
					if (Date.now() > date.getTime() && !d.daily && !d.monthly && !d.yearly)
						continue;
	...
	private initialize(): void {
		...
		if (this._adSchedule.scheduleParams!.repeatDuration < 2 * 60 * 60) {
			StoreProxy.common.alert(
				"Ad schedule duration set to " +
					this._adSchedule.scheduleParams!.repeatDuration +
					" seconds instead of 7200 (2h)!",
			);
		} else if (this._adSchedule.scheduleParams!.repeatMinMessages < 100) {
			StoreProxy.common.alert(
				"Ad schedule min message count set to " +
					this._adSchedule.scheduleParams!.repeatMinMessages +
					" instead of 100!",
			);
		}
	...
	private computeFrame(): void {
		...
		for (let i = 0; i < this._pendingSchedules.length; i++) {
			...
			if (e.trigger.type == TriggerTypes.TWITCHAT_AD) {
				...
				if (
					(StoreProxy.auth.isPremium || StoreProxy.auth.donorLevel > -1) &&
					!StoreProxy.chat.botMessages.twitchatAd.enabled
				)
					continue;
				...
				if (
					...
					!/(^|\s|https?:\/\/)twitchat\.fr($|\s)/gi.test(
						StoreProxy.chat.botMessages.twitchatAd.message,
					)
				)
					continue;
			...
			switch (schedule.type) {
				case TriggerScheduleTypes.REGULAR_REPEAT: {
					...
					if (
						schedule.repeatMinMessages > 0 &&
						e.messageCount < schedule.repeatMinMessages
					)
						execute = false;
					//No min duration and no min message defined, ignore to avoid spam
					if (schedule.repeatDuration <= 0 && schedule.repeatMinMessages <= 0)
						execute = false;
					//schedule next repeat
					if (execute && schedule.repeatDuration)
			...
			if (execute) {
				...
				void TriggerActionHandler.instance.parseScheduleTrigger(e.trigger);
			}
		}
	}
}
