...
class TwitchatAnnouncement extends AbstractSidePanel {
	...
	public param_important: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "announcement.param_important",
		icon: "alert",
	};
	public param_dateStart: TwitchatDataTypes.ParameterData<string> = {
		type: "datetime",
		value: new Date().toISOString().substring(0, 16),
		labelKey: "announcement.param_dateStart",
		icon: "date",
	};
	public param_dateEnd: TwitchatDataTypes.ParameterData<string> = {
		type: "datetime",
		value: "",
		labelKey: "announcement.param_dateEnd",
		icon: "date",
	};
	public param_versionMax: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		value: "",
		labelKey: "announcement.param_versionMax",
	};
	public param_donorsOnly: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "announcement.param_donorsOnly",
		icon: "coin",
	};
	public param_premiumOnly: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "announcement.param_premiumOnly",
		icon: "premium",
	};
	public param_patreonOnly: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "announcement.param_patreonOnly",
		icon: "patreon",
	};
	public param_heatOnly: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
	...
	public beforeMount(): void {
		this.$i18n.availableLocales.forEach((v) => {
			this.param_title.push({
				type: "string",
				value: "",
				storage: v,
				label: v.toUpperCase() + " " + this.$t("announcement.param_title"),
			});
			this.param_text.push({
				type: "string",
				value: "",
				storage: v,
				longText: true,
				label: v.toUpperCase() + " " + this.$t("announcement.param_text"),
			});
		});
	...
	public async submitForm(): Promise<void> {
		...
		this.$confirm(
			this.$t("announcement.create_confirm.title"),
			this.$t("announcement.create_confirm.description"),
		)
			.then(async () => {
	...
	public deleteAnnounce(id: string): void {
		this.$confirm(
			this.$t("announcement.delete_confirm.title"),
			this.$t("announcement.delete_confirm.description"),
		)
			.then(async () => {
				const options = {
					method: "DELETE",
