<template>
	<div :class="classes">
		...
		<TTButton
			@click.capture="openModal('twitchatAnnouncement')"
			icon="announcement"
			v-if="isAdmin"
			secondary
		...
		<div class="commercial" v-tooltip="hasChannelPoints ? '' : $t('cmdmenu.not_affiliate')">
			...
			<TTButton
				...
				v-if="adCooldown == 0"
				small
				@click.capture="startAd(60)"
				:disabled="!canStartCommercial"
				>1'00</TTButton
			>
			<TTButton
				aria-label="Start a 90s ad"
				v-if="adCooldown == 0"
				small
				@click.capture="startAd(90)"
				:disabled="!canStartCommercial"
				>1'30</TTButton
			>
			<TTButton
				aria-label="Start a 120s ad"
				v-if="adCooldown == 0"
				small
				@click.capture="startAd(120)"
				:disabled="!canStartCommercial"
				>2'00</TTButton
			>
			<TTButton
				aria-label="Start a 150s ad"
				v-if="adCooldown == 0"
				small
				@click.capture="startAd(150)"
				:disabled="!canStartCommercial"
				>2'30</TTButton
			>
			<TTButton
				aria-label="Start a 180s ad"
				v-if="adCooldown == 0"
				small
				@click.capture="startAd(180)"
				:disabled="!canStartCommercial"
				>3'00</TTButton
			>
		...
		<ParamItem
			class="roomParam"
			:paramData="param_followOnly"
			v-model="param_followOnly.value"
			@change="setFollowOnly()"
			@click="requestRoomSettingsScopes()"
			noBackground
		/>
		<ParamItem
			class="roomParam"
			:paramData="param_subOnly"
			v-model="param_subOnly.value"
			@change="setSubOnly()"
			@click="requestRoomSettingsScopes()"
			noBackground
		/>
