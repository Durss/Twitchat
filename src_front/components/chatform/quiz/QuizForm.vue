				...
				>
					...
					<div class="form">
						<div class="overlayInstallCard">
							...
							<OverlayInstaller
								type="quiz"
								:sourceSuffix="quiz.title"
								:id="quiz.id"
								:queryParams="{ bid: quiz.id }"
							/>
						</div>
						...
						<Splitter
							><icon name="question" /> {{ $t("quiz.form.questionList") }}</Splitter
						>
						...
						>
							<div
								class="card-item"
								v-for="question in filteredQuestions(quiz)"
								:key="question.id"
							>
								<div class="actions">
									...
									<div class="innerGroup">
										<TTButton
											...
											@click="
												(e: MouseEvent) =>
													selectQuestionMode(e, quiz, question)
											"
											v-tooltip="
												$t('quiz.form.mode_' + question.mode + '.title')
											"
										/>

										<div
											class="durationOverride"
											v-tooltip="$t('quiz.form.durationOverride_tt')"
										>
											<TTButton
												v-if="!durationOverrideState[question.id]"
												icon="countdown"
												tabindex="-1"
												@click="setCustomDuration(quiz.id, question)"
												:secondary="
													question.duration_s && question.duration_s > 0
												"
												:transparent="!question.duration_s"
												>{{
													question.duration_s
														? question.duration_s + "s"
														: ""
												}}</TTButton
											>
											<span v-else class="durationForm">
												<ContentEditable
													...
													numeric
													@blur="
														durationOverrideState[question.id] = false
													"
												/>s
											</span>
										</div>
											...
											<TTButton
												...
												@click="
													(e: MouseEvent | TouchEvent) =>
														selectCustomTolerance(e, quiz, question)
												"
												:secondary="
													question.toleranceLevel !== undefined &&
													question.toleranceLevel >= 0
												"
												:transparent="
													question.toleranceLevel === undefined ||
													question.toleranceLevel < 0
												...
												>{{
													question.toleranceLevel == undefined
														...
														: {
																"0": $t(
																	"quiz.form.tolerances.none",
																).split(" ")[0],
																"1": $t(
																	"quiz.form.tolerances.very_low",
																).split(" ")[0],
																"2": $t(
																	"quiz.form.tolerances.low",
																).split(" ")[0],
																"3": $t(
																	"quiz.form.tolerances.medium",
																).split(" ")[0],
																"4": $t(
																	"quiz.form.tolerances.high",
																).split(" ")[0],
																"5": $t(
																	"quiz.form.tolerances.very_high",
																).split(" ")[0],
								...
								<div class="question">
									<div class="questionHolder">
										<ParamItem
											:paramData="param_question[question.id]"
											v-model="question.question"
											@blur="save(quiz)"
											noBackground
											:autofocus="autoOpenQuestionID === question.id"
										/>
									</div>
										...
										<ToggleBlock
											...
											:subtitle="
												question.mode == 'majority'
													? $t('quiz.form.answers_majority_subtitle')
													: ''
											"
											:title="
												question.mode == 'classic'
											...
											<div class="answerList">
												...
												>
													<TTButton
														...
														"
														:icon="
															answer.correct ? 'checkmark' : 'cross'
														"
						...
						<div
							class="card-item addQuestionBtns"
						...
						>
							...
							<div class="importForm">
								<TTButton
									...
									primary
									accept=".csv"
									@update:file="(file: File) => onCSVImport(quiz, file)"
									>{{ $t("quiz.form.import_bt") }}</TTButton
								>
								<Icon
									class="icon"
									name="info"
									v-tooltip="$t('quiz.form.import_tt')"
								/>
							</div>
							<TTButton
								@click="addQuestion(quiz, 'classic')"
								icon="quiz_classic"
								primary
							...
							>
							<TTButton
								@click="addQuestion(quiz, 'freeAnswer')"
							...
							>
							<TTButton
								@click="addQuestion(quiz, 'majority')"
								icon="quiz_majority"
								primary
...
class QuizForm extends AbstractSidePanel {
	...
	public get maxQuestionCount(): number {
		return this.$store.auth.isPremium
			? this.$config.MAX_QUESTIONS_PER_QUIZ_PREMIUM
			: this.$config.MAX_QUESTIONS_PER_QUIZ;
	}
	...
	): TwitchatDataTypes.QuizParams["questionList"] {
	...
	public selectCustomTolerance(
		...
		const list: { [key: string]: string } = this.$tm("quiz.form.tolerances") as {
			[key: string]: string;
		};
		const px =
	...
	public selectQuestionMode(
		...
		const menu: MenuOptions = {
			...
			items: [
				{
					icon: this.getIcon("icons/quiz_classic.svg"),
					customClass: question.mode === "classic" ? "selected" : "",
					label: this.$t("quiz.form.mode_classic.title"),
					onClick: () => this.changeQuestionMode(quiz, question, "classic"),
				},
				{
					icon: this.getIcon("icons/quiz_freeAnswer.svg"),
					customClass: question.mode === "freeAnswer" ? "selected" : "",
					label: this.$t("quiz.form.mode_freeAnswer.title"),
					onClick: () => this.changeQuestionMode(quiz, question, "freeAnswer"),
				},
				{
					icon: this.getIcon("icons/quiz_majority.svg"),
					customClass: question.mode === "majority" ? "selected" : "",
					label: this.$t("quiz.form.mode_majority.title"),
					onClick: () => this.changeQuestionMode(quiz, question, "majority"),
				},
			],
			mouseScroll: true,
			closeWhenScroll: false,
		...
		this.$store.quiz.quizList.forEach((quiz) => {
			...
			quiz.questionList.forEach((question) => {
				...
				if (!this.param_question[id]) {
					this.param_question[id] = {
						type: "string",
						value: "",
						maxLength: 300,
						placeholderKey: "quiz.form.question_placeholder",
					};
					this.param_answerDuration[id] = {
						type: "number",
						value: 0,
						labelKey: "quiz.form.param_answer_duration",
						icon: "timer",
					};
