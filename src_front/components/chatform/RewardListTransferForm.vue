	...
	<div class="rewardlisttransferform">
		<ul class="steps">
			<template v-for="(label, index) in <string[]>tm('rewards.manage.transfer_steps')">
				<li class="card-item" v-if="index < 3 || props.reward.image">
					<span class="index">{{ index + 1 }}</span>
					<i18n-t
						class="details"
						scope="global"
						tag="div"
						:keypath="'rewards.manage.transfer_steps[' + index + ']'"
					>
						<template #DASHBOARD>
							<TTButton
								primary
								...
								target="_blank"
								icon="newtab"
								>{{ t("rewards.manage.transfer_step_dashboardBt") }}</TTButton
							>
						</template>
						<template #REWARD
							><mark>{{ props.reward.title }}</mark></template
						>
						<template #RECREATE>
							<div class="transfered" v-if="recreated">
								<Icon name="checkmark" />{{
									t("rewards.manage.transfer_step_recreate_done")
								}}
							</div>
							<TTButton
								primary
								...
								icon="channelPoints"
								>{{ t("rewards.manage.transfer_step_recreateBt") }}</TTButton
							>
							<div
			...
			<li class="card-item icons" v-if="props.reward.image">
				<a
					v-if="props.reward.image?.url_1x"
					:href="props.reward.image.url_1x"
					target="_blank"
					:download="props.reward.title + '_28px.png'"
					v-tooltip="t('rewards.manage.download_icon_tt')"
				>
					<img :src="props.reward.image.url_1x" />
					<span>28x28</span>
				</a>
				<a
					v-if="props.reward.image?.url_2x"
					:href="props.reward.image.url_2x"
					target="_blank"
					:download="props.reward.title + '_56px.png'"
					v-tooltip="t('rewards.manage.download_icon_tt')"
				>
					<img :src="props.reward.image.url_2x" />
					<span>56x56</span>
				</a>
				<a
					v-if="props.reward.image?.url_4x"
					:href="props.reward.image.url_4x"
					target="_blank"
					:download="props.reward.title + '_112px.png'"
					v-tooltip="t('rewards.manage.download_icon_tt')"
				>
					<img :src="props.reward.image.url_4x" />
					<span>112x112</span>
				</a>
			</li>
		</ul>
...
const error = ref("");
const recreated = ref(false);
const tranfering = ref(false);

async function executeTransfer(): Promise<void> {
	error.value = "";
	const reward = props.reward!;
	tranfering.value = true;
