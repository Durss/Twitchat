...
class StreamSummary extends AbstractSidePanel {
	...
	public async beforeMount(): Promise<void> {
		...
		for (let i = messages.length - 1; i >= 0; i--) {
			...
			if (!dateOffset && prevDate > 0 && prevDate - m.date > 4 * 60 * 60000) {
				this.streamDuration = Utils.formatDuration(
			...
			switch (m.type) {
				case TwitchatDataTypes.TwitchatMessageType.MESSAGE: {
					...
					if (!userActivities[uid])
						userActivities[uid] = this.getEmptyUserActivities(m.user);
					this.messCount++;
				...
				case TwitchatDataTypes.TwitchatMessageType.HYPE_CHAT: {
					...
					if (!userActivities[uid])
						userActivities[uid] = this.getEmptyUserActivities(m.message.user);
				...
				case TwitchatDataTypes.TwitchatMessageType.REWARD: {
					...
					if (m.message) {
						const emoteCount = m.message_chunks!.filter(
							(v) => v.type == "emote",
						).length;
						this.messCount++;
						this.charCount += m.message.length;
	...
	public exportCSV(globalData: boolean = false): void {
		...
		if (globalData) {
			...
			for (const currency in this.hypeChats)
				csv += `${this.hypeChats[currency]}(${currency}) `;
			csv += ",";
