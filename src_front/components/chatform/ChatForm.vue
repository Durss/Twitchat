	...
	<div :class="classes">
				...
				>
					<ButtonNotification
						...
						v-tooltip="{
							...
							content:
								element.item.id == 'chatters' &&
								$store.params.appearance.showViewersCount.value === true
									? onlineUsersTooltip
									: element.tooltip,
						}"
			...
			<form @submit.prevent="" class="inputForm" name="messageform">
				...
				<div class="inputHolder" v-if="!error && !$store.chat.spamingFakeMessages">
					<div class="replyTo" v-if="$store.chat.replyTo">
						<button class="closeBt" type="button" @click="$store.chat.replyTo = null">
							<Icon name="cross" />
						</button>
						<div class="content">
							<i18n-t
								scope="global"
								:keypath="
									$store.chat.messageMode == 'message'
										? 'chat.form.reply_to'
										: 'chat.form.quoting'
								"
								tag="span"
							...
							>
								<template #USER>
									<a
										...
										@click.stop="
											openUserCard(
												$store.chat.replyTo!.user,
												$store.chat.replyTo!.channel_id,
											)
										"
										>{{ $store.chat.replyTo!.user.displayName }}</a
									>
								</template>
							</i18n-t>
					...
					<div
						class="inputField"
						:class="{ modAction: $store.chat.messageMode != 'message' }"
						:style="inputStyles"
					>
						<div class="actions">
							<ChannelSwitcher
							...
							<ModeratorActionSwitcher
								...
								v-model:mode="$store.chat.messageMode"
							/>

							<GroqChannelAction
								v-if="$store.groq.connected && $store.groq.enabled"
							/>
						</div>
						...
						<button
							...
							@click="$store.params.openParamsPage('connexions', 'youtube')"
						>
						...
						<button
							class="chatInputError"
							v-else-if="
								$store.stream.currentChatChannel.platform == 'youtube' &&
						...
						<button
							class="chatInputError"
							v-else-if="
								$store.stream.currentChatChannel.platform == 'twitch' &&
						...
						<input
							...
							:placeholder="
								$t('chat.form.input_placeholder', {
									CHANNEL: $store.stream.currentChatChannel.name,
								})
							"
							:maxlength="maxLength"
							@input="
			...
			<div class="rightForm">
				...
				<transition name="blink">
					<ButtonNotification
						...
						icon="quiz"
						v-if="
							$store.quiz.quizList.filter(
								(v) => v.enabled && v.questionList.length > 0,
							).length > 0
						"
				...
				<transition name="blink">
					<ButtonNotification
						...
						v-if="
							...
							$store.groq.answerHistory.length > 0
						"
				...
				<transition name="blink">
					<Icon
						class="error"
						name="spotify"
						v-if="$store.music.spotifyConsecutiveErrors > 5"
						v-tooltip="{
							touch: 'hold',
				...
				<transition name="blink">
					<TTButton
						...
						:aria-label="
							$store.emergency.emergencyStarted
								? $t('chat.form.emergency_stopBt_aria')
								: $t('chat.form.emergency_startBt_aria')
						"
						v-tooltip="{
							touch: 'hold',
							content: $store.emergency.emergencyStarted
								? $t('chat.form.emergency_stopBt_aria')
								: $t('chat.form.emergency_startBt_aria'),
						}"
		...
		<div class="floatingButtons">
			...
			<transition name="slide">
				<TTButton
					...
					secondary
					v-if="
						$store.voice.voicemodParams.voiceIndicator &&
						$store.voice.voicemodCurrentVoice
					"
					v-tooltip="{
						touch: 'hold',
				...
				>
					<template #icon>
						<img
							:src="
								'data:image/png;base64,' + $store.voice.voicemodCurrentVoice.image
							"
							alt=""
						/>
					</template>
		...
		<transition name="slide">
			<MessageExportIndicator
				...
				v-if="$store.main.messageExportState"
			/>
		</transition>
...
export class ChatForm extends Vue {
	...
	public get announcementTitle(): TwitchatDataTypes.ParseMessageChunk[] {
		const title =
			this.announcement!.title[this.$i18n.locale] || this.announcement!.title["en"]!;
	...
	public get showObsBtn(): boolean {
		return this.$store.obs.connectionEnabled === true && !OBSWebsocket.instance.connected.value;
	}

	public get qnaSessionActive(): boolean {
		return this.$store.qna.activeSessions.length > 0;
	}

	public get raffleListActive(): TwitchatDataTypes.RaffleData[] {
		return this.$store.raffle.raffleList.filter(
	...
	public get voiceBotStarted(): boolean {
	...
	public get inputStyles(): { [key: string]: string } {
		const currentChaninfo = this.$store.stream.connectedTwitchChans.find(
			(v) => v.user.id == this.$store.stream.currentChatChannel.id,
		);
		if (!currentChaninfo) return {};
		return { backgroundColor: currentChaninfo.color + "30" }; //30 is alpha channel of the color
	}

	public get cypherConfigured(): boolean {
		return this.$store.main.cypherKey?.length > 0;
	}
	...
	public get mustConnectYoutubeChan(): boolean {
		return (
			this.$store.stream.currentChatChannel.platform == "youtube" &&
	...
	public get isModeratedChannel(): boolean {
		...
		return (
			...
			this.$store.auth.twitchModeratedChannels.findIndex((v) => v.broadcaster_id == chanId) >
				-1
		);
	}
	...
	public get pinnedMenuItems() {
		...
		for (const item of items) {
			...
			const tooltip = this.pinDisableState(resultItem).disabled
				? this.$t("global.grant_scope")
				: menuItem.label || this.$t(menuItem.labelKey);
	...
	public get spotifyRateLimited(): boolean {
		return SpotifyHelper.instance.rateLimitedUntil.value > Date.now();
	}

	public get spotifyRateLimitedDurationFormated(): string {
		return Utils.formatDuration(SpotifyHelper.instance.rateLimitedUntil.value - Date.now());
	}

	public pinDisableState(item: (typeof this.pinnedMenuItems)[number]): {
		disabled: boolean;
	...
	public beforeMount(): void {
		...
		EventBus.instance.addEventListener(
			GlobalEvent.TRACK_USER,
			this.updateTrackedUserListHandler,
		);
		EventBus.instance.addEventListener(
			GlobalEvent.UNTRACK_USER,
			this.updateTrackedUserListHandler,
		);
		PublicAPI.instance.addEventListener(
			"SET_ENDING_CREDITS_PRESENCE",
			this.creditsOverlayPresenceHandler,
		);
	...
	public async mounted(): Promise<void> {
			...
			(newVal) => {
				if (newVal) {
					this.raidIntervalUpdate = window.setInterval(() => {
						this.remainingRaidTime = Utils.formatDuration(
		...
		gsap.from(this.$el, { y: 50, delay: 0.2, duration: 1, ease: "sine.out" });
	...
	public async sendMessage(event?: Event): Promise<void> {
		...
		if (this.sendHistory.length > maxHistorySize)
			this.sendHistory = this.sendHistory.splice(-maxHistorySize);
			...
			try {
				...
				if (
					await MessengerProxy.instance.sendMessage(
						...
						this.$store.stream.currentChatChannel.id,
						replyTo,
						false,
						false,
					)
	...
	public toggleEmergencyMode(): void {
		if (!this.$store.emergency.emergencyStarted) {
			this.$confirm(this.$t("emergency.enable_confirm"))
				.then(() => {
		...
		if (this.autoCompleteSearch) {
			for (let i = caretPos; i >= 0; i--) {
				...
				if (
					currentChar == ":" ||
					currentChar == "@" ||
					/\s/gi.test(currentChar) ||
					i == 0
				) {
					const offset = currentChar == ":" || currentChar == "@" ? 1 : 0;
					let prefix = localMessage.substring(0, i - offset);
					const suffix =
						localMessage.substring(i + 1 + this.autoCompleteSearch.length) + " ";
					if (prefix) prefix += " ";
			...
			localMessage =
				localMessage.substring(0, caretPos + 1) +
				code +
				localMessage.substring(caretPos + 1);
	...
	public onKeyDown(e: KeyboardEvent): void {
		...
		if (!this.openAutoComplete) {
			...
			if (e.key == "ArrowUp" || e.key == "ArrowDown") {
				...
				this.sendHistoryIndex = Math.min(
					this.sendHistory.length,
					Math.max(0, this.sendHistoryIndex),
				);
	...
	private getPinnedMenuItemFromId(
