...
class QnaForm extends AbstractSidePanel {
	...
	public param_upvote: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "qna.form.param_upvote",
		icon: "add",
	};
	public param_shareWithMods: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
	...
	public submitForm(): void {
		if (
			!this.$store.qna.createSession(
				this.param_command.value,
				this.param_upvote.value,
				this.param_shareWithMods.value,
			)
		) {
			this.conflict = true;
		} else {
