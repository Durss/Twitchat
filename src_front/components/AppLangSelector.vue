<template>
	<form class="applangselector">
		<div
			...
			@click.ctrl.stop.prevent="onCtrlClick(lang)"
		>
			<input
				type="radio"
				name="language"
			...
			<label :for="'lang_' + lang">
				<CountryFlag
					...
					class="flag"
				/><span class="text">{{ $t("global.lang_label", 1, { locale: lang }) }}</span>
				<Icon
					name="highlight"
					v-if="showRef !== false && langRef == lang"
					v-tooltip="'CTRL+click a language\nto make it the reference'"
				/>
			</label>
		</div>
	</form>
