	...
	<div class="emergencyfollowslistmodal modal">
		...
		<div class="holder" ref="holder">
			...
			<div class="head">
				<i18n-t
					scope="global"
					keypath="followbot.title"
					:plural="followers.length"
					tag="span"
					class="title"
				>
			...
			<div class="content">
				...
				<div class="searchField">
					...
					<input
						type="text"
						v-model="search"
						:placeholder="$t('followbot.search_placeholder')"
					/>
				</div>
						...
						<div :class="userClasses(item)">
							...
							<div class="ctas">
								<Button
									...
									icon="ban"
									v-if="
										item.user.channelInfo[item.channel_id]!.is_banned !== true
									"
								/>
								...
								<Button
									...
									@click="unfollow(item)"
									@mouseover="
										initTooltip($event, item.id, 'followbot.unfollow_tt')
									"
									alert
									icon="unfollow"
									v-if="
										item.user.channelInfo[item.channel_id]!.is_following == true
									"
								/>
								...
								<Button
									...
									:href="'https://twitch.tv/' + item.user.login"
									@mouseover="
										initTooltip($event, item.id, 'followbot.details_tt')
									"
									icon="info"
								/>
				...
				<div class="batchActions">
					<Button @click="banAll()" :loading="batchActionInProgress" alert icon="ban">{{
						$t("followbot.banBt")
					}}</Button>
					<Button
						@click="unfollowAll()"
						:loading="batchActionInProgress"
						alert
						icon="unfollow"
						>{{ $t("followbot.unfollowBt") }}</Button
					>
				</div>
				<div class="ctas">
					<Button
						...
						secondary
						icon="save"
						>{{ $t("followbot.exportBt") }}</Button
					>
					<Button @click="reviewLater()" class="later" secondary icon="countdown">{{
						$t("followbot.laterBt")
					}}</Button>
				</div>
				<div class="ctas">
					<Button @click="clearList()" icon="checkmark">{{
						$t("followbot.finishBt")
					}}</Button>
				</div>
			</div>
		</div>
...
class EmergencyFollowsListModal extends Vue {
	...
	public async ban(follow: TwitchatDataTypes.MessageFollowingData): Promise<void> {
		...
		await TwitchUtils.banUser(
			...
			follow.channel_id,
			undefined,
			"Automatically banned by Twitchat emergency mode on " + Utils.formatDate(new Date()),
		);
	...
	public async banAll(): Promise<void> {
		this.$confirm(
			this.$t("followbot.ban_all_confirm_title"),
			this.$t("followbot.ban_all_confirm_desc"),
		)
			.then(async () => {
				...
				for (let i = 0; i < list.length; i++) {
					...
					this.scrollOffset = Math.max(
						0,
						i * (this.itemSize + this.itemMargin) - bounds.height / 2,
					);
	...
	public unfollowAll(): void {
		this.$confirm(
			this.$t("followbot.unfollow_confirm_title"),
			this.$t("followbot.unfollow_confirm_desc"),
		)
			.then(async () => {
				this.batchActionInProgress = true;
				const list = this.followers;
	...
	public clearList(): void {
		this.$confirm(
			this.$t("followbot.clear_confirm_title"),
			this.$t("followbot.clear_confirm_desc"),
		)
			.then(() => {
	...
	public reviewLater(): void {
		this.$confirm(
			this.$t("followbot.later_confirm_title"),
			this.$t("followbot.later_confirm_desc"),
		)
			.then(() => {
				this.$emit("close");
			})
	...
	public exportCSV(): void {
		...
		for (let i = 0; i < len; i++) {
			...
			csv +=
				"," + (follower.user.channelInfo[follower.channel_id]!.is_banned === true ? 1 : 0);
			if (i < len - 1) csv += "\n";
		}
