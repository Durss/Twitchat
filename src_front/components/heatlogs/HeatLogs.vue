			...
			<div v-for="item in logs" :key="item.id" :class="getRowClasses(item)">
				<div class="head" @click="idToExpandState[item.id] = !idToExpandState[item.id]">
					<div class="infos">
						...
						<div class="status" v-tooltip="'Anonymous click'" v-if="item.anonymous">
							<Icon name="anon" />
						</div>
						<div class="status" v-if="item.user">
							<Icon name="user" /> {{ item.user.login }}
						</div>
					...
					<div class="messages">
						<ul class="messages">
							<li v-for="entry in item.targets">
								<template v-if="entry.spotify"
									><Icon name="spotify" />Spotify overlay</template
								>
								<template v-if="entry.ulule"
									><Icon name="ulule" />Ulule overlay</template
								>
								<template v-if="entry.distortiontID"
									><Icon name="distort" />{{
										getDistortionNameById(entry.distortiontID)
									}}</template
								>
								...
								<HeatScreenPreview
									class="heatScreen"
									v-if="
										entry.customAreaID && getScreenByAreaId(entry.customAreaID)
									"
									renderOnly
									:selectedAreas="[entry.customAreaID]"
									:screen="getScreenByAreaId(entry.customAreaID)"
								/>
								<template v-else-if="entry.customAreaID"
									><Icon name="polygon" /> &lt;UNKNOWN AREA ID
									{{ entry.customAreaID }}&gt;</template
								>

								<template v-if="entry.obsSource"
									><Icon name="obs" />{{ entry.obsSource }}</template
								>
								<div class="coordinates">
									<span
										><strong>X:</strong> {{ (entry.x * 100).toFixed(0) }}%</span
									>
									<span
										><strong>Y:</strong> {{ (entry.y * 100).toFixed(0) }}%</span
									>
								</div>
							</li>
						</ul>
