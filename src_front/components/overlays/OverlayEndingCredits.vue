<template>
	<div
		class="overlayendingcredits"
		v-if="data && slotList && !noEntry"
		:style="styles"
		ref="holder"
	>
		<template v-for="item in slotList">
			<div
				:class="item.holderClasses"
				:style="item.categoryStyles"
				ref="listItem"
				:id="'item_' + item.params.id"
			>
				<h1 data-title :style="item.titleStyles">
					<Icon :name="item.slot.icon" v-if="item.slot.id != 'text'" />{{
						item.params.label
					}}
				</h1>
				...
				<div
					v-if="
						data.premiumWarningSlots && data.premiumWarningSlots[item.slot.id] === true
					"
					data-list
					class="list premium"
					:style="item.entryStyles"
				>
					<Icon name="premium" />{{ data.labels.premium_only }}
				</div>

				<div
					v-else-if="item.entryCount == 0"
					data-list
					class="list empty"
					:style="item.entryStyles"
				>
					{{ data.labels.no_entry }}
				</div>

				<div v-else data-list class="list" :style="item.entryStyles">
					<div
						v-if="item.params.slotType == 'hypechats'"
						v-for="entry in makeUnique(item.params, data.hypeChats || [])
							.concat()
							.sort((a, b) => b.amount - a.amount)
							.splice(0, item.params.maxEntries)"
						class="item"
					>
					...
					<div
						v-if="item.params.slotType == 'ytSuperchat'"
						v-for="entry in getSortedSuperMessages(item.params).splice(
							0,
							item.params.maxEntries,
						)"
						class="item"
					>
						<span class="info">{{ entry.login }}</span>
					...
					<div
						v-if="item.params.slotType == 'ytSuperSticker'"
						v-for="entry in getSortedSuperMessages(item.params).splice(
							0,
							item.params.maxEntries,
						)"
					...
					>
						<div class="stickerList">
							<img
								v-for="sticker in entry.stickerUrlList"
								:src="sticker"
								class="sticker"
								alt="sticker"
					...
					<div
						v-if="item.params.slotType == 'tiktokGifts'"
						v-for="entry in getSortedTikTokGift(item.params).splice(
							0,
							item.params.maxEntries,
						)"
						class="item"
					>
						<div class="stickerList">
							<img
								v-for="sticker in entry.imageUrlList"
								:src="sticker"
								class="sticker"
								alt="sticker"
								referrerpolicy="no-referrer"
						...
						<div class="user">
							...
							<div class="amount" v-if="item.params.showAmounts === true">
								<span class="value"
									><img src="@/assets/icons/tiktok_diamond.svg" />{{
										entry.cumulatedAmount || entry.amount
									}}</span
								>
							</div>
						</div>
					...
					<div
						v-if="item.params.slotType == 'tiktokLikes'"
						v-for="entry in getSortedTikTokLikes(item.params).splice(
							0,
							item.params.maxEntries,
						)"
						class="item"
					>
						<span class="info">{{ entry.login }}</span>
						<span class="count" v-if="item.params.showAmounts === true"
							><Icon name="follow" class="userIcon" />{{ entry.count }}</span
						>
					</div>

					<div
						v-if="item.params.slotType == 'tiktokShares'"
						v-for="entry in getSortedTikTokShares(item.params).splice(
							0,
							item.params.maxEntries,
						)"
						class="item"
					>
						<span class="info">{{ entry.login }}</span>
						<span class="count" v-if="item.params.showAmounts === true"
							><Icon name="share" class="userIcon" />{{ entry.count }}</span
						>
					</div>

					<div
						v-if="item.params.slotType == 'subs'"
						v-for="entry in getSortedSubs(item.params).splice(
							0,
							item.params.maxEntries,
						)"
						class="item"
					>
						<Icon
							class="badge"
							v-if="item.params.showBadges && entry.value.fromActiveSubs !== true"
							:name="entry.type == 'subgift' ? 'gift' : 'sub'"
						/>
						...
						<span
							...
							v-if="
								entry.type != 'subgift' &&
								item.params.showSubMonths === true &&
								entry.value.subDuration
							"
						>
							− {{ getMonthsDurationlabel(entry.value.subDuration) }}</span
						>
						<span
							class="count"
							v-if="entry.type == 'subgift' && item.params.showAmounts === true"
						>
							− {{ entry.value.total }}</span
						>
					</div>

					<div
						v-if="item.params.slotType == 'cheers'"
						v-for="entry in getCheers(item.params)
							.concat()
							.splice(0, item.params.maxEntries)"
						class="item"
					>
						<span class="info">{{ entry.login }}</span>
						<span class="count" v-if="item.params.showAmounts === true"
							><Icon name="bits" class="bitsIcon" />{{ entry.bits }}</span
						>
					</div>

					<div
						v-if="item.params.slotType == 'raids'"
						v-for="entry in makeUnique(item.params, data.raids || [])
							.concat()
							.splice(0, item.params.maxEntries)"
						class="item"
					>
						<span class="info">{{ entry.login }}</span>
						<span class="count" v-if="item.params.showAmounts === true"
							><Icon name="user" class="userIcon" />{{ entry.raiders }}</span
						>
					</div>

					<div
						v-if="item.params.slotType == 'follows'"
						v-for="entry in (data.follows || [])
							.concat()
							.splice(0, item.params.maxEntries)"
						class="item"
					>
					...
					<div
						v-if="item.params.slotType == 'so_in'"
						v-for="entry in (data.shoutouts || [])
							.concat()
							.filter((v) => v.received)
					...
					>
						<span class="info">{{ entry.login }}</span>
						<span class="count" v-if="item.params.showAmounts === true"
							><Icon name="user" class="userIcon" />{{ entry.viewers }}</span
						>
					</div>

					<div
						v-if="item.params.slotType == 'so_out'"
						v-for="entry in (data.shoutouts || [])
							.concat()
							.filter((v) => !v.received)
							.sort((a, b) => b.viewers - a.viewers)
					...
					<div
						v-if="item.params.slotType == 'hypetrains'"
						v-for="entry in (data.hypeTrains || [])
							.concat()
							.splice(0, item.params.maxEntries)"
						class="item"
					>
						<span
							v-html="
								data.labels.train
									.replace(
										/\{LEVEL\}/gi,
										`<strong>${parseInt(entry.level + '')}</strong>`,
									)
									.replace(
										/\{PERCENT\}/gi,
										`<strong>${parseFloat(entry.percent + '')}</strong>`,
									)
							"
						></span>
						<template
							v-if="
								item.params.showTrainConductors &&
								(entry.conductorBits || entry.conductorSubs)
							"
						>
							<div v-if="entry.conductorSubs" class="conductor">
								<Icon class="badge" name="sub" /><span>{{
									entry.conductorSubs.login
								}}</span>
								<!-- <span>x{{entry.conductorSubs.subs}} <Icon class="badge" name="sub" /></span> -->
							</div>
							<div v-if="entry.conductorBits" class="conductor">
								<Icon class="badge" name="bits" /><span>{{
									entry.conductorBits.login
								}}</span>
					...
					<div
						v-if="item.params.slotType == 'bans'"
						v-for="entry in (data.chatters || [])
							.concat()
							.filter((v) => v.bans > 0)
					...
					<div
						v-if="item.params.slotType == 'timeouts'"
						v-for="entry in (data.chatters || [])
							.concat()
							.filter((v) => v.tos > 0)
							.sort((a, b) => b.tosDuration - a.tosDuration)
							.splice(0, item.params.maxEntries)"
						class="item"
					>
						<span class="info">{{ entry.login }}</span>
						<span class="count" v-if="item.params.showAmounts === true"
							><Icon name="timeout" class="timeout" />{{
								formatDuration(entry.tosDuration)
							}}s</span
						>
					</div>

					<div
						v-if="item.params.slotType == 'rewards'"
						v-for="entry in getRewards(item.params)
							.concat()
							.splice(0, item.params.maxEntries)
					...
					<div
						v-if="item.params.slotType == 'chatters'"
						v-for="entry in getSortedChatters(item.params)"
						class="item"
					>
						<Icon
							class="badge"
							v-if="item.params.showBadges && (entry.vip || entry.mod || entry.sub)"
							:name="
								entry.mod ? 'mod' : entry.vip ? 'vip' : entry.sub ? 'sub' : 'min'
							"
						/>
						<span class="info">{{ entry.login }}</span>
						<span class="count" v-if="item.params.showAmounts === true"
							><Icon name="whispers" class="messageIcon" />{{ entry.count }}</span
						>
					</div>

					<div
						v-if="item.params.slotType == 'polls'"
						v-for="entry in (data.polls || []).concat()"
						class="item"
					>
						...
						<div class="pollItems">
							<div v-for="choice in entry.choices" class="pollItem">
								<p>
									<Icon name="checkmark" v-if="choice.win" /><span
										class="dot"
										v-else
										>• </span
									>{{ choice.title }}
								</p>
								<p v-if="item.params.showAmounts">
									{{ Math.round((choice.votes / entry.votes) * 100) }}%
								</p>
							</div>
						</div>
					</div>
					...
					<div
						v-if="item.params.slotType == 'predictions'"
						v-for="entry in (data.predictions || []).concat()"
						class="item"
					>
						<span class="info">{{ entry.title }}</span>
						<div class="pollItems">
							<div v-for="choice in entry.outcomes" class="pollItem">
								...
								<p v-if="item.params.showAmounts">
									{{ choice.points }}<Icon name="channelPoints" />
								</p>
							</div>
						</div>
					</div>
					...
					<div
						v-if="item.params.slotType == 'tips'"
						v-for="entry in getTips(item.params).splice(0, item.params.maxEntries)"
						class="item"
					>
					...
					<div
						v-if="item.params.slotType == 'merch'"
						v-for="entry in getMerch(item.params).splice(0, item.params.maxEntries)"
						class="item"
					>
						<!-- <Icon class="badge" v-if="item.params.showBadges" :name="{kofi:'kofi', tipeee:'tipeee', patreon:'patreon', streamlabs:'streamlabs', streamelements:'streamelements'}[entry.platform]" /> -->
						<span class="info">{{ entry.login }}</span>
					...
					<div
						...
						v-for="entry in getSortedPowerups(item.params)"
						class="item"
						:class="entry.skinID || 'emote'"
					>
						<div class="emoteList">
							<img
								v-if="entry.emoteUrlList"
								v-for="url in entry.emoteUrlList"
								class="emote"
								:src="url"
								alt="emote"
							/>
						...
						<div
							class="amount"
							v-if="(entry.count || 0) > 0 && item.params.uniqueUsers === true"
						>
							x<strong>{{ entry.count }}</strong>
						</div>
					</div>
					...
					>
						<span class="info">{{ entry.login }}</span>
						<span class="count" v-if="item.params.showAmounts === true">
							− {{ getMonthsDurationlabel(entry.months) }}</span
						>
						<span class="count" v-if="item.params.showTotalAmounts === true">
							− {{ entry.lifetimeAmount.toFixed(0) }}{{ item.params.currency }}</span
						>
					</div>
				</div>
			</div>
		...
		<Teleport to="body">
			<img
				class="ending_credits_powerupEmote"
				alt="emote"
				ref="powerupEmote"
...
class OverlayEndingCredits extends AbstractOverlay {
	...
	public getTips(item: TwitchatDataTypes.EndingCreditsSlotParams) {
		const tips = this.makeUnique(
			item,
			(this.data?.tips || []).filter(
				(tip) =>
	...
	public getSortedSubs(params: TwitchatDataTypes.EndingCreditsSlotParams) {
		...
		let res: (
			| { type: "sub"; value: TwitchatDataTypes.StreamSummaryData["subs"][0] }
		...
		if (params.showAllSubs === true) {
			res = res.concat(
				subs
					.filter((v) => v.fromActiveSubs === true && tierToShow[v.tier])
					.map((v) => {
						return { type: "sub", value: v };
					}),
			);
			res = res.concat(
				resubs
		...
		if (params.showAllSubs !== true && params.showAllSubgifters !== true) {
			...
			if (params.showSubs || params.showResubs || params.showSubgifts)
				allowedPlatforms.push("twitch");
			if (params.showSubsYoutube || params.showSubgiftsYoutube)
				allowedPlatforms.push("youtube");
			if (params.showSubsTiktok) allowedPlatforms.push("tiktok");

			if (
				params.showSubs !== false ||
				params.showSubsYoutube !== false ||
				params.showSubsTiktok !== false
			) {
				res = res.concat(
					subs
						.filter(
							(v) =>
								v.fromActiveSubs !== true &&
								tierToShow[v.tier] &&
								allowedPlatforms.includes(v.platform),
						)
						.map((v) => {
							return { type: "sub", value: v };
						}),
	...
	public getPatreonMembers(params: TwitchatDataTypes.EndingCreditsSlotParams) {
		...
		let members: TwitchatDataTypes.StreamSummaryData["patreonMembers"][number][] = (
			this.data?.patreonMembers || []
		)
			.filter((v) => (params.patreonTiers || []).includes(v.tier) || allTiers)
			.concat();
	...
	public getSortedSuperMessages<T>(
		params: TwitchatDataTypes.EndingCreditsSlotParams,
	): ((
		| TwitchatDataTypes.StreamSummaryData["superChats"][number]
		| TwitchatDataTypes.StreamSummaryData["superStickers"][number]
		...
		} else if (params.slotType == "ytSuperSticker") {
			let tmp = this.data!.superStickers.map((v) => {
				return { ...v, stickerUrlList: [v.stickerUrl!] };
			});
	...
	public getSortedTikTokGift<T>(
		params: TwitchatDataTypes.EndingCreditsSlotParams,
	): (TwitchatDataTypes.StreamSummaryData["tiktokGifts"][number] & {
		cumulatedAmount?: number;
		imageUrlList?: string[];
	})[] {
		let res: (TwitchatDataTypes.StreamSummaryData["tiktokGifts"][number] & {
			cumulatedAmount?: number;
			imageUrlList?: string[];
		})[] = [];

		let tmp = this.data!.tiktokGifts.map((v) => {
			return { ...v, cumulatedAmount: 0, imageUrlList: [v.imageUrl!] };
		});
		if (params.uniqueUsers === true) {
	...
	public getSortedTikTokLikes<T>(
		params: TwitchatDataTypes.EndingCreditsSlotParams,
	): TwitchatDataTypes.StreamSummaryData["tiktokLikes"][number][] {
	...
	public getSortedTikTokShares<T>(
		params: TwitchatDataTypes.EndingCreditsSlotParams,
	): TwitchatDataTypes.StreamSummaryData["tiktokShares"][number][] {
	...
	public getSortedPowerups(
		params: TwitchatDataTypes.EndingCreditsSlotParams,
	): (TwitchatDataTypes.StreamSummaryData["powerups"][number] & {
		emoteUrlList?: string[];
		count?: number;
	})[] {
		let anims: (TwitchatDataTypes.StreamSummaryData["powerups"][number] & { count: number })[] =
			[];
		let emotes: (TwitchatDataTypes.StreamSummaryData["powerups"][number] & {
			emoteUrlList: string[];
		})[] = [];
		if (params.showPuSkin !== false)
			anims = this.data!.powerups.filter((v) => v.type == "animation").map((v) => {
				return { ...v, count: 1 };
			});
		if (params.showPuEmote !== false)
			emotes = this.data!.powerups.filter((v) => v.type == "gigantifiedemote").map((v) => {
				return { ...v, emoteUrlList: [v.emoteUrl!] };
			});
		if (params.showPuCeleb !== false)
			emotes = emotes.concat(
				this.data!.powerups.filter((v) => v.type == "celebration").map((v) => {
					return { ...v, emoteUrlList: [v.emoteUrl!] };
				}),
			);
	...
	public getRewards(item: TwitchatDataTypes.EndingCreditsSlotParams) {
		const res: {
			total: number;
			users: { login: string; uid: string; total: number }[];
			reward: TwitchatDataTypes.StreamSummaryData["rewards"][0]["reward"];
		}[] = [];
	...
	public getCheers(item: TwitchatDataTypes.EndingCreditsSlotParams) {
		...
		if (item.showPinnedCheers !== false)
			res = res.concat(cheers.filter((v) => v.pinned === true));
		if (item.showNormalCheers !== false)
			res = res.concat(cheers.filter((v) => v.pinned === false));
	...
	public beforeMount(): void {
		...
		this.summaryDataHandler = (e: TwitchatEvent<"SET_ENDING_CREDITS_DATA">) =>
			this.onSummaryData(e);
		this.paramsDataHandler = (e: TwitchatEvent<"ON_ENDING_CREDITS_CONFIGS">) =>
			this.onParamsData(e);
	...
	public getEntryCountForSlot(item: TwitchatDataTypes.EndingCreditsSlotParams): number {
		...
		switch (item.slotType) {
			case "hypechats":
				count = this.makeUnique(item, this.data?.hypeChats || []).length;
				break;
			case "subs":
				count = this.getSortedSubs(item).length;
				break;
			case "cheers":
				count = this.getCheers(item).length;
				break;
			case "raids":
				count = this.makeUnique(item, this.data?.raids || []).length;
				break;
			case "follows":
				count = (this.data?.follows || []).length;
				break;
			case "hypetrains":
				count = (this.data?.hypeTrains || []).length;
				break;
			case "so_in":
				count = (this.data?.shoutouts || []).filter((v) => v.received === true).length;
				break;
			case "so_out":
				count = (this.data?.shoutouts || []).filter((v) => v.received === false).length;
				break;
			case "rewards":
				count = this.getRewards(item).length;
				break;
			case "bans":
				count = (this.data?.chatters || []).filter((v) => v.bans > 0).length;
				break;
			case "timeouts":
				count = (this.data?.chatters || []).filter((v) => v.tosDuration > 0).length;
				break;
			case "polls":
				count = (this.data?.polls || []).length;
				break;
			case "predictions":
				count = (this.data?.predictions || []).length;
				break;
			case "vips":
				count = (this.data?.chatters || []).filter((v) => v.vip).length;
				break;
			case "mods":
				count = (this.data?.chatters || []).filter((v) => v.mod).length;
				break;
			case "text":
				count = item.text || item.label ? 1 : 0;
				break;
			case "chatters":
				count = this.getSortedChatters(item).length;
				break;
			case "tips":
				count = this.getTips(item).length;
				break;
			case "merch":
			...
			case "shoutouts":
				...
				break;
			case "ytSuperchat":
				count = this.getSortedSuperMessages(item).length;
				break;
			case "ytSuperSticker":
				count = this.getSortedSuperMessages(item).length;
				break;
			case "tiktokGifts":
				count = this.getSortedTikTokGift(item).length;
				break;
			case "tiktokLikes":
				count = (this.data?.tiktokLikes || []).length;
				break;
			case "tiktokShares":
				count = (this.data?.tiktokShares || []).length;
				break;
			case "patreonMembers":
				count = this.getPatreonMembers(item).length;
				break;
		}
	...
	public makeUnique<T>(slot: TwitchatDataTypes.EndingCreditsSlotParams, values: T[]): T[] {
		...
		switch (slot.slotType) {
			case "hypechats": {
				...
				type keyTypeNumber = KeysMatching<
					TwitchatDataTypes.StreamSummaryData["hypeChats"][0],
					number
				>;
			...
			case "ytSuperchat": {
				...
				type keyTypeNumber = KeysMatching<
					TwitchatDataTypes.StreamSummaryData["superChats"][0],
					number
				>;
				let key: keyType = "login";
				let val: keyTypeNumber = "amount";
			...
			case "ytSuperSticker": {
				...
				type keyTypeNumber = KeysMatching<
					TwitchatDataTypes.StreamSummaryData["superStickers"][0],
					number
				>;
				let key: keyType = "login";
				let val: keyTypeNumber = "amount";
			...
			case "subs": {
				...
				type keyTypeNumber = KeysMatching<
					TwitchatDataTypes.StreamSummaryData["subgifts"][0],
					number
				>;
				let key: keyType = "login";
			...
			case "cheers": {
				...
				type keyTypeNumber = KeysMatching<
					TwitchatDataTypes.StreamSummaryData["bits"][0],
					number
				>;
				let key: keyType = "login";
			...
			case "raids": {
				...
				type keyTypeNumber = KeysMatching<
					TwitchatDataTypes.StreamSummaryData["raids"][0],
					number
				>;
				let key: keyType = "login";
			...
			case "tips": {
				...
				type keyTypeNumber = KeysMatching<
					TwitchatDataTypes.StreamSummaryData["tips"][0],
					number
				>;
				let key: keyType = "login";
				let val: keyTypeNumber = "amount";
			...
			case "tiktokLikes": {
				...
				type keyTypeNumber = KeysMatching<
					TwitchatDataTypes.StreamSummaryData["tiktokLikes"][0],
					number
				>;
				let key: keyType = "login";
			...
			case "tiktokShares": {
				...
				type keyTypeNumber = KeysMatching<
					TwitchatDataTypes.StreamSummaryData["tiktokShares"][0],
					number
				>;
				let key: keyType = "login";
				let val: keyTypeNumber = "count";
	...
	private async onControl(e: TwitchatEvent<"SET_ENDING_CREDITS_CONTROL">): Promise<void> {
		...
		if (data.scrollToSectionID != undefined) {
			...
			if (this.interpolating) {
				const bounds = document
					.getElementById("item_" + data.scrollToSectionID)
				...
				let targetYPos = window.innerHeight * 0.2;
				gsap.to(tween, {
					y: bounds.y - targetYPos,
					duration: 1,
					ease: "sine.inOut",
					onUpdate: () => {
		...
		if (data.next === true || data.prev === true) {
			let targetYPos = window.innerHeight * 0.5;
			...
			lists.forEach((item, index) => {
				...
				if (
					(py < closestPos && py > 0) ||
					(closestItemIndex == -1 && index == lists.length - 1)
				) {
		...
		this.$nextTick().then(async () => {
			if (resetScroll && this.data?.params?.startDelay) {
				await new Promise<void>((resolve) => {
					this.startDelayTimeout = window.setTimeout(
						() => resolve(),
						this.data!.params!.startDelay * 1000,
					);
				});
			}
			...
			if (resetScroll) {
				...
				emotes.forEach((imgTag, index) => {
					...
					let px = Math.random() * (vw - 112 * props.scale) + 112 * props.scale * 0.5;
					imgTag.style.left = px + "px";
					imgTag.style.top = py + "px";
					py += (Math.random() * 0.25 + 0.75) * 250;
					if (behind) {
	...
	private renderFrame(ts: number): void {
		...
		lists.forEach((item) => {
			...
			if (this.data?.params?.stickyTitle !== true) {
				...
				list.style.maskImage =
					...
					(titleHeight - listBounds.top + fadeSize) +
					"px, black " +
					(titleHeight * 2 - listBounds.top + fadeSize) +
					"px)";
				item.style.maskImage = "none";
				//@ts-ignore
				list.style["-webkit-mask-image"] =
					"linear-gradient(transparent " +
					(titleHeight - listBounds.top + fadeSize) +
					"px, black " +
					(titleHeight * 2 - listBounds.top + fadeSize) +
			...
			if (fadeSize > 1) {
				document.body.style.maskImage =
					"linear-gradient(transparent 0, black " +
					fadeSize +
					"px, black " +
					(screenHeight - fadeSize * 2) +
					"px, transparent " +
					screenHeight +
					"px)";
				//@ts-ignore
		...
		if (this.posY < -bounds.height) {
			if (!this.creditsComplete) {
				...
				if (this.data?.params?.loop !== true) {
					const speed =
						(((this.data?.params?.speed || 2) + this.speedScale) / 1000) * fps;
					const emotes = (this.$refs.powerupEmote as HTMLImageElement[]) || null;
					gsap.to(emotes, {
						opacity: 0,
						duration: 0.5,
		...
		if (this.data?.params?.timing == "duration" && this.speedScaleInc == 0) {
			const percent = Math.max(
				0,
				(Date.now() - this.scrollStarted_at) / (this.data?.params?.duration * 1000),
			);
			let prevY = this.posY;
	...
	private buildSlots(): void {
		...
		slots.forEach((slotParams) => {
			const slot = TwitchatDataTypes.EndingCreditsSlotDefinitions.find(
				(v) => v.id == slotParams.slotType,
			)!;
		...
		if (powerups.length > 0) {
			...
			for (let i = 0; i < powerups.length; i++) {
				...
				const behind = Math.random() > 0.2;
				const maxSpeed = behind ? 0.5 : 1;
				this.powerUpEmoteProps.push({
					image: powerups[i]!.emoteUrl!,
					behind,
					angle: (Math.random() - Math.random()) * 360,
					speedRatio: Math.random() * maxSpeed + 0.75,
					angleSpeed: (Math.random() - Math.random()) * 0.5,
					scale:
						(Math.random() * 1 + 1) *
						(behind ? Math.random() * 0.25 + 0.25 : Math.random() * 0.2 + 0.8),
				});
			}
		}
	...
	private getCategoryClasses(
		item: TwitchatDataTypes.EndingCreditsSlotParams,
		entryCount: number,
	): string[] {
		...
		if (entryCount > 0) {
			...
			//If requestion 3 or 3 cols but only 1 item is available, switch to 1 col mode
				...
				(item.layout == "3cols" && entryCount == 1) ||
				(item.layout == "2cols" && entryCount == 1)
			)
				res.push("layout_col");
