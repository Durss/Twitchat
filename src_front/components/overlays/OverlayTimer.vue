		...
		<template v-if="countdownValue && configCountdown">
			<div
				...
				:style="{
					...
					backgroundColor: configCountdown.bgEnabled
						? configCountdown.bgColor
						: 'transparent',
				}"
			>
...
class OverlayTimer extends AbstractOverlay {
	...
	private countdownEventHandler!: (
		e: TwitchatEvent<"ON_COUNTDOWN_START" | "ON_COUNTDOWN_COMPLETE">,
	) => void;
	...
	public beforeMount(): void {
		...
		this.timerPresenceHandler = () => {
			PublicAPI.instance.broadcast("ON_TIMER_OVERLAY_PRESENCE");
		};
	...
	public async onTimerEvent(e: TwitchatEvent<"ON_TIMER_START" | "ON_TIMER_STOP">): Promise<void> {
		const data = e.data;
		if (data.id != this.overlayId && !(data.isDefault && this.overlayId == "")) return;
		...
		if (e.type == "ON_TIMER_START") {
			...
			if (!wasVisible || this.timerHidding) {
				...
				if (this.$refs.timer)
					gsap.fromTo(
						this.$refs.timer as HTMLDivElement,
						{ y: "-100%" },
						{ duration: 0.5, y: "0%" },
					);
			} else {
				gsap.to(this.$refs.timer as HTMLDivElement, { duration: 0.5, y: "0%" });
			}
			this.timerHidding = false;
		} else if (this.$refs.timer) {
			this.timerHidding = true;
			gsap.to(this.$refs.timer as HTMLDivElement, {
				duration: 0.5,
				y: "-100%",
				onComplete: () => {
					this.timerData = null;
	...
	public async onCountdownEvent(
		e: TwitchatEvent<"ON_COUNTDOWN_START" | "ON_COUNTDOWN_COMPLETE">,
	): Promise<void> {
		if (e.data.id != this.overlayId && !(e.data.isDefault && this.overlayId == "")) return;
		...
		if (e.type == "ON_COUNTDOWN_START") {
			...
			if (!wasVisible || this.countdownHidding) {
				...
				if (this.$refs.countdown)
					gsap.fromTo(
						this.$refs.countdown as HTMLDivElement,
						{ y: "-100%" },
						{ duration: 0.5, y: "0%" },
					);
			} else {
				gsap.to(this.$refs.countdown as HTMLDivElement, { duration: 0.5, y: "0%" });
			}
			this.countdownHidding = false;
		} else if (this.$refs.countdown) {
			this.countdownHidding = true;
			gsap.to(this.$refs.countdown as HTMLDivElement, {
				duration: 0.5,
				y: "-100%",
				onComplete: () => {
	...
	public computeValues(): void {
		if (this.countdownData && this.countdownData.startAt_ms) {
			...
			if (this.countdownData.paused) {
				elapsed =
					this.countdownData.pausedAt_ms! -
					this.countdownData.startAt_ms +
					this.countdownData.offset_ms;
			} else {
		...
		if (this.timerData && this.timerData.startAt_ms) {
			...
			if (this.timerData.paused) {
				elapsed =
					Math.floor(
						(this.timerData.pausedAt_ms! -
							this.timerData.startAt_ms +
							this.timerData.offset_ms) /
							1000,
					) * 1000;
			} else {
				elapsed =
					Math.floor(
						(Date.now() - this.timerData.startAt_ms + this.timerData.offset_ms) / 1000,
					) * 1000;
			}
