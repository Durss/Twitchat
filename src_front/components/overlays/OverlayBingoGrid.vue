<template>
	<div :class="classes" v-if="ready">
		...
		<div v-else-if="error" class="error card-item alert">
			<Icon name="alert" />Bingo grid not found
		</div>
	</div>
</template>
...
export class OverlayBingoGrid extends AbstractOverlay {
	...
	override requestInfo(): void {
		PublicAPI.instance.broadcast("GET_BINGO_GRID_CONFIGS", { id: this.id });
	}
	...
	public broadcastPresence(): void {
		PublicAPI.instance.broadcast("SET_BINGO_GRID_OVERLAY_PRESENCE", { id: this.id });
	}
	...
	private async onLeaderboard(e: Parameters<typeof this.leaderboardHandler>[0]): Promise<void> {
		...
		gsap.fromTo(leaderboardHolder, { opacity: 0 }, { opacity: 1, duration: 0.75 });

		// let vertical = this.bingo!.cols <= this.bingo!.rows;
		let fontSize =
			Math.min(scrollHolderBounds.height / this.debugScale / 10, this.width / 10) + "px";
		...
		gsap.fromTo(
			listHolder,
			{ y: scrollHolderBounds.height / this.debugScale },
			{ y: -listBounds.height / this.debugScale, duration, ease: "none", repeat: -1 },
		);
	}
	 ...
	 */
	private async onBingoUpdate(e: TwitchatEvent<"ON_BINGO_GRID_CONFIGS">): Promise<void> {
		if (e.data) {
			const data = e.data;
			...
			if (animate) {
				...
				for (const entry of this.bingo.entries) {
					if (entry.check) {
						...
						if (checks) gsap.set(checks[0]!, { opacity: 0.8 });
					}
				}
	...
	private async openCloseGrid(open: boolean): Promise<void> {
		...
		return new Promise((resolve) => {
			...
			gsap.fromTo(
				[this.$refs.cellsHolder, this.$refs.gridHolder],
				{ scale: scaleFrom },
				{
					scale: scaleTo,
	...
	private async pushEvent(data: (typeof this.pendingEvents)[number]): Promise<void> {
		if (data.type == "user") {
			...
			const existingIndex = this.pendingEvents.findIndex(
			...
			if (
				existingIndex > 0 &&
				this.pendingEvents[existingIndex]!.userBingo!.count < data.userBingo!.count
			) {
	...
	private async execNextEvent(): Promise<void> {
		...
		switch (item.type) {
			...
			case "bingo": {
				await this.openCloseGrid(true);
				await this.animateBingos(
					item.bingo!.vertical,
					item.bingo!.horizontal,
					item.bingo!.diagonal,
				);
				break;
			}
	...
	private async animateViewer(data: IUserBingoData): Promise<void> {
		...
		if (this.canPlayWinSound && this.bingo!.winSoundVolume > 0) {
			this.winSoundVolume.volume = Math.min(
				1,
				Math.max(0, this.bingo!.winSoundVolume / 100 || 0.5),
			);
		...
		gsap.to((this.$refs.alertsBg as ComponentPublicInstance).$el, {
			rotate: "360deg",
			duration: 1,
			ease: "none",
		});
		...
		gsap.fromTo(
			...
			{
				top:
					boundsHolder.height / this.debugScale + boundsUser.height / this.debugScale / 2,
			},
			{
				top: -boundsUser.height / this.debugScale,
				duration: 1.5,
		...
		let index = 0;
		dots.forEach((dot) => {
			const height = Math.random() * boundsHolder.height + boundsHolder.height * 0.1;
			dot.style.opacity = Math.random() * 0.3 + 0.1 + "";
			dot.style.left =
				(index / (dots.length - 1) + (Math.random() - Math.random()) * 0.1) * 80 + 10 + "%";
			dot.style.top =
				boundsHolder.height / this.debugScale + Math.random() * height * 5 + "px";
