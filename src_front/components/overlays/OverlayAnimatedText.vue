...
class OverlayAnimatedText extends AbstractOverlay {
	...
	private id: string = "";
	private configHandler!: (e: TwitchatEvent<"ON_ANIMATED_TEXT_CONFIGS">) => void;
	private textHandler!: (e: TwitchatEvent<"SET_ANIMATED_TEXT_CONTENT">) => void;
	private closeHandler!: (e: TwitchatEvent<"ON_ANIMATED_TEXT_CLOSE">) => void;
	...
	public beforeMount(): void {
		...
		PublicAPI.instance.addEventListener("SET_ANIMATED_TEXT_CONTENT", this.textHandler);
		PublicAPI.instance.addEventListener("ON_ANIMATED_TEXT_CLOSE", this.closeHandler);
		PublicAPI.instance.addEventListener("ON_ANIMATED_TEXT_CONFIGS", this.configHandler);
	}
	...
	public beforeUnmount(): void {
		...
		super.beforeUnmount();
		PublicAPI.instance.removeEventListener("SET_ANIMATED_TEXT_CONTENT", this.textHandler);
		PublicAPI.instance.removeEventListener("ON_ANIMATED_TEXT_CLOSE", this.closeHandler);
		PublicAPI.instance.removeEventListener("ON_ANIMATED_TEXT_CONFIGS", this.configHandler);
	...
	public async next(): Promise<void> {
		...
		if (this.currentEntry) {
			...
			PublicAPI.instance.broadcast("ON_ANIMATED_TEXT_HIDE_COMPLETE", {
				queryId: this.currentEntry.queryId,
			});
		...
		if (this.currentEntry.autoHide) {
			...
			this.autoHideTO = window.setTimeout(
				async () => {
					// Close text
					await this.hideText();
					this.currentEntry = null;
	...
	private async showText(): Promise<void> {
		...
		return new Promise<void>((resolve) => {
			this.split = new SplitType(this.$refs["text"] as HTMLElement, {
				split: ["words", "chars"],
				charClass: "char",
				wordClass: "word",
			});
			...
			const chars =
				(Array.from((this.$refs["text"] as HTMLElement).querySelectorAll("*")).filter(
				...
				case "swarm": {
					...
					const angleDistance = (angle1: number, angle2: number): number => {
						const angle = Math.abs(
							((angle1 - angle2 + Math.PI) % (Math.PI * 2)) - Math.PI,
						);
						return angle;
					};
					const renderFrame = () => {
						...
						for (let i = 0; i < chars.length; i++) {
							...
							if (leader.x <= target.x) {
								...
								currY +=
									(leader.y - currY + Math.sin(leaderAngle) * leaderAmp) * 0.2;
						...
						const dir = Math.atan2(
							bounds.height / 2 - leader.y,
							bounds.width - leader.x,
						);
