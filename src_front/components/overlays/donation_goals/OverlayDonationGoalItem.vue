<template>
	<div :class="['overlaydonationgoalitem', skin]" :data-index="index" v-if="data.visible">
		<div
			ref="holder"
			:style="styles"
			:class="{
				holder: true,
				current: data.distanceToCurrentIndex === 0 || state == 'closing',
				...
				questionMarks: data.percent < 0.1,
			}"
		>
			<div class="content" id="content" ref="content">
				<span class="amount goal" id="amount" v-if="data.distanceToCurrentIndex === 0"
					>{{ getFormattedNumber(currentValue)
					}}<span class="currency" v-if="overlayParams.currency">{{
						overlayParams.currency
					}}</span></span
				>
				<div class="label" id="title">
					<span
						class="title"
						v-if="
							data.goalItem.secret !== true ||
							data.goalItem.secret_type !== 'progressive'
						"
						>{{ data.goalItem.title }}</span
					>
					<TextSplitter
						class="title"
						ref="textSplitter"
						:message="data.goalItem.title"
						v-else
					></TextSplitter>
					<div class="mystery">???</div>
				</div>
				<span class="amount" id="amount"
					>{{ getFormattedNumber(data.goalItem.amount)
					}}<span class="currency" v-if="overlayParams.currency">{{
						overlayParams.currency
					}}</span></span
				>
				<div
					class="hideTimer"
					id="timer"
					v-if="data.hidePercent > 0"
					:style="{ width: data.hidePercent + '%' }"
				></div>
			...
			<template v-for="(p, i) in particles">
				...
				>
					<polygon
						points="33.9 64.47 44.8 43.84 67.79 39.85 51.54 23.1 54.84 0 33.9 10.28 12.95 0 16.25 23.1 0 39.85 22.99 43.84 33.9 64.47"
					/>
				</svg>
				...
				>
					<path
						d="M44.46,0c-5.15,0-9.79,2.14-13.11,5.57-3.32-3.43-7.96-5.57-13.11-5.57C8.16,0,0,8.16,0,18.24c0,17.84,31.35,40.98,31.35,40.98,0,0,31.35-22.75,31.35-40.98C62.69,8.16,54.53,0,44.46,0Z"
					/>
				</svg>
			</template>
			<slot></slot>
...
class OverlayDonationGoalItem extends Vue {
	...
	public beforeMount(): void {
		...
		watch(
			() => this.data.percent,
			async () => this.onPercentUpdate(),
		);
		watch(
			() => this.data.goalItem.title,
			async () => this.onPercentUpdate(),
		);
		watch(
			() => this.data.goalItem.secret,
			async () => this.onPercentUpdate(),
		);
		watch(
			() => this.data.goalItem.secret_type,
			async () => this.onPercentUpdate(),
		);
		watch(
			() => this.data.distanceToCurrentIndex,
			() => this.onRender(),
		);
		watch(
			() => this.overlayParams.limitEntryCount,
			() => this.onRender(),
		);
		watch(
			() => this.overlayParams.maxDisplayedEntries,
			() => this.onRender(),
		);
		watch(
			() => this.overlayParams.hideDone,
			() => this.onRender(),
		);
	...
	private initParticles(): void {
		for (let i = 0; i < 20; i++) {
			this.particles.push({
				...
				s: (Math.random() - Math.random()) * 0.5 + 1,
				x: 0,
				y: 0,
			});
		}
	...
	private async open(): Promise<void> {
		...
		return new Promise<void>((resolve) => {
			...
			gsap.from(this.$refs.holder as HTMLElement, {
				...
				duration: 0.25,
				ease: "quad.out",
				clearProps: "padding,borderBottom",
			});
			gsap.from(content, {
				padding: 0,
				scaleY: 0,
	...
	private async close(): Promise<void> {
		...
		if (this.data.percent >= 1 && this.overlayParams.hideDone === true) {
			...
			gsap.to(content, {
				...
				duration: 0.25,
				ease: "quad.in",
				clearProps: "scaleY,height,padding",
				onComplete: () => {
					this.data.visible = false;
					this.state = "closed";
				},
	...
	private async onRender(): Promise<void> {
		const maxDist = this.overlayParams.limitEntryCount
			? this.overlayParams.maxDisplayedEntries || 0
			: 999999;
		...
		let isWithinRange =
			showAll ||
			(Math.abs(this.data.distanceToCurrentIndex) <= maxDist &&
				this.data.distanceToCurrentIndex >= 0);
	...
	public burstParticles(): void {
		...
		for (let i = 0; i < shines.length; i++) {
			...
			gsap.fromTo(
				shine,
				{ opacity: Math.random() * 0.5 + 0.5, left: -(Math.random() * 100 + 100) + "%" },
				{
					left: "100%",
		...
		gsap.fromTo(
			holder,
			{ scaleX: 1.15 },
			{ scaleX: 1, ease: "elastic.out", duration: 1, clearProps: "scaleX" },
		);
		gsap.fromTo(
			holder,
			{ scaleY: 1.25 },
			{ scaleY: 1, ease: "elastic.out", duration: 1, clearProps: "scaleY", delay: 0.07 },
		);
		...
		for (let i = 0; i < this.particles.length; i++) {
			...
			params.a = percent * Math.PI * 2;
			params.x = -bounds.width * 0.25;
			params.y = (Math.random() - Math.random()) * bounds.height + bounds.height * 0.5;

			// const endX = params.x + Math.cos(params.a) * params.v * bounds.width * .75;
			// const endY = params.y + Math.sin(params.a) * params.v * bounds.height;
			const endX = params.x + bounds.width * 2;
			const endY = (Math.random() - Math.random()) * bounds.height + bounds.height * 0.5;
			const endR = params.r + (Math.random() - Math.random()) * Math.PI;
	...
	private async onPercentUpdate(): Promise<void> {
		gsap.to(this, {
			localPercent: this.data.percent,
			duration: 0.5,
			ease: "sine.inOut",
			onComplete: () => {
		...
		if (
			this.data.visible &&
			this.data.goalItem.secret &&
			this.data.goalItem.secret_type == "progressive"
		) {
			await this.$nextTick();

			const prng = Utils.seededRandom(this.prngSeed);
			let letters = [
				...(this.$refs.textSplitter as ComponentPublicInstance).$el.querySelectorAll(
					".letter",
				),
			];
...
.overlaydonationgoalitem {
	.holder {
		...
		background-size: 200%;
		border-radius: 0.5em;
		border-bottom: 0.3em solid v-bind(color);
		width: 100vw;
		max-width: 33em;
		position: relative;
		overflow: hidden;
