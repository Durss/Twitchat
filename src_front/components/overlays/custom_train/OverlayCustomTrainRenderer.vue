		...
		<div class="progress">
			<div class="holder back">
				...
				<div class="infoHolder">
					<div
						class="record"
						v-if="showIncomingRecord && !showLevelUp && !isRecord"
					></div>
					<ContentEditable
						tag="div"
			...
			<div class="approaching" v-if="showApproaching !== false">
				<div class="content">
					...
					<div class="events wrap" v-else>
						<div class="wrapper">
							<div
								v-for="i in Math.ceil(eventCount / 2)"
								:key="i"
								:class="{ done: i <= eventDone }"
							>
								<Icon name="checkmark" v-if="i <= eventDone" />
							</div>
						</div>
						<div class="wrapper">
							<div
								v-for="i in Math.floor(eventCount / 2)"
								:key="i"
								:class="{ done: i + Math.ceil(eventCount / 2) <= eventDone }"
							>
								<Icon
									name="checkmark"
									v-if="i + Math.ceil(eventCount / 2) <= eventDone"
								/>
							</div>
						</div>
						<div class="time">{{ timeLeft }}</div>
...
class OverlayCustomTrainRenderer extends Vue {
	...
	public get cssFontSize() {
		return this.size + "px";
	}

	public get cssColorTextGeneric() {
		return this.isRecord ? this.recordColorText : this.colorText;
	}

	public get cssColorBgGeneric() {
		return this.isRecord ? this.recordColorBg : this.colorBg;
	}

	public get cssColorRecordFade() {
		return this.recordColorText + "40";
	}

	public get cssColorTextFade() {
		return this.cssColorTextGeneric + "80";
	}

	public get cssColorBgFade() {
		return this.cssColorBgGeneric + "80";
	}

	public get cssFillPercent(): string {
		return this.easedPercent * 100 + "%";
	}

	public get cssRecordPercent(): string {
		return this.recordPercent * 100 + "%";
	}
	...
	public mounted() {
		...
		let [x, y, w, h] = svgElement
			.getAttribute("viewBox")
			?.split(" ")
			.map((v) => parseFloat(v)) || [0, 0, 0, 0];
		if (w) w *= 1.25;
		// h *= 1.5;
	...
	public async levelUpAnimation(): Promise<void> {
		...
		gsap.to(this, { easedPercent: 1, duration: 0.5 });
		...
		await this.animateLabelSlowmo(labelHolder);
		if (!this.showLevelUp_local) return; //This flag can be set back to false when reaching new record
		this.percent_local = this.easedPercent = this.percent;
	...
	public async recordAnimation(): Promise<void> {
		...
		this.showLevelUp_local = false;
		gsap.to(this, { easedPercent: this.percent, duration: 0.5 });
	...
	public async successAnimation(): Promise<void> {
		...
		this.localTitleSuccess = this.titleSuccess;
		await this.animateLabelSlowmo(labelHolder);
		this.localTitleSuccess = this.titleSuccessSummary
			.replace("{LEVEL}", this.localLevelIndex.toString())
			.replace("{PERCENT}", Math.floor(this.percent_local * 100).toString());
	...
	private async animateLabelSlowmo(labelHolder: HTMLElement): Promise<void> {
		gsap.fromTo(
			labelHolder,
			{ x: "20%" },
			{ x: "-20%", duration: 2, ease: "slow(0.5,0.8,false)" },
		);
		gsap.fromTo(labelHolder, { opacity: 0 }, { opacity: 1, duration: 0.25, ease: "sine.out" });
		gsap.to(labelHolder, { opacity: 0, duration: 0.25, delay: 1.75, ease: "sine.in" });
		await Utils.promisedTimeout(2000);
	}

	private async animateLabelPaused(
		labelHolder: HTMLElement,
		bounce: boolean = true,
		...
		gsap.fromTo(
			labelHolder,
			{ x: "20%", opacity: 0 },
			{ x: "0", opacity: 1, duration: 0.5, ease: "back.out", delay: 0.5 },
		);
		if (bounce) {
			const loops = (pauseDuration / 1000 - 0.5) / 0.25;
			const loopsRot = (pauseDuration / 1000 - 0.5) / 1;
			gsap.to(labelHolder, {
				scaleY: 1.2,
				duration: 0.25,
				ease: "sine.inOut",
...
.overlaycustomtrainrenderer {
	...
	.emoteWall {
		...
		height: 100%;
		pointer-events: none;
		animation: scrollUp 0.35s linear infinite;
