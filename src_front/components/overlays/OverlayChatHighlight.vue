		...
		<div :class="classes" ref="holder" id="highlight_holder" v-if="params && !clipData">
			...
			<div class="infos" id="highlight_infos">
				<div class="login" id="highlight_login" v-if="user">
					...
					<Icon
						name="tiktok"
						v-if="user && user.platform == 'tiktok'"
						style="color: #000000"
					/>
					<Icon
						name="kick"
						v-if="user && user.platform == 'kick'"
						style="color: #50f918"
					/>
					<Icon
						name="instagram"
						v-if="user && user.platform == 'instagram'"
						style="color: #d34a65"
					/>
...
class OverlayChatHighlight extends Vue {
	...
	private overlayPresenceHandler!: (
		e: TwitchatEvent<"GET_CHAT_HIGHLIGHT_OVERLAY_PRESENCE">,
	) => void;
	...
	public get classes(): string[] {
		return ["holder", "position-" + this.params?.position, this.skin];
	}
	...
	public async mounted(): Promise<void> {
		...
		this.overlayPresenceHandler = () => {
			PublicAPI.instance.broadcast("SET_CHAT_HIGHLIGHT_OVERLAY_PRESENCE");
		};
	...
	private async onShowMessage(
		e: TwitchatEvent<"SET_CHAT_HIGHLIGHT_OVERLAY_MESSAGE">,
	): Promise<void> {
	...
	private async hideCurrent(): Promise<void> {
		...
		if (this.params.position.indexOf("r") > -1) {
			gsap.to(holder, { x: "105%", duration: 0.5, ease: "sine.in" });
		} else if (this.params.position.indexOf("l") > -1) {
			gsap.to(holder, { x: "-105%", duration: 0.5, ease: "sine.in" });
		} else if (this.params.position == "t") {
			gsap.to(holder, { y: "-105%", duration: 0.5, ease: "sine.in" });
		} else if (this.params.position == "b") {
			gsap.to(holder, { y: "105%", duration: 0.5, ease: "sine.in" });
		} else if (this.params.position == "m") {
			gsap.to(holder, { scale: 0, duration: 0.5, ease: "back.in" });
		}
	...
	private showCurrent(): void {
		...
		if (this.params.position.indexOf("r") > -1) {
			gsap.from(holder, { x: "105%", duration: 0.5, ease: "sine.out" });
		} else if (this.params.position.indexOf("l") > -1) {
			gsap.from(holder, { x: "-105%", duration: 0.5, ease: "sine.out" });
		} else if (this.params.position == "t") {
			gsap.from(holder, { y: "-105%", duration: 0.5, ease: "sine.out" });
		} else if (this.params.position == "b") {
			gsap.from(holder, { y: "105%", duration: 0.5, ease: "sine.out" });
		} else if (this.params.position == "m") {
			gsap.from(holder, { scale: 0, duration: 0.5, ease: "back.out" });
		}
...
.overlaychathighlight {
	...
	.holder.etc {
		...
		border-radius: 1em;
		filter: drop-shadow(0.35em 0.35em 0px #6bf9ff);
		box-shadow: none;
		z-index: 1;
		padding: 2em;
		@margin: 5vh;
