...
class OverlayDonationGoals extends AbstractOverlay {
	...
	public get color(): string {
		return this.state!.params.color || "#000000";
	}
	...
	public beforeMount(): void {
		...
		if (this.id) {
			...
			PublicAPI.instance.addEventListener(
				...
				this.overlayParamsHandler,
			);
		}

		//@ts-ignore
		window.raise = (amount: number) => {
			this.onOverlayParams(
				new TwitchatEvent("ON_DONATION_GOALS_OVERLAY_CONFIGS", {
					...this.state!,
		...
		window.donationEvent = (amount: string, username: string) => {
			this.onDonation(
				new TwitchatEvent("ON_DONATION_EVENT", { amount, username, overlayId: this.id }),
			);
		};
	}
	 ...
	 */
	public async onOverlayParams(
		e: Parameters<typeof this.overlayParamsHandler>[0],
	): Promise<void> {
		const state = e.data;
		if (!state) return;
	...
	public onActivity(): void {
		...
		this.scrollTimeout = window.setTimeout(() => {
			...
			const maxPos = document.body.clientHeight * 0.5;
	...
	private buildLocalParams(): void {
		...
		for (const goal of this.state!.params.goalList) {
			const target = Math.min(
				1,
				Math.max(0, (this.localRaised - offset) / (goal.amount - offset)),
			);
	...
	private async showNextAlert(): Promise<void> {
		...
		gsap.fromTo(
			...
			{ y: "100%" },
			{
				...
				ease: "sine.out",
				onComplete: () => {
					let showDuration = Math.max(0.15, 10 - Math.pow(this.poolAlerts.length, 0.75));
					//Holder might have changed between show and hide
					let holder = this.$refs.notification as HTMLElement | ComponentPublicInstance[];
					if (Array.isArray(holder) && holder.length > 0)
						holder = holder[0]!.$el as HTMLElement;
					if (!holder) return;
					//Hide notification
					gsap.fromTo(
						holder,
