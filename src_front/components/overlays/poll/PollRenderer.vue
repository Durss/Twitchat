<template>
	<div
		class="pollrenderer"
		v-show="showContent"
		:class="{ ['position-' + placement]: true, win: showWinner }"
		id="holder"
	>
		...
		<div id="list" class="list" v-if="mode == 'list'">
			<div
				...
				v-for="(c, index) in entries"
				ref="bar"
			>
				<div class="index" v-if="showIndex !== false">
					<span>{{ index + 1 }}</span>
				</div>
				<div class="subContent">
					...
					<div class="bar" id="list_choice_bar" :style="getAnswerStyles(c)">
						<div class="details" id="list_choice_bar_details">
							<span
								id="list_choice_bar_details_percent"
								class="percent"
								v-if="showPercent"
								>{{ getPercent(c).toFixed(0) }}%</span
							>
							<span id="list_choice_bar_details_votes" class="votes" v-if="showVoters"
								><Icon name="user" class="icon" />{{ c.voters ?? c.votes }}</span
							>
							<span
								id="list_choice_bar_details_points"
								class="points"
								v-if="showVotes"
								><Icon name="channelPoints" class="icon" />{{ c.votes }}</span
							>
						</div>
					</div>
				</div>
		...
		<div id="line" class="battle" v-else ref="holder">
			<div id="line_labelList" class="labels" v-if="showLabels">
				<h2
					id="line_labelList_label"
					class="outcomeTitle"
					:class="getWinClasses(c)"
					v-for="(c, index) in entries"
					:style="{ flexBasis: getPercent(c) + '%' }"
				>
					<div class="index" v-if="showIndex !== false">
						<span>{{ index + 1 }}</span>
					</div>
			...
			<div class="chunks" id="line_bar" ref="bar">
				...
				>
					<div class="details" id="line_bar_item_details">
						<span id="line_bar_item_details_percent" class="percent" v-if="showPercent"
							>{{ getPercent(c).toFixed(0) }}%</span
						>
						<span id="line_bar_item_details_votes" class="votes" v-if="showVoters"
							><Icon name="user" class="icon" />{{ c.voters ?? c.votes }}</span
						>
						<span id="line_bar_item_details_points" class="points" v-if="showVotes"
							><Icon name="channelPoints" class="icon" />{{ c.votes }}</span
						>
					</div>
				</div>
...
class PollRenderer extends Vue {
	...
	public getWinClasses(
		c: TwitchatDataTypes.MessagePollDataChoice | TwitchatDataTypes.MessagePredictionDataOutcome,
	): string[] {
		if (this.winningId) {
	...
	public async doOpen(): Promise<void> {
		...
		return new Promise((resolve) => {
			...
			gsap.fromTo(
				this.$el,
				{ scale: 0 },
				{ scale: 1, duration: 0.5, ease: "back.out", clearProps: true },
			);
			...
			if (progressBar && !this.showWinner) {
				...
				gsap.fromTo(
					...
					{ duration: duration / 1000, ease: "none", width: "0%" },
				);
			}
			...
			if (labels) {
				...
				gsap.fromTo(
					labels,
					{ width: 0 },
					{ ease: "back.out", duration: 0.5, width, clearProps: true },
				);
			}
	...
	public async doClose(): Promise<void> {
		return new Promise((resolve) => {
			gsap.to(this.$el, { scale: 0, duration: 0.5, ease: "back.in", onComplete: resolve });
		});
	}
}
...
.pollrenderer {
	...
	.index {
		...
		span {
			filter: drop-shadow(-2px 0 0 var(--color1)) drop-shadow(2px 0 0 var(--color1))
				drop-shadow(0 -2px 0 var(--color1)) drop-shadow(0 2px 0 var(--color1));
		}
	}
