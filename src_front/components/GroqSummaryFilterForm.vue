<template>
	<div
		:class="{
			groqsummaryfilterform: true,
			'card-item': standalone === false,
	...
	>
		...
		<ParamItem
			v-if="modeLocal == 'duration'"
			:paramData="param_duration"
			v-model="duration"
			noBackground
		/>
		<ParamItem
			v-if="modeLocal == 'count'"
			:paramData="param_count"
			v-model="count"
			noBackground
		/>
		...
		<ToggleBlock :open="false" :title="$t('global.advanced_params')" small>
			<div class="advancedParams">
				<ParamItem
					:paramData="param_preprompt_toggle"
					v-model="param_preprompt_toggle.value"
					noBackground
				>
					<ParamItem
						:childLevel="1"
						:paramData="param_preprompt_value"
						v-model="preprompt"
						noBackground
					/>
				</ParamItem>
				...
				<ParamItem
					:paramData="param_ignore_short"
					v-model="param_ignore_short.value"
					noBackground
				/>

				<div class="channelList" v-if="channelList.size > 1">
					<button
						:class="{ channel: true, selected: selectedChannels.includes(chan?.name) }"
		...
		<TTButton
			...
			:loading="loading"
			:disabled="filteredMessageList.length === 0"
			>{{
				$t(
					"groq.summarize_duration_bt",
					{ COUNT: filteredMessageList.length },
...
class GroqSummaryFilterForm extends Vue {
	...
	public param_filter: TwitchatDataTypes.ParameterData<"duration" | "count"> = {
		type: "list",
		value: "duration",
		labelKey: "groq.param_filter",
		icon: "filters",
	};
	public param_duration: TwitchatDataTypes.ParameterData<number> = {
		...
		min: 1,
		max: 1000000000,
		labelKey: "groq.param_duration",
		icon: "timer",
	};
	public param_count: TwitchatDataTypes.ParameterData<number> = {
		type: "number",
		value: 10,
		min: 1,
		max: 100000,
		labelKey: "groq.param_count",
		icon: "number",
	};
	public param_ignore_short: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: true,
		labelKey: "groq.param_ignore_short",
		icon: "emote",
	};
	public param_preprompt_toggle: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "groq.param_preprompt",
		icon: "edit",
	};
	public param_preprompt_value: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		value: "",
		longText: true,
		placeholderKey: "groq.param_preprompt_placeholder",
	};
	...
	public get channelList() {
		...
		const chans: Map<
			string,
			NonNullable<TwitchatDataTypes.AbstractTwitchatMessage["channelSource"]>
		> = new Map();
		for (let i = this.messageList.length - 1; i > -1; i--) {
			const m = this.messageList[i]!;
			if (m.type != TwitchatDataTypes.TwitchatMessageType.MESSAGE) continue;
	...
	public toggleChannel(
		channel: typeof this.channelList extends Map<string, infer V> ? V : never,
	): void {
