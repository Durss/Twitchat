	...
	public onClick(e: MouseEvent): void {
		...
		if (target.id) {
			...
			if (id != null) {
				...
				if (
					(this.knobMode !== false || this.childMode !== false) &&
					id?.indexOf("EffectSelect") === 0
				) {
					this.selectedButtons = this.selectedButtons.filter(
						(v) => v.indexOf("EffectSelect") || v == id,
					);
				}

				//Unselect any previously selected bank  if in knobMode
				if (
					(this.samplerMode !== false || this.childMode !== false) &&
					id?.indexOf("SamplerSelect") === 0
				) {
					this.selectedButtons = this.selectedButtons.filter(
						(v) => v.indexOf("SamplerSelect") || v == id,
					);
				}
	...
	private setButtonStates(): void {
		...
		if (this.knobMode !== false) {
			...
			if (
				...
				/EffectSelect[0-9]/.test(this.selectedButtons.join(","))
			) {
				this.allowedButtons.push("echo", "gender", "pitch", "reverb");
			}
		} else if (!/EffectSelect[0-9]/.test(this.selectedButtons.join(","))) {
			this.allowedButtons = this.allowedButtons.filter(
				(v) => v != "echo" && v != "gender" && v != "pitch" && v != "reverb",
			);
		}
		...
		if (this.samplerMode !== false) {
			...
			if (
				this.childMode === false ||
				/SamplerSelect(A|B|C)/.test(this.selectedButtons.join(","))
			) {
