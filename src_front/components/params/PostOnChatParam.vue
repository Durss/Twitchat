...
class PostOnChatParam extends Vue {
	...
	public enabledParam: TwitchatDataTypes.ParameterData<boolean, unknown, string> = {
		value: false,
		type: "boolean",
	};
	public textParam: TwitchatDataTypes.ParameterData<string> = {
		value: "",
		type: "string",
		longText: true,
		maxLength: 500,
	...
	public async beforeMount(): Promise<void> {
		if (this.botMessageKey) {
			...
			if (
				this.botMessageKey == "twitchatAd" &&
				this.textParam.value.indexOf("/announce") == 0
			) {
				// Remove /announcexxx from message. It's now given as prefix prop.
				this.textParam.value = this.textParam.value.replace(
					/\/announce[a-z]*\s(.*)/i,
					"$1",
				);
			}
	...
	public async saveParams(saveToStore = true): Promise<void> {
		...
		if (this.enabledParam.value && this.isFirstRender) {
			...
			this.placeholderTarget = (
				this.$refs.paramItem as ParamItemClass
