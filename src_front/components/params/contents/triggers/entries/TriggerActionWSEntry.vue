	...
	<div class="triggeractionwsentry triggerActionForm">
		<div class="card-item info warn" v-if="!websocketConnected">
			...
			<i18n-t
				...
				class="label"
				tag="p"
				keypath="triggers.actions.http_ws.need_to_connect"
			>
				<template #LINK>
					<a @click="openConnectForm()">{{
						$t("triggers.actions.http_ws.need_to_connect_link")
					}}</a>
				</template>
			</i18n-t>
		</div>
		...
		<div class="card-item tags">
			<div class="head">
				<p class="title" v-if="parameters.length > 0">
					{{ $t("triggers.actions.http_ws.select_param_ws") }}
				</p>
				<ParamItem
					class="toggleAll"
					noBackground
...
class TriggerActionWSEntry extends AbstractTriggerActionEntry {
	...
	public param_payload: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		value: "",
		longText: true,
		maxLength: 10000,
		labelKey: "triggers.actions.http_ws.payload_description",
	};
