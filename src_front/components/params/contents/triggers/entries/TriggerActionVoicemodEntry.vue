<template>
	<div class="triggeractionvoicemodentry triggerActionForm" v-if="!vmConnected">
		<div class="item info warn">
			...
			<i18n-t scope="global" class="label" tag="p" keypath="triggers.actions.voicemod.header">
				<template #LINK>
					<a @click="$store.params.openParamsPage(contentConnexions, subcontentVM)">{{
						$t("triggers.actions.voicemod.header_link")
					}}</a>
				</template>
			</i18n-t>
		</div>
	...
	<div class="triggeractionvoicemodentry triggerActionForm" v-else>
		...
		<ParamItem
			:paramData="param_voiceList"
			v-model="voiceItemID"
			@change="onSelectVoice()"
			v-if="param_action.value == 'voice'"
		/>
		<ParamItem
			:paramData="param_soundsList"
			v-model="soundItemID"
			@change="onSelectSound()"
			v-if="param_action.value == 'sound'"
		/>
	</div>
</template>
...
import {
	TriggerActionVoicemodDataActionList,
	type ITriggerPlaceholder,
	type TriggerActionVoicemodData,
	type TriggerData,
...
class TriggerActionVoicemodEntry extends AbstractTriggerActionEntry {
	...
	public param_action: TwitchatDataTypes.ParameterData<
		TriggerActionVoicemodDataAction,
		TriggerActionVoicemodDataAction
	> = {
		type: "list",
		listValues: [],
		value: "voice",
		icon: "",
		labelKey: "triggers.actions.voicemod.param_action",
	};
	public param_voiceList: TwitchatDataTypes.ParameterData<string, string> = {
		type: "list",
		listValues: [],
		value: "",
		icon: "voice",
		labelKey: "triggers.actions.voicemod.param_voice",
	};
	public param_soundsList: TwitchatDataTypes.ParameterData<string, string> = {
		type: "list",
		listValues: [],
		value: "",
		icon: "unmute",
		labelKey: "triggers.actions.voicemod.param_sound",
	};

	public voiceItemID: string = "";
	public soundItemID: string = "";
	private voiceListItemIDToData: {
		[key: string]: { type: "id" | "placeholder"; value: string };
	} = {};
	private soundListItemIDToData: {
		[key: string]: { type: "id" | "placeholder"; value: string };
	} = {};

	public get vmConnected(): boolean {
		return VoicemodWebSocket.instance.connected.value;
	}
	public get subcontentVM(): TwitchatDataTypes.ParamDeepSectionsStringType {
		return TwitchatDataTypes.ParamDeepSections.VOICEMOD;
	}
	...
	public beforeMount(): void {
		this.param_action.listValues = TriggerActionVoicemodDataActionList.map(
			(v): TwitchatDataTypes.ParameterDataListValue<TriggerActionVoicemodDataAction> => {
				return {
					value: v,
	...
	public onPlaceholderUpdate(placeholders: ITriggerPlaceholder<any>[]): void {
		...
		if (ph.length > 0) {
			...
			for (const p of ph) {
				...
				voiceList.push({
					label: " ⤷ " + this.$t(p.descKey, { NAME: "{" + p.tag.toUpperCase() + "}" }),
					value: id,
					disabled: true,
				});
				soundList.push({
