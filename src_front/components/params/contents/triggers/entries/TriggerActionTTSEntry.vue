	...
	<div class="triggeractionttsentry triggerActionForm" v-if="!$store.tts.params.enabled">
		<div class="info warn">
			...
			<i18n-t scope="global" class="label" tag="p" keypath="triggers.actions.tts.header">
				<template #LINK>
					<a @click="$store.params.openParamsPage(contentTTS)">{{
...
class TriggerActionTTSEntry extends AbstractTriggerActionEntry {
	...
	public param_customVoice: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		icon: "voice",
	...
	public get contentTTS(): TwitchatDataTypes.ParameterPagesStringType {
