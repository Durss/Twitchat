	...
	<div class="triggeractionstreamerbotentry triggerActionForm">
		<div class="card-item info warn" v-if="!$store.streamerbot.connected">
			...
			<i18n-t
				...
				class="label"
				tag="p"
				keypath="triggers.actions.streamerbot.need_to_connect"
			>
				<template #LINK>
					<a @click="openConnectForm()">{{
						$t("triggers.actions.streamerbot.need_to_connect_link")
					}}</a>
				</template>
			</i18n-t>
		</div>
		...
		<div class="headerList">
			<div
				class="header head"
				v-if="action.streamerbotData!.params && action.streamerbotData!.params.length > 0"
			>
			...
			<TTButton
				class="center"
				icon="add"
				v-if="(action.streamerbotData!.params?.length || 0) < 40"
				@click="addParam()"
				>{{ $t("triggers.actions.streamerbot.add_arg_bt") }}</TTButton
			>
		</div>
	</div>
</template>
...
class TriggerActionStreamerbotEntry extends AbstractTriggerActionEntry {
	...
	public param_action: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "",
		labelKey: "triggers.actions.streamerbot.param_action",
	};
	...
	public beforeMount(): void {
		...
		watch(
			() => this.$store.streamerbot.actionList,
			() => this.buildActionList(),
		);
	}
