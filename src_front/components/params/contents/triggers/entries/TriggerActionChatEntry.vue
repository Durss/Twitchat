...
class TriggerActionChatEntry extends AbstractTriggerActionEntry {
	...
	public param_reply: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		icon: "reply",
	...
	public get canReply(): boolean {
		...
		).includes(this.triggerData.type);
	}

	public insertCommand(cmd: TwitchatDataTypes.CommandData): void {
		this.param_message.value =
			cmd.cmd.replace(/(?:\{([^}]+)\}?)/gi, "$1") + "" + this.param_message.value;
	}
