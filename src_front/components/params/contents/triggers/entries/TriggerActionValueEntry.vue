	...
	<div class="triggeractionvalueentry triggerActionForm">
		<div class="card-item list">
			...
			<vue-select
				...
				:selectable="
					(v: TwitchatDataTypes.ParameterDataListValue<string>) =>
						action.values.indexOf(v.value) == -1
				"
				appendToBody
				multiple
			></vue-select>
		...
		<div
			class="card-item valueList"
			v-if="selectedPerUserValue.length > 0 && userSourceOptions.length > 1"
		>
			<div class="head">
				<Icon name="user" class="icon" />
				<span>{{
					$t("triggers.actions.value.user_source_title", selectedPerUserValue.length)
				}}</span>
			</div>
			...
			<div class="card-item dark" v-for="item in selectedPerUserValue" :key="item.id">
				...
				<select :id="'select_' + item.id" v-model="action.valueUserSources[item.id]">
					<option v-for="opt in userSourceOptions" :value="opt.key">
						{{ $t(opt.labelKey, { PLACEHOLDER: opt.key.toUpperCase() }) }}
					</option>
				</select>
			</div>
		</div>
		...
		>
			<div class="head">
				<Icon name="user" class="icon" />
				<span>{{
					$t("triggers.actions.value.user_action_title", selectedPerUserValue.length)
				}}</span>
			</div>
			...
			<div class="card-item dark" v-for="item in selectedPerUserValue" :key="item.id">
				...
				<select :id="'select_' + item.id" v-model="action.userAction![item.id]">
					<option v-for="opt in param_userAction.listValues" :value="opt.value">
						{{ $t(opt.labelKey!) }}
					</option>
				</select>
			</div>
		...
		<ParamItem
			...
			v-model="action.newValue"
			key="value"
			forceChildDisplay
		>
			<ParamItem
				:childLevel="1"
				:paramData="param_maths"
				v-model="action.interpretMaths"
				key="maths"
				noBackground
			/>
		</ParamItem>
...
import {
	VALUE_EDIT_SOURCE_CHATTERS,
	VALUE_EDIT_SOURCE_EVERYONE,
	VALUE_EDIT_SOURCE_SENDER,
...
class TriggerActionValueEntry extends AbstractTriggerActionEntry {
	...
	public param_values: TwitchatDataTypes.ParameterData<string[], string> = {
		type: "list",
		labelKey: "triggers.actions.value.select_label",
		value: [],
		listValues: [],
	};
	public param_value: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		labelKey: "triggers.actions.value.value_label",
		value: "",
	};
	public param_maths: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		labelKey: "triggers.actions.value.interpret_arithmetic",
		value: true,
	};
	public param_userAction: TwitchatDataTypes.ParameterData<
		NonNullable<TriggerActionValueData["userAction"]>[string],
		NonNullable<TriggerActionValueData["userAction"]>[string]
	> = { type: "list", labelKey: "triggers.actions.value.param_action", value: "update" };
	...
	public get showAction(): boolean {
		...
		for (const cid of this.action.values) {
			if (this.action.userAction && this.action.userAction[cid] === "delete")
				deleteBatchActions++;
		}
	...
	public get userSourceOptions(): { key: string; labelKey: string }[] {
		const res: { key: string; labelKey: string }[] = [
			...
			{
				labelKey: "triggers.actions.value.user_source_sender",
				key: VALUE_EDIT_SOURCE_SENDER,
			},
			{
				labelKey: "triggers.actions.value.user_source_everyone",
				key: VALUE_EDIT_SOURCE_EVERYONE,
			},
			{
				labelKey: "triggers.actions.value.user_source_chatters",
				key: VALUE_EDIT_SOURCE_CHATTERS,
			},
		];
		...
		if (this.triggerData.chatCommandParams) {
			this.triggerData.chatCommandParams.forEach((v) => {
				res.push({
					labelKey: "triggers.actions.value.user_source_placeholder",
					key: v.tag,
				});
			});
		}
		...
		this.userPLaceholders
			.filter((v) => v.tag.indexOf(VALUE_PLACEHOLDER_PREFIX) == -1)
			.forEach((v) => {
				res.push({
					labelKey: "triggers.actions.value.user_source_placeholder",
					key: v.tag,
	...
	public get selectedPerUserValue(): TwitchatDataTypes.ValueData[] {
		return this.$store.values.valueList.filter(
			(v) => v.perUser === true && this.action.values.findIndex((v2) => v2 === v.id) > -1,
		);
	}
