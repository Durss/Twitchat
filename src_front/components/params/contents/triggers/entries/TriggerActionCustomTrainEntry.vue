...
<script lang="ts">
import {
	TriggerActionCustomTrainData_ActionList,
...
class TriggerActionCustomTrainEntry extends AbstractTriggerActionEntry {
	...
	public param_trainId: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "",
		icon: "train",
		labelKey: "triggers.actions.custom_train.param_trainId",
	};
	public param_action: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "",
		icon: "",
		labelKey: "triggers.actions.custom_train.param_action",
	};
	public param_value: TwitchatDataTypes.ParameterData<string> = {
		...
		icon: "add",
		maxLength: 1000,
		labelKey: "triggers.actions.custom_train.param_value",
	};
	...
	public beforeMount(): void {
		...
		this.param_trainId.listValues = this.$store.customTrain.customTrainList.map(
			(train, index) => {
				return {
					value: train.id,
					label:
						train.title ||
						this.$t("overlay.customTrain.default_title") + " " + (index + 1),
				};
			},
		);
