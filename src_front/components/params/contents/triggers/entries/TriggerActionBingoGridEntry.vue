<template>
	<div class="triggeractionbingogridentry triggerActionForm">
		<template v-if="param_grid.listValues!.length > 0">
			<ParamItem
				:paramData="param_grid"
				@change="onSelectGrid"
				v-model="action.bingoGrid.grid"
			/>
			<ParamItem :paramData="param_action" v-model="action.bingoGrid.action" />
			<div class="card-item subEntries" v-if="isCellAction">
				<SwitchButton
					v-model="action.bingoGrid.cellActionMode"
					:values="['id', 'coords']"
					:labels="[
						$t('triggers.actions.bingoGrid.param_cell_id'),
						$t('triggers.actions.bingoGrid.param_cell_coordinates'),
					]"
				></SwitchButton>
				...
				<template v-if="action.bingoGrid.cellActionMode == 'id'">
					<ParamItem
						:paramData="param_cellId"
						v-model="action.bingoGrid.cellId"
						noBackground
					/>
				</template>
			</div>
			<ParamItem
				...
				v-model="action.bingoGrid.label"
				v-if="action.bingoGrid.action == 'rename' || action.bingoGrid.action == 'add_cell'"
			/>
		</template>
		<div v-else class="info alert">
			<p>{{ $t("triggers.actions.bingoGrid.no_grid") }}</p>
			<TTButton light alert small @click="createGrid()">{{
...
class TriggerActionBingoGridEntry extends AbstractTriggerActionEntry {
	public param_grid: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "",
		icon: "bingo_grid",
		labelKey: "triggers.actions.bingoGrid.param_grid",
	};
	public param_action: TwitchatDataTypes.ParameterData<
		TriggerActionBingoGridData["bingoGrid"]["action"],
		TriggerActionBingoGridData["bingoGrid"]["action"]
	> = {
		type: "list",
		value: "tick",
		icon: "click",
		labelKey: "triggers.actions.bingoGrid.param_action",
	};
	public param_x: TwitchatDataTypes.ParameterData<number> = {
		type: "number",
		value: 0,
		min: 1,
		max: 10,
		icon: "coord_x",
		labelKey: "triggers.actions.bingoGrid.param_x",
	};
	public param_y: TwitchatDataTypes.ParameterData<number> = {
		type: "number",
		value: 0,
		min: 1,
		max: 10,
		icon: "coord_y",
		labelKey: "triggers.actions.bingoGrid.param_y",
	};
	public param_name: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		value: "",
		icon: "label",
		labelKey: "triggers.actions.bingoGrid.param_name",
	};
	public param_cellId: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "",
		icon: "bingo_grid",
		labelKey: "triggers.actions.bingoGrid.param_cell",
	};
	...
	public get isCellAction(): boolean {
		return (
			this.action.bingoGrid.action == "tick" ||
	...
	public beforeMount(): void {
		...
		this.param_grid.listValues.unshift({
			value: "",
			label: this.$t("global.select_placeholder"),
		});
	...
	public onSelectGrid(): void {
		...
		if (grid) {
			...
			this.param_cellId.listValues = grid.entries
