<template>
	<div class="triggeractionhighlightentry triggerActionForm">
		<i18n-t scope="global" tag="div" class="info" keypath="triggers.actions.highlight.header">
			<template #LINK>
				<a @click="openHighlightParams()">{{
					$t("triggers.actions.highlight.header_link")
				}}</a>
			</template>
		</i18n-t>
		<ParamItem class="show" :paramData="show_conf" v-model="action.show" />
		<ParamItem
			v-if="show_conf.value === true"
			:paramData="message_conf"
			v-model="action.text"
		/>
	</div>
</template>
...
class TriggerActionHighlightEntry extends AbstractTriggerActionEntry {
	...
	public show_conf: TwitchatDataTypes.ParameterData<boolean, boolean> = {
		type: "list",
		value: false,
		listValues: [],
		icon: "show",
	};
	public message_conf: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		longText: true,
		value: "",
		icon: "highlight",
