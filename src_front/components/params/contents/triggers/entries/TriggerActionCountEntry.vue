<template>
	<div class="triggeractioncountentry triggerActionForm">
			...
			<vue-select
				...
				:selectable="
					(v: TwitchatDataTypes.ParameterDataListValue<string>) =>
						action.counters.indexOf(v.value) == -1
				"
				appendToBody
				multiple
			></vue-select>
		...
		<div
			class="card-item counterList"
			v-if="selectedPerUserCounters.length > 0 && userSourceOptions.length > 1"
		>
			<div class="head">
				<Icon name="user" class="icon" />
				<span>{{
					$t("triggers.actions.count.user_source_title", selectedPerUserCounters.length)
				}}</span>
			</div>
		...
		>
			<div class="head">
				<Icon name="user" class="icon" />
				<span>{{
					$t("triggers.actions.count.user_action_title", selectedPerUserCounters.length)
				}}</span>
			</div>
...
import {
	...
	TriggerActionCountDataActionList,
	type ITriggerPlaceholder,
	type TriggerActionCounterData,
	type TriggerData,
...
class TriggerActionCountEntry extends AbstractTriggerActionEntry {
	...
	public param_counters: TwitchatDataTypes.ParameterData<string[], string> = {
		type: "list",
		labelKey: "triggers.actions.count.select_label",
		value: [],
		listValues: [],
	};
	public param_value: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		labelKey: "triggers.actions.count.value_label",
		value: "",
		maxLength: 100,
		icon: "add",
	};
	public param_action: TwitchatDataTypes.ParameterData<string, string> = {
		type: "list",
		labelKey: "triggers.actions.count.select_action",
		value: "",
		listValues: [],
	};
	public param_userAction: TwitchatDataTypes.ParameterData<
		NonNullable<TriggerActionCounterData["userAction"]>[string],
		NonNullable<TriggerActionCounterData["userAction"]>[string]
	> = { type: "list", labelKey: "triggers.actions.count.param_action", value: "update" };
	...
	public get userSourceOptions(): { key: string; labelKey: string }[] {
		const res: { key: string; labelKey: string }[] = [
			...
			{
				labelKey: "triggers.actions.count.user_source_sender",
				key: COUNTER_EDIT_SOURCE_SENDER,
			},
			{
				labelKey: "triggers.actions.count.user_source_everyone",
				key: COUNTER_EDIT_SOURCE_EVERYONE,
			},
			{
				labelKey: "triggers.actions.count.user_source_chatters",
				key: COUNTER_EDIT_SOURCE_CHATTERS,
			},
		];
		...
		if (this.triggerData.chatCommandParams) {
			this.triggerData.chatCommandParams.forEach((v) => {
				res.push({
					labelKey: "triggers.actions.count.user_source_placeholder",
					key: v.tag,
				});
			});
		}
	...
	public get selectedPerUserCounters(): TwitchatDataTypes.CounterData[] {
		return this.$store.counters.counterList.filter(
			(v) => v.perUser === true && this.action.counters.findIndex((v2) => v2 === v.id) > -1,
		);
	}
	...
	public beforeMount(): void {
		...
		const counters: TwitchatDataTypes.ParameterDataListValue<string>[] =
		...
		for (const value of TriggerActionCountDataActionList) {
			actionList.push({
				value,
				labelKey: "triggers.actions.count.action_" + value.toLowerCase(),
			});
		}
		this.param_action.listValues = actionList;

		watch(
			() => this.selectedPerUserCounters,
