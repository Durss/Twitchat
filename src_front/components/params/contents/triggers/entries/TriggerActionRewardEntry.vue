	...
	<div class="triggeractionrewardentry triggerActionForm">
		...
		<template v-if="param_action.value != 'create' && param_action.value != 'refund'">
			...
			<ParamItem
				...
				v-else-if="param_reward.listValues!.length > 0"
				v-model="action.rewardAction.rewardId"
			/>
			<i18n-t
				scope="global"
				tag="div"
				class="card-item alert"
				keypath="triggers.actions.reward.no_manageable_reward"
				v-else
			>
		...
		<ParamItem
			:paramData="param_state"
			v-if="param_action.value == 'toggle' && param_reward.listValues!.length > 0"
			v-model="action.rewardAction.state"
		/>

		<RewardListEditForm
			v-if="param_action.value == 'create'"
			v-model="action.rewardAction.rewardEdit"
			:placeholderList="placeholderList"
			triggerMode
		/>

		<RewardListEditForm
			v-if="param_action.value == 'edit' && selectedRewardData"
...
import {
	TriggerActionRewardDataActionList,
	TriggerActionRewardDataStateList,
	type TriggerActionRewardData,
	type TriggerActionRewardDataAction,
...
class TriggerActionRewardEntry extends AbstractTriggerActionEntry {
	...
	public param_reward: TwitchatDataTypes.ParameterData<string, string> = {
		type: "list",
		value: "",
		labelKey: "triggers.actions.reward.param_reward",
	};
	public param_action: TwitchatDataTypes.ParameterData<
		TriggerActionRewardDataAction,
		TriggerActionRewardDataAction
	> = { type: "list", value: "toggle", labelKey: "triggers.actions.reward.param_action" };
	public param_state: TwitchatDataTypes.ParameterData<
		TriggerActionRewardDataState,
		TriggerActionRewardDataState
	> = { type: "list", value: "toggle", labelKey: "triggers.actions.reward.param_state" };
	...
	public beforeMount(): void {
		this.param_action.listValues =
			TriggerActionRewardDataActionList.filter((v) => {
		...
		TwitchUtils.getRewards(false, true).then((res) => {
			this.param_reward.listValues = res
				.map((v) => {
			...
			if (this.triggerData.type == TriggerTypes.REWARD_REDEEM) {
				...
				if (!reward || reward.should_redemptions_skip_request_queue) {
					this.param_action.listValues = this.param_action.listValues?.filter(
						(v) => v.value != "refund",
					);
				}
			}
