<template>
	<div class="triggeractiongoxlrentry triggerActionForm">
		...
		<ParamItem
			:paramData="param_fxPreset"
			v-model="action.fxPresetIndex"
			v-if="param_action.value == 'fx_on'"
		/>
		<ParamItem
			:paramData="param_faderId"
			v-model="action.faderId"
			v-if="param_action.value == 'set_fader'"
		/>
		<ParamItem
			:paramData="param_faderValue"
			v-model="action.faderValue"
			v-if="param_action.value == 'set_fader'"
		/>
		<ParamItem
			:paramData="param_profile"
			v-model="action.profile"
			v-if="param_action.value == 'profile'"
		/>
...
import {
	TriggerActionGoXLRDataActionList,
	type TriggerActionGoXLRData,
	type TriggerActionGoXLRDataAction,
...
class TriggerActionGoXLREntry extends AbstractTriggerActionEntry {
	...
	public sampleTarget: Extract<
		...
		| "SamplerTopRight"
	>[] = [];
	public param_action: TwitchatDataTypes.ParameterData<TriggerActionGoXLRDataAction, string> = {
		type: "list",
		value: "fx_on",
		labelKey: "triggers.actions.goxlr.param_action",
	};
	public param_fxPreset: TwitchatDataTypes.ParameterData<number> = {
		type: "list",
		value: 0,
		labelKey: "triggers.actions.goxlr.param_fxPreset",
	};
	public param_faderId: TwitchatDataTypes.ParameterData<GoXLRTypes.InputTypesData> = {
		type: "list",
		value: "Mic",
		labelKey: "triggers.actions.goxlr.param_faderIndex",
	};
	public param_profile: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "Mc",
		labelKey: "triggers.actions.goxlr.param_profile",
	};
	public param_faderValue: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		value: "128",
		labelKey: "triggers.actions.goxlr.param_faderValue",
	};
	...
	public beforeMount(): void {
		this.param_action.listValues = TriggerActionGoXLRDataActionList
			...
			.map((v) => {
				return { value: v.code, labelKey: "triggers.actions.goxlr.action_" + v.code };
			});

		this.param_fxPreset.listValues = [0, 1, 2, 3, 4, 5].map((v) => {
			return { value: v, labelKey: "triggers.actions.goxlr.param_fxPreset" + (v + 1) };
		});
		this.param_fxPreset.listValues.unshift({
			value: -1,
			labelKey: "triggers.actions.goxlr.param_fxPreset_keep",
		});
		...
		if (this.action.sampleIndex) {
			const bt1 = ("SamplerSelect" + this.action.sampleIndex[0]) as
				| "SamplerSelectA"
				| "SamplerSelectB"
				| "SamplerSelectC";
			const bt2 = ("Sampler" + this.action.sampleIndex[1]) as
				| "SamplerBottomLeft"
				| "SamplerBottomRight"
				| "SamplerTopLeft"
				| "SamplerTopRight";
		...
		GoXLRSocket.instance.addEventListener(
			GoXLRSocketEvent.FADER_VOLUME,
			this.goxlrFaderHandler,
		);
		GoXLRSocket.instance.addEventListener(
			GoXLRSocketEvent.BUTTON_PRESSED,
			this.goxlrButtonHandler,
		);
	}

	public beforeUnmount(): void {
		GoXLRSocket.instance.removeEventListener(
			GoXLRSocketEvent.FADER_VOLUME,
			this.goxlrFaderHandler,
		);
		GoXLRSocket.instance.removeEventListener(
	...
	public samplerTargetChange(): void {
		...
		if (this.sampleTarget.length == 2) {
			...
			const bt2 = this.sampleTarget[1]!.replace("Sampler", "") as
				| "BottomLeft"
				| "BottomRight"
				| "TopLeft"
				| "TopRight";
	...
	private onGoxlrButton(e: GoXLRSocketEvent): void {
		switch (this.action.action) {
			case "sample_play": {
				...
				if (
					e.buttonId == "SamplerTopLeft" ||
					e.buttonId == "SamplerTopRight" ||
					e.buttonId == "SamplerBottomLeft" ||
					e.buttonId == "SamplerBottomRight"
				) {
					if (GoXLRSocket.instance.getButtonState("SamplerSelectA"))
						this.action.sampleIndex![0] = "A";
					if (GoXLRSocket.instance.getButtonState("SamplerSelectB"))
						this.action.sampleIndex![0] = "B";
					if (GoXLRSocket.instance.getButtonState("SamplerSelectC"))
						this.action.sampleIndex![0] = "C";
					...
					if (e.buttonId == "SamplerBottomLeft")
						this.action.sampleIndex![1] = "BottomLeft";
					if (e.buttonId == "SamplerBottomRight")
						this.action.sampleIndex![1] = "BottomRight";
				}
				break;
			}
		}
