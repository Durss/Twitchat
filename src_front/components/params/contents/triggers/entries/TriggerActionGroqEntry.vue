<template>
	<div class="triggeractiongroqentry triggerActionForm">
		...
		<ParamItem :paramData="param_jsonMode" v-model="action.groqData!.jsonMode">
			<ParamItem
				:paramData="param_json"
				v-model="action.groqData!.json"
				noBackground
				@blur="onBlurJSON"
				:childLevel="1"
				:error="jsonError != ''"
			...
			<i18n-t
				...
				class="infos"
				tag="div"
				keypath="triggers.actions.groq.jsonMode_hint"
			>
				<template #LINK>
					<a
						href="https://console.groq.com/docs/text-chat#json-mode-object-object"
						target="_blank"
						>{{ $t("triggers.actions.groq.jsonMode_hint_link") }}</a
					>
				</template>
			</i18n-t>
		</ParamItem>
		...
		<ParamItem
			:paramData="param_outputPlaceholder"
			v-model="action.groqData!.outputPlaceholder"
		/>
		...
		<i18n-t
			...
			v-if="
				action.groqData!.outputPlaceholder && action.groqData!.outputPlaceholder.length > 0
			"
		>
...
class TriggerActionGroqEntry extends AbstractTriggerActionEntry {
	public param_preprompt: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		value: "",
		longText: true,
		labelKey: "triggers.actions.groq.preprompt",
	};
	public param_prompt: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		value: "",
		longText: true,
		labelKey: "triggers.actions.groq.prompt",
	};
	public param_jsonMode: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "triggers.actions.groq.jsonMode",
	};
	public param_json: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		value: "",
		longText: true,
		placeholderKey: "triggers.actions.groq.json",
	};
	public param_model: TwitchatDataTypes.ParameterData<string, string> = {
		type: "list",
		value: "",
		longText: true,
		placeholderKey: "triggers.actions.groq.model",
	};
	public param_outputPlaceholder: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		value: "",
		labelKey: "triggers.actions.groq.output_placeholder",
		maxLength: 30,
	...
	public beforeMount(): void {
		if (!this.action.groqData)
			this.action.groqData = {
				preprompt: "",
				prompt: "",
				model: "",
				jsonMode: false,
		...
		this.param_model.listValues = this.$store.groq.availableModels.map((m) => ({
			...
			label:
				m.id.replace(/-/g, " ") +
				(m.id == this.$store.groq.defaultModel ? " (default)" : ""),
		}));
	}
