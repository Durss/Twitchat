	...
	<div class="triggeractionplayabilityentry triggerActionForm">
		<div class="card-item info warn" v-if="!$store.playability.connected">
			...
			<i18n-t
				...
				class="label"
				tag="p"
				keypath="triggers.actions.playability.need_to_connect"
			>
				<template #LINK>
					<a @click="openConnectForm()">{{
						$t("triggers.actions.playability.need_to_connect_link")
					}}</a>
				</template>
			</i18n-t>
		</div>
		...
		<div class="actionList" v-else>
			<div class="action" v-for="(param, index) in action.playabilityData!.outputs">
				<ParamItem
					:paramData="param_outputs[index]"
					v-model="param.code"
					noBackground
					@change="buildValueFields(index)"
				/>
				<div class="value">
					...
					<ParamItem
						:paramData="param_values[index]"
						v-model="param.value"
						noBackground
						:key="param_values[index]!.storage!"
					/>
				</div>
			...
			<TTButton
				class="center"
				icon="add"
				v-if="(action.playabilityData!.outputs?.length || 0) < 40"
				@click="addOutput()"
				>{{ $t("triggers.actions.playability.add_output_bt") }}</TTButton
			>
		</div>
	</div>
</template>
...
class TriggerActionPlayabilityEntry extends AbstractTriggerActionEntry {
	...
	public param_outputs: TwitchatDataTypes.ParameterData<
		string,
		string,
		unknown,
		NonNullable<typeof this.action.playabilityData>["outputs"][0],
		NonNullable<typeof this.action.playabilityData>["outputs"][0]
	>[] = [];
	public param_values: (
		| TwitchatDataTypes.ParameterData<number>
		| TwitchatDataTypes.ParameterData<boolean>
	)[] = [];
	...
	public openConnectForm(): void {
		this.$store.params.openParamsPage(
			...
			TwitchatDataTypes.ParamDeepSections.PLAYABILITY,
		);
	}
	...
	public buildValueFields(resetValueIndex?: number): void {
		for (let index = 0; index < this.action.playabilityData!.outputs.length; index++) {
			...
			const outputType = (output.type =
				this.availableOutputs.find((v) => v.storage?.code === output.code)?.storage?.type ||
				"mouseButton");
			...
			if (["keyboard", "mouseButton", "button"].includes(outputType)) {
				...
				this.param_values[index] = {
					...
					listValues: [
						...
						{
							labelKey: "triggers.actions.playability.output_press_release",
							value: "press_release",
						},
					],
				};
			} else if (["axis"].includes(outputType)) {
				if (index == resetValueIndex) {
			...
			} else if (["trigger"].includes(outputType)) {
				if (index == resetValueIndex) {
