...
class TriggerActionExtensionEntry extends AbstractTriggerActionEntry {
	...
	public param_id: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "",
		labelKey: "triggers.actions.extension.param_id",
		icon: "extension",
	};
	public param_enable: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "",
		labelKey: "triggers.actions.extension.param_enable",
		icon: "disable",
	};
	public param_slot: ISlotItem<string> = {
		type: "list",
		value: "",
		labelKey: "triggers.actions.extension.param_slot",
		slotIndex: "1",
		slotType: "component",
	};
		...
		if (extension) {
			...
			for (const key in slotList) {
				...
				for (let i = 0; i < count; i++) {
					...
					list.push(<ISlotItem<any>>{
						value: slotType + "_" + (i + 1),
						label: this.$t("extensions.type_" + slotType) + suffix,
						slotIndex: (i + 1).toString(),
						slotType,
					});
				}
			}
