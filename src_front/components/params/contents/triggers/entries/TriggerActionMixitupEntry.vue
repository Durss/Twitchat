	...
	<div class="triggeractionmixitupentry triggerActionForm">
		<div class="card-item info warn" v-if="!$store.mixitup.connected">
			...
			<i18n-t
				...
				class="label"
				tag="p"
				keypath="triggers.actions.mixitup.need_to_connect"
			>
				<template #LINK>
					<a @click="openConnectForm()">{{
						$t("triggers.actions.mixitup.need_to_connect_link")
					}}</a>
				</template>
			</i18n-t>
		</div>
		...
		<div class="headerList">
			<div
				class="header head"
				v-if="action.mixitupData!.params && action.mixitupData!.params.length > 0"
			>
			...
			<TTButton
				class="center"
				icon="add"
				v-if="(action.mixitupData!.params?.length || 0) < 40"
				@click="addParam()"
				>{{ $t("triggers.actions.mixitup.add_arg_bt") }}</TTButton
			>
		</div>
	</div>
</template>
...
class TriggerActionMixitupEntry extends AbstractTriggerActionEntry {
	...
	public param_commandId: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "",
		labelKey: "triggers.actions.mixitup.param_commandId",
	};
	...
	public beforeMount(): void {
		...
		watch(
			() => this.$store.mixitup.commandList,
			() => this.buildCommandList(),
		);
	}
	...
	private buildCommandList(): void {
		...
		const grouped = this.$store.mixitup.commandList.reduce(
			(acc, action) => {
		...
		Object.keys(grouped)
			...
			.forEach((groupName) => {
				...
				list.push({
					value: "",
					label: groupName,
					group: grouped[groupName]!.sort((a, b) => a.Name.localeCompare(b.Name)).map(
						(action) => {
