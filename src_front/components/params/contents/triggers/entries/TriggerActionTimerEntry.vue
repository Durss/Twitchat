<template>
	<div class="triggeractiontimerentry triggerActionForm">
		...
		<template v-else>
			<ParamItem
				:paramData="param_timer"
				v-model="action.timerData.timerId"
				@change="updateActions"
			/>
			<template v-if="timer">
				<ParamItem
					:paramData="param_action"
					v-model="action.timerData.action"
					v-if="action.timerData.timerId"
				/>
				<ParamItem
					:paramData="param_duration"
					v-model="action.timerData.duration"
...
import {
	TriggerActionTimerData_ACTION,
	type ITriggerPlaceholder,
	type TriggerActionTimerData,
	type TriggerData,
...
class TriggerActionTimerEntry extends AbstractTriggerActionEntry {
	...
	public param_timer: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "start",
		labelKey: "triggers.actions.timer.param_timer",
	};
	public param_action: TwitchatDataTypes.ParameterData<
		(typeof TriggerActionTimerData_ACTION)[number]
	> = { type: "list", value: "start", labelKey: "triggers.actions.timer.param_action" };
	public param_duration: TwitchatDataTypes.ParameterData<string> = {
		type: "string",
		value: "",
		labelKey: "triggers.actions.timer.param_duration",
	};
	...
	public updateActions(): void {
		const timer = this.$store.timers.timerList.find(
			(t) => t.id == this.action.timerData.timerId,
		);
		if (!timer) return;
