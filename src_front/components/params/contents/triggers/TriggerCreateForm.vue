<template>
	<div class="triggercreateform">
		...
		<i18n-t scope="global" tag="div" class="card-item alert require" v-if="needObsConnect"
		...
		<i18n-t scope="global" tag="div" class="card-item alert require" v-if="needRewards"
		...
		<i18n-t scope="global" tag="div" class="card-item alert require" v-if="needPowerUps"
		...
		<TriggerSearchForm class="searchForm" :debounceDelay="100" v-else-if="subtriggerList.length == 0"
			v-model="search" />
		...
		<div :class="search ? 'list search' : 'list'" v-if="!selectedTriggerType">
			<div class="category" v-for="c in eventCategories" :key="c.category.labelKey"
				...
				<i18n-t scope="global" tag="div" class="card-item alert require"
					v-if="!obsConnected && isOBSCategory(c.category) && !search" keypath="triggers.obs.require">
					<template #URL>
						<a @click="openOBS()">{{ $t("triggers.obs.require_url") }}</a>
					</template>
				</i18n-t>
				...
				<div v-for="e in c.events" :key="e.value" :class="getTriggerClasses(e)"
					...
					<TTButton class="triggerBt" :icon="e.icon" :premium="e.premium === true"
		...
		<div class="sublist">
			<template v-for="item in subtriggerList">
				<component :is="item.subValues ? 'div' : 'button'"
					:class="item.icon ? 'subEventBt hasIcon' : 'subEventBt'" @click="selectSubType(item)">
					<img class="icon" v-if="item.icon" :src="item.icon"
						:style="!item.background ? {} : { backgroundColor: item.background, filter: 'none' }">
					<span class="label">{{ item.label }}</span>
					<span class="small" v-if="item.labelSmall">{{ item.labelSmall }}</span>
				</component>
				<template v-if="item.subValues">
					<button v-for="subItem in item.subValues"
						:class="item.icon ? 'subEventBt subSubEventBt hasIcon' : 'subEventBt subSubEventBt'"
						@click="selectSubType(subItem, item)">
						<img class="icon" v-if="subItem.icon" :src="subItem.icon"
							:style="!subItem.background ? {} : { backgroundColor: subItem.background, filter: 'none' }">
