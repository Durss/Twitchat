<template>
	<div class="connecttwitchbot parameterContent">
		...
		<section v-if="!$store.twitchBot.connected">
			<TTButton
				@click="$store.twitchBot.startAuthFlow($event)"
				icon="twitch"
				:loading="$store.twitchBot.connecting"
		...
		<section v-if="$store.twitchBot.connected">
			<TTButton alert @click="$store.twitchBot.disconnect()"
				>{{ $t("global.disconnect") }} - {{ $store.twitchBot.userInfos?.login }}</TTButton
			>
		</section>
...
class ConnectTwitchBot extends Vue {
	...
	public async addMod(): Promise<void> {
		...
		this.$store.users.getUserFrom(
			"twitch",
			chanId,
			this.$store.twitchBot.userInfos?.user_id,
			this.$store.twitchBot.userInfos?.login,
			undefined,
			async (user) => {
