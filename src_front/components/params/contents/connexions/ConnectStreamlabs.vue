		...
		<template v-else>
			...
			<section class="card-item charity">
				...
				<template v-if="$store.streamlabs.charityTeam">
					<i18n-t
						scope="global"
						keypath="streamlabs.charity_connected"
						class="info"
						tag="div"
					>
						<template #NAME>
							<a :href="$store.streamlabs.charityTeam.pageUrl" target="_blank"
								><Icon name="newtab" />{{ $store.streamlabs.charityTeam.title }}</a
							>
						</template>
					</i18n-t>
					<TTButton alert @click="disconnectCharityCampaign()">{{
						$t("global.disconnect")
					}}</TTButton>
				</template>
				<template v-else>
	...
	public beforeMount(): void {
		if (!this.$store.streamlabs.connected) {
			if (this.$store.streamlabs.authResult.code) {
				...
				this.loading = true;
				this.$store.streamlabs.getAccessToken().then((success) => {
					this.error = !success;
					this.loading = false;
					this.loadAuthURL();
				});
		...
		this.$store.debug.simulateMessage<TwitchatDataTypes.StreamlabsDonationData>(
		...
		this.$store.debug.simulateMessage<TwitchatDataTypes.StreamlabsMerchData>(
			TwitchatDataTypes.TwitchatMessageType.STREAMLABS,
			(mess) => {
		...
		this.$store.debug.simulateMessage<TwitchatDataTypes.StreamlabsPatreonPledgeData>(
			TwitchatDataTypes.TwitchatMessageType.STREAMLABS,
			(mess) => {
		...
		this.$store.debug.simulateMessage<TwitchatDataTypes.StreamlabsCharityData>(
			TwitchatDataTypes.TwitchatMessageType.STREAMLABS,
			(mess) => {
