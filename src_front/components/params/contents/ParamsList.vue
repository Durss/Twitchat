		...
		>
			<div :class="getClasses(p, key as string)" v-if="buildIndex > index">
				<ParamItem
					...
					autoFade
					v-model="p.value"
					@change="$store.params.updateParams()"
				>
					<div
						v-if="
							p.id == 212 &&
					...
					<div v-else-if="p.id == 216 && p.value === true" class="config">
						<Button
							small
							secondary
							@click="$store.params.openParamsPage(contentSpoiler)"
					...
					<div v-else-if="p.id == 217 && p.value === true" class="config">
						<Button
							small
							secondary
							@click="$store.params.openParamsPage(contentAlert)"
							>{{ $t("global.configure") }}</Button
						>
					</div>
...
class ParamsList extends Vue implements IParameterContent {
	...
	public get contentSpoiler(): TwitchatDataTypes.ParameterPagesStringType {
		return TwitchatDataTypes.ParameterPages.SPOILER;
	}
	public get contentAlert(): TwitchatDataTypes.ParameterPagesStringType {
	...
	private startSequentialBuild(): void {
		...
		this.buildInterval = window.setInterval(() => {
			...
			if (this.buildIndex >= Object.keys(this.params).length) {
				...
				const param =
					this.$store.main.tempStoreValue || this.$store.params.currentPageSubContent;
				if (param) {
					this.$nextTick().then(() => {
	...
	private computeMissingScopeStates(): void {
		for (const key in this.params) {
			...
			this.disabledStates[id] =
				(p.id == 212 && !this.isOBSConnected) || this.missingScopeStates[id];
		}
	}
}
