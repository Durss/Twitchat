<template>
	<div :class="classes">
		...
		<div class="obsSceneName" v-if="screen.activeOBSScene">
			<Icon name="obs" />{{ screen.activeOBSScene }}
		</div>

		<div class="enableBt" v-if="selectAreaMode === false && canEnable">
			<ToggleButton
				v-model="screen.enabled"
				@click.stop
				@change="$emit('update')"
				:alert="!screen.enabled"
			/>
		</div>
		...
		<div class="ctas" v-if="selectAreaMode === false && renderOnly == false">
			...
			<button
				@click.stop="$emit('duplicate', screen.id)"
				v-tooltip="$t('global.duplicate')"
				v-if="canDuplicate !== false"
			>
				<Icon name="copy" theme="primary" />
			</button>
			<button @click.stop="$emit('delete', screen.id)" v-tooltip="$t('global.delete')">
...
class HeatScreenPreview extends Vue {
	...
	public get canEnable(): boolean {
		...
		return (
			this.$store.heat.screenList.filter((v) => v.enabled).length < max ||
