...
class OverlayParamsCredits extends Vue {
	public param_fadeSize: TwitchatDataTypes.ParameterData<number> = {
		type: "slider",
		value: 50,
		min: 0,
		max: 400,
	...
	public param_padding: TwitchatDataTypes.ParameterData<number> = {
		type: "slider",
		value: 100,
		min: 0,
		max: 1000,
	...
	public param_paddingTitle: TwitchatDataTypes.ParameterData<number> = {
		type: "slider",
		value: 100,
		min: 0,
		max: 1000,
	...
	public param_stickyTitle: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "overlay.credits.param_stickyTitle",
		icon: "pin",
	};
	public param_fontTitle: TwitchatDataTypes.ParameterData<string> = {
		type: "font",
		value: "",
		labelKey: "overlay.credits.param_fontTitle",
		icon: "font",
	};
	public param_fontEntry: TwitchatDataTypes.ParameterData<string> = {
		type: "font",
		value: "",
		labelKey: "overlay.credits.param_fontEntry",
		icon: "font",
	};
	public param_titleColor: TwitchatDataTypes.ParameterData<string> = {
		type: "color",
		value: "#cccccc",
	};
	public param_entryColor: TwitchatDataTypes.ParameterData<string> = {
		type: "color",
		value: "#ffffff",
	};
	public param_textShadow: TwitchatDataTypes.ParameterData<number> = {
		type: "slider",
		value: 1,
		min: 0,
		max: 100,
	...
	public param_timing: TwitchatDataTypes.ParameterData<string> = {
		type: "list",
		value: "speed",
	...
	public param_speed: TwitchatDataTypes.ParameterData<number> = {
		type: "slider",
		min: 1,
		max: 1000,
		value: 200,
	...
	public param_scale: TwitchatDataTypes.ParameterData<number> = {
		type: "slider",
		min: 1,
		max: 100,
		value: 30,
	...
	public param_showIcons: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: true,
	...
	public param_loop: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: true,
	...
	public param_startDelay: TwitchatDataTypes.ParameterData<number> = {
		type: "slider",
		min: 0,
		max: 30,
		value: 0,
	...
	public param_ignoreBots: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: true,
		labelKey: "overlay.credits.param_ignoreBots",
		icon: "bot",
	};
	public param_hideEmptySlots: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: true,
		labelKey: "overlay.credits.param_hideEmptySlots",
		icon: "hide",
	};
	public param_ignoreCustomBots: TwitchatDataTypes.ParameterData<string[]> = {
		type: "editablelist",
		value: [],
		max: 50,
		maxLength: 25,
	...
	public param_powerUpEmotes: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
	...
	public param_showAllActiveSubs: {
	...
	public param_showAllActiveSubgifters: {
		[key: string]: TwitchatDataTypes.ParameterData<boolean, undefined, boolean>;
	} = {};
	...
	public param_showTipsStreamlabs: { [key: string]: TwitchatDataTypes.ParameterData<boolean> } =
		{};
	public param_showTipsStreamelements: {
	...
	public beforeMount(): void {
			...
			for (let i = 0; i < this.slots.length; i++) {
				...
				if (!defaultSlot) {
					...
					if (this.getDefinitionFromSlot(slot.slotType).premium) {
						...
						if (!this.isPremium && slot.showAmounts != undefined)
							slot.showAmounts = false;
					} else {
		...
		if (!this.isPremium) {
			...
			if (this.$store.endingCredits.overlayData.powerUpEmotes === undefined)
				this.$store.endingCredits.overlayData.powerUpEmotes = true;
		}

		watch(
			() => this.$store.endingCredits.overlayData,
			() => this.saveParams(),
			{ deep: true },
		);
	...
	public async addSlot(
		...
		if (slotDef.hasAmount) {
			...
			} else if (!this.isPremium && slotDef.premium) {
				entry.showAmounts = false;
			}
			let icon = "number";
			if (slotDef.id == "patreonMembers") icon = "date";
			this.param_showAmounts[id] = {
				type: "boolean",
				icon,
				value: entry.showAmounts || true,
				labelKey: this.getDefinitionFromSlot(slotType).amountLabel,
				premiumOnly: true,
			};
		}
		...
		if (slotDef.id == "rewards") {
				...
				children.push({
					...
					editCallback: (data) => {
						...
						if (data.value === false && entry.rewardIds!.includes(data.storage!.id)) {
							...
							entry.rewardIds = entry.rewardIds!.filter(
								(v) => v !== data.storage!.id,
							);
						}
					},
			...
			const placeholderList = TriggerEventPlaceholders(
				TriggerTypes.GLOBAL_PLACEHOLDERS,
			).concat();
			this.param_text[id] = {
				type: "string",
				value: "",
			...
			if (entry.showSubsYoutube == undefined || !this.isPremium)
				entry.showSubsYoutube = false;
			if (entry.showSubgiftsYoutube == undefined || !this.isPremium)
			...
			this.param_showSubgiftsYoutube[id] = {
				type: "boolean",
			...
			if (entry.showSubsKofi === undefined)
				entry.showTipsKofi = entry.showSubsYoutube ?? true; //I made a mistake in the past, setting this v-model's entry to "showSubsYoutube".
			...
			this.param_showTipsStreamlabs[id] = {
				type: "boolean",
			...
			this.param_showTipsStreamelements[id] = {
				type: "boolean",
			...
			this.param_showMerchStreamlabs[id] = {
				type: "boolean",
			...
			if (entry.patreonTiers == undefined)
				entry.patreonTiers = StoreProxy.patreon.tierList.map((v) => v.id) || [];

			this.param_anonLastNames[id] = {
				type: "boolean",
			...
			this.param_sortByTotalAmount[id] = {
				type: "boolean",
			...
			for (const tier of tierList) {
				...
				children.push({
					...
					editCallback: (data) => {
						if (
							data.value === true &&
							!entry.patreonTiers!.includes(data.storage!.id)
						) {
