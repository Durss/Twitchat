		...
		>
			...
			>
				<template #right_actions>
					<TTButton
						@click.stop="$store.animatedText.deleteAnimatedText(entry.id)"
						icon="trash"
						alert
					/>
				</template>
				...
				<template #left_actions>
					<ToggleButton
						...
						v-if="
							...
							entry.enabled ||
							$store.animatedText.animatedTextList.filter((v) => v.enabled).length <
...
class OverlayParamsAnimatedText extends Vue {
	...
	public param_animStyle: {
		[key: string]: TwitchatDataTypes.ParameterData<
			TwitchatDataTypes.AnimatedTextData["animStyle"]
		>;
	} = {};
	...
	public get maxReached(): boolean {
		...
		const max = this.$store.auth.isPremium
			? this.$config.MAX_ANIMATED_TEXT_PREMIUM
	...
	private initParams(): void {
		this.$store.animatedText.animatedTextList.forEach((entry) => {
			...
			this.param_animDurationScale[id] = {
				type: "slider",
				value: 1,
				min: 0,
				step: 0.05,
			...
			this.param_animStyle[id].listValues = TwitchatDataTypes.AnimatedTextData_AnimStyles.map(
				(v) => {
					return {
						value: v,
