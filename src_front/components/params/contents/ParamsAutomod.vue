...
class ParamsAutomod  extends Vue implements IParameterContent {
	...
	public param_banUserNames: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
	 ...
	 */
	public get testClean(): string {
		return UnicodeUtils.instance.normalizeAlphaNum(this.testStr).toLowerCase().trim();
	}
		...
		for (const f of this.automodData.keywordsFilters) {
			...
			if (valid) {
				if (reg.test(this.testClean)) {
					...
					reg.test(UnicodeUtils.instance.normalizeLeet(this.testStr).toLowerCase().trim())
				) {
					matchingRules.push(f);
				}
			}
	...
	public deleteRule(rule: TwitchatDataTypes.AutomodParamsKeywordFilterData): void {
		this.$confirm(
			this.$t("automod.delete_confirm_title"),
			this.$t("automod.delete_confirm_description"),
		)
			.then(() => {
	...
	private initRule(data: TwitchatDataTypes.AutomodParamsKeywordFilterData): void {
		...
		this.param_ruleLabel[data.id] = {
			labelKey: "automod.rule.name",
			type: "string",
			value: "",
			maxLength: 30,
		};
		this.param_ruleRegex[data.id] = {
		...
		this.param_ruleSync[data.id] = {
			labelKey: "automod.rule.sync",
			type: "boolean",
			value: false,
			icon: "anon",
		};
		this.param_ruleEmergency[data.id] = {
			labelKey: "automod.rule.emergency",
			type: "boolean",
			value: false,
			icon: "emergency",
		};
		this.param_ruleOnlyFirst[data.id] = {
