	...
	<div class="paramstriggers parameterContent">
		...
		<div class="head">
			...
			>
				<template #CMD
					><mark v-click2Select>{{
						$store.chat.commands.find((v) => v.id == "triggerlogs")?.cmd
					}}</mark></template
				>
			</i18n-t>
		</div>
		...
		<template v-if="showList && !showForm">
			<Button
				...
				v-if="
					...
					($store.auth.isPremium && activeTriggerCount < $config.MAX_TRIGGERS_PREMIUM)
				"
				icon="add"
				primary
...
class ParamsTriggers  extends Vue implements IParameterContent {
	...
	public get currentTriggerData(): TriggerData | null {
		return this.$store.triggers.currentEditTriggerData;
	}
	public get showList(): boolean {
		return this.currentTriggerData == null;
	}
	public get isAffiliate(): boolean {
		return this.$store.auth.twitch.user.is_affiliate || this.$store.auth.twitch.user.is_partner;
	}
	public get canManageRewards(): boolean {
		return TwitchUtils.hasScopes([TwitchScopes.MANAGE_REWARDS]);
	}
	public get canReadBits(): boolean {
		return TwitchUtils.hasScopes([TwitchScopes.READ_CHEER]);
	}
	public get canManageExtensions(): boolean {
					...
					async (data) => {
						...
						if (trigger.type == TriggerTypes.SLASH_COMMAND) {
						 ...
						 if (m.type == TwitchatDataTypes.TwitchatMessageType.REWARD) {
							...
							let twitchReward = this.$store.rewards.rewardList.find(
						 ...
						 if (
							m.type ==
							TwitchatDataTypes.TwitchatMessageType.OBS_PLAYBACK_STATE_UPDATE
						) {
						...
						if (m.type == TwitchatDataTypes.TwitchatMessageType.VALUE_UPDATE) {
							const value = this.$store.values.valueList.find(
								(v) => v.id == trigger.valueId,
							);
							if (value) {
								m.value = value;
						 ...
						 */ else if (triggerEvent.value == TriggerTypes.COUNTDOWN_START) {
						 ...
						 if (triggerEvent.value == TriggerTypes.SUBGIFT) {
							...
							if (trigger.heatClickSource == "area" && trigger.heatAreaIds) {
								(m as TwitchatDataTypes.MessageHeatClickData).areaId =
									Utils.pickRand(trigger.heatAreaIds);
							}
							...
							triggerEvent.value == TriggerTypes.COMMUNITY_CHALLENGE_COMPLETE
						) {
							//Remove ban duration so it counts as a ban, not a timeout
							(
							...
							(m as TwitchatDataTypes.MessageMusicAddedToQueueData).failReason =
								this.$t("triggers.actions.music.fail_reasons." + code, {
									DURATION: "03:33",
							...
							(m as TwitchatDataTypes.KofiDonationData).amountFormatted =
								amountFormatted;
							...
							(m as TwitchatDataTypes.KofiSubscriptionData).eventType =
								"subscription";
