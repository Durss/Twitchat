<template>
	<div class="paramsdiscordquickactions">
		<TTButton primary icon="add" @click="$store.discord.addQuickAction()">{{
			$t("discord.quick_actions_addBt")
		}}</TTButton>
		<ToggleBlock
			medium
			editableTitle
		...
		>
			<template #left_actions>
				<TTButton
					...
					icon="trash"
					alert
					@click.stop="$store.discord.delQuickAction(a.data)"
				/>
			</template>
			<div class="entry">
				<ParamItem
					:key="'message_' + a.data.id"
					class="param"
					@change="$emit('change')"
					:paramData="a.message"
					v-model="a.data.message"
				></ParamItem>
				<ParamItem
					:key="'channelId_' + a.data.id"
					class="param"
					@change="$emit('change')"
					:paramData="a.channel"
					v-model="a.data.channelId"
				></ParamItem>
			</div>
		</ToggleBlock>
	</div>
...
class ParamsDiscordQuickActions extends Vue {
	...
	public get quickActions(): {
		data: TwitchatDataTypes.DiscordQuickActionData;
		...
		return (this.$store.discord.quickActions || []).map((a) => {
			return reactive({
				...
				message: {
					type: "string",
					value: a.message || "",
					labelKey: "discord.quick_actions_message",
					placeholderList: this.placeholders,
					longText: true,
					maxLength: 2000,
				},
				channel: {
					type: "list",
					value: a.channelId || "",
					listValues: this.discordChans,
					labelKey: "discord.quick_actions_channel",
				},
			});
		});
	}
