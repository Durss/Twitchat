<template>
	<div class="triggerimportform modal">
		...
		<div class="holder" ref="holder">
			<div class="head">
				<h1 class="title">
					<Icon name="download" /> {{ $t("exporter.importForm.title") }}
				</h1>
			...
			<form
				...
				class="content"
				v-if="typeof importedSettings.data === 'string'"
			>
				<p class="info"><Icon name="key" />{{ $t("exporter.importForm.dataProtected") }}</p>
				<input
					type="password"
					:placeholder="$t('exporter.importForm.dataPassword')"
					v-model="password"
				/>
				<span class="card-item alert" v-if="decryptError" @click="decryptError = false"
					><Icon name="alert" />{{ $t("exporter.importForm.dataInvalidPassword") }}</span
				>
				<TTButton class="submitBt" icon="unlock" type="submit">{{
					$t("global.submit")
				}}</TTButton>
			</form>
			<div class="content" v-else-if="importedSettings.data.version < TRIGGER_SHARED_VERSION">
				...
				<TTButton class="submitBt" icon="cross" @click="close()">{{
					$t("global.close")
				}}</TTButton>
			</div>
			<div
				class="content"
				v-else-if="
					importedSettings.data.autoDelete_at > 0 &&
					importedSettings.data.autoDelete_at < Date.now()
				"
			>
			...
			<div class="content" v-else>
				...
				<template v-if="importedSettings.data.params.length > 0">
					...
					<ul class="paramList">
						<li
							v-for="(param, index) in importedSettings.data.params"
							:key="index"
							...
							<div class="holder">
								<p class="label">
									<Icon
										name="alert"
										theme="secondary"
										v-tooltip="$t('global.mandatory')"
										v-if="isErroredField(param)"
									/>
									<label
										:for="'field_' + index"
										v-html="
											param.description.replace(/(\(.*?\))/gi, '<i>$1</i>')
										"
									></label>
								</p>
								...
								<template v-else-if="param.valueType === 'list'">
									<select :inputId="'field_' + index" v-model="param.value">
										...
										<option
											v-for="(item, idx) in param.listItems!"
											:key="idx"
											:value="item"
										>
											{{ item }}
				...
				<ToggleBlock :title="$t('exporter.importForm.details')" :open="false" small>
					...
					<ul>
						<li v-if="importedSettings.data.triggers?.length > 0">
							{{ importedSettings.data.triggers.length }}
							{{
								$t(
									"exporter.importForm.importTypes.triggers",
									importedSettings.data.triggers.length,
								)
							}}
						</li>
						<li v-if="importedSettings.data.counters?.length > 0">
							{{ importedSettings.data.counters.length }}
							{{
								$t(
									"exporter.importForm.importTypes.counters",
									importedSettings.data.counters.length,
								)
							}}
						</li>
						<li v-if="importedSettings.data.values?.length > 0">
							{{ importedSettings.data.values.length }}
							{{
								$t(
									"exporter.importForm.importTypes.values",
									importedSettings.data.values.length,
								)
							}}
						</li>
						<li v-if="importedSettings.data.labels?.length > 0">
							{{ importedSettings.data.labels.length }}
							{{
								$t(
									"exporter.importForm.importTypes.labels",
									importedSettings.data.labels.length,
								)
							}}
						</li>
						<li v-if="importedSettings.data.animatedTexts?.length > 0">
							{{ importedSettings.data.animatedTexts.length }}
							{{
								$t(
									"exporter.importForm.importTypes.animatedTexts",
									importedSettings.data.animatedTexts.length,
								)
							}}
						</li>
						<li v-if="importedSettings.data.customTrains?.length > 0">
							{{ importedSettings.data.customTrains.length }}
							{{
								$t(
									"exporter.importForm.importTypes.customTrains",
									importedSettings.data.customTrains.length,
								)
							}}
						</li>
						<li v-if="importedSettings.data.endingCreditsSlots?.length > 0">
							{{ importedSettings.data.endingCreditsSlots.length }}
							{{
								$t(
									"exporter.importForm.importTypes.endingCreditsSlots",
									importedSettings.data.endingCreditsSlots.length,
								)
							}}
						</li>
						<li v-if="importedSettings.data.timers?.length > 0">
							{{ importedSettings.data.timers.length }}
							{{
								$t(
									"exporter.importForm.importTypes.timers",
									importedSettings.data.timers.length,
								)
							}}
						</li>
				...
				<TTButton
					...
					v-tooltip="isFormValid ? '' : $t('global.mandatory_fill')"
					:loading="isLoading"
...
import {
	TRIGGER_SHARED_VERSION,
	type SettingsExportData,
	type TriggerImportData,
...
class TriggerImportForm extends Vue {
	...
	public get isFormValid(): boolean {
		...
		for (const p of this.importedSettings.data.params) {
			if (
				...
				return false;
			if (p.valueType === "number" && (typeof p.value !== "number" || isNaN(p.value)))
				return false;
	...
	public isErroredField(param: SettingsExportData["params"][number]): boolean {
		if (
			...
			(typeof param.value !== "string" || param.value.trim().length === 0)
		)
			return true;
