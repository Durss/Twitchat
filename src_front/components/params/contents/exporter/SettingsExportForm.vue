		...
		<form @submit.prevent="submit">
			...
			<div class="paramList" v-if="paramList.length > 0">
				<div v-for="(param, index) in paramList" :key="index" class="paramItem">
					...
					<ParamItem
						...
						v-model="param.listItems"
						v-if="param.valueType === 'list'"
					/>
				</div>
			</div>
			...
			<div class="block">
				<div class="autoDeleteToggle">
					<label for="autoDeleteToggle">Auto delete settings</label>
					<ToggleButton inputId="autoDeleteToggle" v-model="autoDelete" />
				</div>

				<div class="autoDeleteValue" v-if="autoDelete">
					<label for="autoDeleteDate">Auto delete at</label>
					<input type="datetime-local" id="autoDeleteDate" v-model="autoDeleteDate" />
				</div>
			</div>

			<div class="block">
				<div class="protectToggle">
					<label for="protectToggle">Protect with password</label>
					<ToggleButton inputId="protectToggle" v-model="protect" />
				</div>

				<div class="protectPass" v-if="protect">
					<p class="info">
						Settings will be encrypted with no way to recover them if you lose your
						password.
					</p>
					<label for="protectPass">Password</label>
					<input type="password" id="protectPass" v-model="password" />
				</div>
			</div>
			...
			<TTButton
				...
				type="submit"
				:disabled="!canSubmit"
				:loading="$store.exporter.exportingSelectedSettings"
				>Create</TTButton
			>
...
class SettingsExportForm extends Vue {
	...
	public get canSubmit(): boolean {
		...
		if (
			this.$store.exporter.selectedTimerIDs.length === 0 &&
