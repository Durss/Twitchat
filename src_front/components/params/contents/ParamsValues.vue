		...
		<section class="card-item" v-if="showForm">
			<form @submit.prevent="createValue()">
				...
				<ParamItem
					...
					v-newflag="{ date: $config.NEW_FLAGS_DATE_V11, id: 'values_form_more' }"
				>
					<ParamItem
						class="child"
						noBackground
				...
				>
					<template #left_actions>
						<ToggleButton
							...
							v-if="
								($store.auth.isPremium && entry.value.enabled === false) ||
							...
							<template v-if="Object.keys(entry.value.users ?? {}).length > 0">
								<div class="search">
									...
									<Icon
										name="loader"
										class="loader"
										v-show="entry.idToLoading[entry.value.id] === true"
									/>
								</div>
...
class ParamsValues  extends Vue implements IParameterContent {
	...
	public get maxReached(): boolean {
		...
		const max = this.$store.auth.isPremium
			? this.$config.MAX_VALUES_PREMIUM
		...
		watch(
			...
			() => {
				...
				this.param_placeholder.errorMessage = exists
					? this.$t("values.form.placeholder_conflict")
					: "";
			},
		);
	...
	public deleteValue(entry: ValueEntry): void {
		this.$confirm(this.$t("values.delete_confirm.title"), this.$t("values.delete_confirm.desc"))
			.then(() => {
					...
					if ((entry.value.users || {})[u.id]) {
						...
						const userEntry: UserEntry = {
							...
							param: reactive({
								type: "string",
								value:
									value.users && value.users[u.id]
										? value.users![u.id]!.value
										: "",
								maxLength: 100000,
							}),
							platform: "twitch",
	...
	public resetUsers(entry: ValueEntry): void {
		this.$confirm(
			this.$t("values.reset_users_confirm.title"),
			this.$t("values.reset_users_confirm.desc"),
		)
			.then(() => {
				//Reset value data
	...
	public clearUsers(entry: ValueEntry): void {
		this.$confirm(
			this.$t("values.delete_users_confirm.title"),
			this.$t("values.delete_users_confirm.desc"),
		)
			.then(() => {
				//Reset value data
				entry.value.users = {};
