		...
		<i18n-t
			...
			:keypath="
				messageData.moderator
					? 'global.moderation_action.unbanned_by'
					: 'global.moderation_action.unbanned'
			"
		>
			<template #USER>
				<a
...
class ChatBan extends AbstractChatMessage {
	...
	public mounted(): void {
		...
		if (this.messageData.moderator) {
			aria = this.$t("global.moderation_action.unbanned_by", {
				MODERATOR: this.messageData.moderator.displayName,
				USER: this.messageData.user.displayName,
			});
		} else {
			aria = this.$t("global.moderation_action.unbanned", {
				USER: this.messageData.user.displayName,
			});
		}
