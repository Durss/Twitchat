...
export class MessageListFilter extends Vue {
	...
	>[] = [];
	public param_toggleAll: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "chat.filters.select_all",
	};
	public param_showBadges: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "chat.filters.show_user_badges",
		icon: "show",
	};
	public param_hideBadges: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "chat.filters.hide_user_badges",
		icon: "hide",
	};
	public param_hideUsers: TwitchatDataTypes.ParameterData<string, string> = {
		type: "editablelist",
		value: "",
	...
	public param_showPanelsHere: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "chat.filters.show_panels_here",
	};
	public param_showGreetHere: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
		labelKey: "chat.filters.show_greet_here",
	};
	public param_backgroundColor: TwitchatDataTypes.ParameterData<string> = {
		type: "color",
		value: "",
	};
	public messageKeyToScope:
	...
	private messagesCache: Partial<{
		[key in (typeof TwitchatDataTypes.MessageListFilterTypes)[number]["type"]]:
			| TwitchatDataTypes.ChatMessageTypes[]
			| null;
	}> = {};
	private subMessagesCache: Partial<{
		[key in keyof TwitchatDataTypes.ChatColumnsConfigMessageFilters]:
			| TwitchatDataTypes.ChatMessageTypes[]
			| null;
	}> = {};

	public get whisperType() {
		return TwitchatDataTypes.TwitchatMessageType.WHISPER;
	}
	public get messageType() {
		return TwitchatDataTypes.TwitchatMessageType.MESSAGE;
	}

	public get classes(): string[] {
		const res = ["messagelistfilter"];
		if (this.$store.params.appearance.splitViewVertical.value === true)
			res.push("verticalSplitMode");
	...
	public get channels() {
		...
		if (this.$store.tiktok.connected) {
			const user: TwitchatDataTypes.TwitchatUser = JSON.parse(
				JSON.stringify(this.$store.auth.twitch.user),
			);
			user.id = "tiktok";
	...
	public async previewMessage(
		...
		if (filter.type == TwitchatDataTypes.TwitchatMessageType.NOTICE) {
			this.$store.debug.simulateNotice<TwitchatDataTypes.MessageNoticeData>(
			...
			this.$store.debug.simulateMessage<TwitchatDataTypes.MessageNoticeData>(
			...
			this.$store.debug.simulateMessage<TwitchatDataTypes.MessageShoutoutData>(
			...
			this.$store.debug.simulateMessage<TwitchatDataTypes.MessageStreamOnlineData>(
			...
			this.$store.debug.simulateMessage<TwitchatDataTypes.MessageStreamOfflineData>(
			...
			this.$store.debug.simulateMessage<TwitchatDataTypes.MessageUnbanRequestData>(
			...
			await this.$store.debug.simulateMessage<TwitchatDataTypes.MessageSubscriptionData>(
			...
			this.$store.debug.simulateMessage<TwitchatDataTypes.MessageYoutubeSubscriptionData>(
			...
			await this.$store.debug.simulateMessage<TwitchatDataTypes.MessageCheerData>(
			...
			this.$store.debug.simulateMessage<TwitchatDataTypes.MessageYoutubeSuperChatData>(
			...
			this.$store.debug.simulateMessage<TwitchatDataTypes.MessageYoutubeSuperStickerData>(
			...
			this.$store.debug.simulateMessage<TwitchatDataTypes.MessagePrivateModeratorData>(
	...
	public async previewSubMessage(
	...
	public deleteColumn(): void {
		this.$store.main
			.confirm(
				this.$t("chat.delete_col_confirm_title"),
				this.$t("chat.delete_col_confirm_desc"),
			)
			.then(() => {
...
.messagelistfilter {
	...
	.holder {
		...
		.previewList {
			...
			filter: drop-shadow(0 0 0.5em var(--color-text));
			.preview {
