		...
		<i18n-t scope="global" tag="div" keypath="chat.bingo.title">
			<template #WINNER>
				<a
					...
					@click.stop="
						openUserCard(messageData.bingoData.winners![0], messageData.channel_id)
					"
					>{{ messageData.bingoData.winners![0].displayName }}</a
				>
			</template>
			<template #ANSWER>
				<img
					class="answer emote"
					:src="messageData.bingoData.emoteValue?.twitch?.image.hd"
					v-if="messageData.bingoData.guessEmote"
				/>
				<strong class="answer" v-else-if="messageData.bingoData.guessNumber">{{
					messageData.bingoData.numberValue
				}}</strong>
