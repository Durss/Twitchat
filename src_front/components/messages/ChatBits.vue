	...
	<div :class="classes" @contextmenu="onContextMenu($event, messageData, $el)">
		...
		<div class="holder">
			...
			<div class="quote" v-if="messages.length > 0">
				<span
					v-tooltip="messages.length > 1 ? mess.bits + ' bits' : null"
					v-for="mess in messages"
					><ChatMessageChunksParser
						:chunks="mess.message_chunks"
						:channel="mess.channel_id"
...
class ChatBits extends AbstractChatMessage {
	...
	public get messages(): TwitchatDataTypes.MessageCheerData[] {
		...
		if (this.messageData.message_chunks && this.messageData.message_chunks.length > 0)
			res.push(this.messageData);
		if (this.childrenList) {
	...
	public mounted(): void {
		const reason = this.$t(
			"chat.bits",
			{ COUNT: this.totalBits, USER: this.messageData.user.displayName },
			this.totalBits,
		);
		this.$store.accessibility.setAriaPolite(reason + " " + this.messageData.message);
		this.computeState();
		watch(
			() => this.messageData.pinned,
			() => this.computeState(),
		);
	}
	...
	private async computeState(): Promise<void> {
		...
		const remainingDuration = Math.max(
			0,
			duration - (Date.now() - this.messageData.date) / 1000,
		);
