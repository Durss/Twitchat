...
export default class AbstractChatMessage extends Vue {
	...
	public beforeMount() {
		if (
			this.messageData.type == TwitchatDataTypes.TwitchatMessageType.MESSAGE ||
			this.messageData.type == TwitchatDataTypes.TwitchatMessageType.WHISPER
		) {
			this.canModerateUser_local = this.canModerateUser(
			...
			this.canModerateUser_local = this.canModerateUser(
				this.messageData.message.user,
				this.messageData.message.channel_id,
			);
		}
		...
		if (
			...
			this.canModerateMessage =
				this.canModerateUser_local &&
				//If it's not announcement (they're not deletable)
				(!("twitch_announcementColor" in this.messageData) ||
					this.messageData.twitch_announcementColor == undefined) &&
		...
		watch(
			() => this.$store.params.appearance.displayTimeRelative.value,
			() => {
	...
	public copyJSON(): void {
		void Utils.copyToClipboard(JSON.stringify(this.messageData));
		console.log(this.messageData);
		gsap.fromTo(this.$el, { scale: 1.2 }, { duration: 0.5, scale: 1, ease: "back.out(1.7)" });
	}
	...
	public applyStyles(): void {
		...
		} else if (this.messageData.todayFirst && params.highlight1stToday.value === true) {
			color = params.highlight1stToday_color.value as string;
		} else if (this.messageData.hasMention && params.highlightMentions.value === true) {
		...
		} else if (this.messageData.user.is_partner && params.highlightPartners.value === true) {
