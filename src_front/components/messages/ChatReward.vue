	...
	<div :class="classes" @contextmenu="onContextMenu($event, messageData, $el)">
		...
		<div class="holder">
			<div v-for="entry in rewardList" :key="entry.vo.id" class="item">
				<img
					:src="getIcon(entry.vo)"
					alt="reward"
					class="icon"
					:style="{ backgroundColor: entry.vo.reward.color }"
				/>

				<TTButton
					v-if="entry.vo.message_html == undefined && getCanRefund()"
					@click.stop="refund()"
					v-tooltip="
						entry.count > 1
							? $t('chat.reward.refundOne_tt')
				...
				<span class="card-item secondary count" v-if="entry.count > 1"
					>x{{ entry.count }}</span
				>
				...
				<i18n-t scope="global" tag="span" keypath="chat.reward.message">
					...
					<template #COST>
						<span class="cost" v-if="entry.vo.reward.cost > 0"
							>({{ entry.vo.reward.cost }}pts)</span
						>
					</template>
				</i18n-t>
				...
				<div
					class="quote"
					v-if="
						$store.params.appearance.showRewardsInfos.value === true &&
						entry.vo.reward.description
					"
				>
					{{ entry.vo.reward.description }}
				</div>
			...
			<div
				class="prompt quote"
				v-if="childrenList"
				v-for="child in childrenList.filter((v) => v.message_html != undefined)"
				:key="child.id"
			>
				<TTButton
					v-if="getCanRefund(child)"
...
class ChatReward extends AbstractChatMessage {
	...
	public get rewardList(): { count: number; vo: TwitchatDataTypes.MessageRewardRedeemData }[] {
		...
		const finalList: { count: number; vo: TwitchatDataTypes.MessageRewardRedeemData }[] =
			Object.keys(idCountDictionary).map((id) => {
	...
	public getCanRefund(message?: TwitchatDataTypes.MessageRewardRedeemData): boolean {
		...
		return (
			this.refundableIds[message.reward.id] != undefined &&
