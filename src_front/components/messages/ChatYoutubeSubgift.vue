		...
		<div class="holder">
			<i18n-t scope="global" tag="p" keypath="chat.youtube_subgift.title">
				...
				<template #LIST>
					<span
						class="user"
						v-if="messageData.gift_recipients.length > 0"
						v-for="(u, index) in messageData.gift_recipients"
						:key="u.id"
					>
						<a
							class="userlink"
							@click.stop="
								openUserCard(u, messageData.channel_id, messageData.platform)
							"
							>{{ u.displayName }}</a
						>
						<span v-if="index == messageData.gift_recipients.length - 2"
							>&nbsp;{{ $t("global.and") }}&nbsp;</span
						>
