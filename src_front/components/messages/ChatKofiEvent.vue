	...
	>
		...
		<div class="messageHolder">
			<i18n-t scope="global" tag="span" :keypath="labelKey">
				...
				<template #COMMISSION_LINK v-if="messageData.eventType == 'commission'">
					<a :href="messageData.url" target="_blank"
						><strong
							><Icon name="newtab" />{{ $t("chat.kofi.commission_link") }}</strong
						></a
					>
				</template>
				...
				<template #PRODUCT v-if="messageData.eventType == 'merch' && messageData.products">
					<strong>{{
						$t(
							"chat.kofi.merch_product",
							{ COUNT: messageData.products.length },
							messageData.products.length,
						)
					}}</strong>
					<i
						>(
						<template v-for="(p, index) in messageData.products" :key="p.id">
							<a :href="'https://ko-fi.com/s/' + p.id" target="_blank"
								><Icon name="newtab" />🎁</a
							>
							<!-- <span class="count" v-if="p.quantity">(x{{ p.quantity }})</span> -->
							<span v-if="index == messageData.products.length - 2"
								>&nbsp;{{ $t("global.and") }}&nbsp;</span
							>
							<span v-else-if="index < messageData.products.length - 1 && index >= 0"
								>,
							</span>
						</template>
						)</i
			...
			<div
				class="quote"
				v-if="
					(messageData.eventType == 'donation' || messageData.eventType == 'merch') &&
					messageData.message
				"
			>
...
class ChatKofiEvent extends AbstractChatMessage {
	...
	public get labelKey(): string {
		switch (this.messageData.eventType) {
			case "merch":
				return "chat.kofi.merch";
			case "commission":
				return "chat.kofi.commission";
			case "donation":
				return "chat.kofi.donation";
			case "subscription":
				return this.messageData.tier
					? "chat.kofi.subscription_tier"
					: "chat.kofi.subscription";
		}
		return "";
	}
}
