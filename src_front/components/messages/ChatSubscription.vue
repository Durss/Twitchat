		...
		<div>
			...
			<div class="holder" v-if="messageData.is_gift">
				<i18n-t
					...
					tag="span"
					:keypath="
						messageData.is_targetedSubgift
							? 'chat.subscription.sub_gift_months'
							: 'chat.subscription.sub_gift'
					"
				>
					<template #USER>
						<a
					...
					<template #LIST>
						<span
							...
							v-if="giftRecipients.length > 0"
							v-for="(u, index) in giftRecipients"
							:key="u.id"
						>
							<a
								class="userlink"
								@click.stop="openUserCard(u, messageData.channel_id)"
								>{{ u.displayName }}</a
							>
							<span v-if="index == giftRecipients.length - 2"
								>&nbsp;{{ $t("global.and") }}&nbsp;</span
							>
				...
				<i18n-t
					...
					keypath="chat.subscription.sub_gift_total"
					:plural="totalSubgifts"
				>
					<template #COUNT>
			...
			<div class="holder" v-else-if="messageData.is_giftUpgrade">
				<i18n-t
					...
					tag="span"
					:keypath="
						messageData.gift_upgradeSender
							? 'chat.subscription.sub_gift_upgrade'
							: 'chat.subscription.sub_gift_upgrade_anon'
					"
				>
					<template #USER>
						<a
					...
					<template #GIFTER v-if="messageData.gift_upgradeSender">
						<a
							class="userlink"
							@click.stop="
								openUserCard(messageData.gift_upgradeSender, messageData.channel_id)
							"
							>{{ messageData.gift_upgradeSender.displayName }}</a
						>
					</template>
					<template #TIER>
						<strong>{{ messageData.tier }}</strong>
			...
			<div class="holder" v-else>
				...
				<i18n-t
					...
					:keypath="
						messageData.tier == 'prime'
							? 'chat.subscription.resub_prime'
							: 'chat.subscription.resub'
					"
				>
					<template #USER>
						<a
				...
				<i18n-t
					...
					keypath="chat.subscription.sub_duration"
				...
				<div class="additional" v-if="messageData.months > 1">
					<i18n-t
						scope="global"
						keypath="chat.subscription.sub_prepaid"
