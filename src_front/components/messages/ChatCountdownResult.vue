<template>
	<div class="chatcountdownresult chatMessage highlight">
		...
		<i18n-t
			scope="global"
			tag="div"
			v-if="messageData.aborted === true"
			keypath="chat.countdown.abort"
		>
			<template #DURATION
				><strong>{{ messageData.duration_str }}</strong></template
			>
			<template #ABORT_DURATION
				><strong>{{ messageData.finalDuration_str }}</strong></template
			>
		</i18n-t>
		...
		<i18n-t
			scope="global"
			tag="div"
			v-else-if="messageData.endedAt_str"
			keypath="chat.countdown.complete"
		>
			<template #DURATION
				><strong>{{ messageData.duration_str }}</strong></template
			>
