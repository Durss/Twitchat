<template>
	<div class="chatmessageclippending chatMessage highlight">
		...
		<div class="holder" v-else-if="!error">
			...
			<div class="ctas">
				<Button small :loading="highlighting" @click.stop="highlight()" icon="highlight">{{
					$t("global.moderation_action.clip_created_highlightBt")
				}}</Button>
				<Button
					small
					type="link"
					...
					target="_blank"
					icon="newtab"
					>{{ $t("global.moderation_action.clip_created_publishBt") }}</Button
				>
			</div>
		</div>
...
class ChatMessageClipPending extends AbstractChatMessage {
	...
	public mounted(): void {
		this.$store.accessibility.setAriaPolite(
			this.$t("global.moderation_action.clip_creating", { LINK: "" }),
		);
		...
		this.interval = window.setInterval(async () => {
			...
			if (!this.loading) {
				...
				if (this.error) {
					...
					this.$store.accessibility.setAriaPolite(
						this.$t("global.moderation_action.clip_created", { LINK: "" }),
					);
				}
			}
		}, 1000);
