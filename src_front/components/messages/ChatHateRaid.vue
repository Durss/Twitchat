	...
	<div class="chathateraid chatMessage highlight alert">
		...
		<div class="content">
			...
			<template v-else>
				<ul class="terms">
					<li
						v-for="term in messageData.terms"
						:key="term.id"
						:class="term.unblocked ? 'strike' : ''"
					>
						{{ term.text }}
					</li>
				</ul>
				...
				<ul class="users">
					<li
						v-for="user in messageData.haters"
						:key="user.id"
						:class="user.channelInfo[messageData.channel_id]?.is_banned ? 'strike' : ''"
					>
						<Icon
							name="ban"
							v-if="user.channelInfo[messageData.channel_id]?.is_banned"
						/>
			...
			<a
				class="footer"
				href="https://dashboard.twitch.tv/settings/moderation/blocked-terms"
				target="_blank"
				>{{ $t("chat.hate_raid.footer") }} <Icon name="newtab"
			/></a>
		</div>
		<div class="ctas" v-if="canUnblockTerms || canBanUsers">
			<TTButton
				v-if="canUnblockTerms"
				icon="unblock"
				@click.stop="unblockTerms()"
				:loading="unblocking"
				light
				alert
				small
				>{{ $t("chat.hate_raid.cancel_bt") }}</TTButton
			>
			<TTButton
				v-if="canBanUsers"
				icon="ban"
				@click.stop="banHaters()"
				:loading="banning"
				light
				alert
				small
				>{{ $t("chat.hate_raid.banUsers_bt") }}</TTButton
			>
		</div>
...
class ChatHateRaid extends AbstractChatMessage {
	...
	public get canBanUsers(): boolean {
		return (
			this.messageData.haters.filter(
				(v) => !v.channelInfo[this.messageData.channel_id]?.is_banned,
			).length > 0
		);
	}
	...
	public async banHaters(): Promise<void> {
		...
		for (const user of this.messageData.haters) {
			...
			await TwitchUtils.banUser(
				...
				undefined,
				"You have been banned because you took part  what appears to be a hate raid",
			);
		}
		Database.instance.updateMessage(this.messageData);
		this.banning = false;
