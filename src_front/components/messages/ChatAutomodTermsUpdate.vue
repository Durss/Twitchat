...
class ChatAutomodTermsUpdate extends AbstractChatMessage {
	...
	public get classes(): string[] {
		...
		if (
			this.messageData.action == "add_blocked" ||
			this.messageData.action == "remove_permitted"
		) {
			res.push("error");
		}
		if (
			this.messageData.action == "add_permitted" ||
			this.messageData.action == "remove_blocked"
		) {
			res.push("success");
		}
		return res;
