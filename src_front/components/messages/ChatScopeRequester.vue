<template>
	<div class="chatscoperequester chatMessage highlight error">
		...
		<div class="info">
			<p class="title">
				{{ $t("chat.scope_request.title", messageData.twitch_scopes.length) }}
			</p>
			...
			<Button
				...
				alert
				icon="lock_fit"
				@click.stop="requestScopes()"
				>{{ $t("chat.scope_request.grantBt") }}</Button
			>
			<Button class="filterBt" small light alert icon="filters" @click.stop="openFilters()">{{
				$t("chat.scope_request.openFiltersBt")
			}}</Button>
		</div>
	</div>
</template>
