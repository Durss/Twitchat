<template>
	<div :class="classes" @contextmenu="onContextMenu($event, messageData, $el)">
		...
		<div class="holder">
			<div class="message">
				<a
					:href="'https://twitch.tv/' + messageData.message.user.login"
					target="_blank"
					@click.stop.prevent="
						openUserCard(messageData.message.user, messageData.message.channel_id)
					"
					class="login"
					>{{ messageData.message.user.displayName }} :</a
				>
				<ChatMessageChunksParser
					:chunks="messageData.message.message_chunks"
					:channel="messageData.message.channel_id"
