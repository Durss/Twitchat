		...
		<div class="messageHolder">
			<i18n-t scope="global" tag="span" :keypath="labelKey">
				<template #USER>
					<template v-if="messageData.user">
						<a
							...
							target="_blank"
							@click.stop.prevent="
								openUserCard(messageData.user!, messageData.channel_id)
							"
				...
				<template #PLAYLIST v-if="messageData.playlistTarget">
					<a
						class="userlink"
						v-if="messageData.playlistTarget!.url"
						:href="messageData.playlistTarget!.url"
						target="_blank"
						>{{ messageData.playlistTarget!.title }}</a
					>
			...
			<div class="trackHolder" v-if="messageData.trackAdded">
				<div class="coverAndInfos">
					...
					<div class="trackInfo">
						...
						<span class="duration"
							><Icon name="timer" />{{
								formatDuration(messageData.trackAdded.duration)
							}}</span
						>
					</div>
				</div>
				<TTButton
					...
					icon="newtab"
					type="link"
					:href="messageData.trackAdded.url"
					target="_blank"
					v-tooltip="$t('chat.added_to_queue.open_track')"
				/>
				<TTButton
					class="cta"
					small
					icon="delete"
					v-if="messageData.skipped !== true && !messageData.playlistTarget"
					@click.stop="removeTrack()"
					alert
					v-tooltip="$t('chat.added_to_queue.skip_track')"
				/>
				<TTButton
					class="cta"
					small
					icon="ban"
					v-if="canBanFromSR && !isBanned"
					@click.stop="banFromSR()"
					secondary
					v-tooltip="$t('chat.added_to_queue.ban_user')"
				/>
				<TTButton
					class="cta"
					small
					icon="unban"
					v-if="canBanFromSR && isBanned"
					@click.stop="unBanFromSR()"
					alert
					v-tooltip="$t('chat.added_to_queue.unban_user')"
				/>
			</div>

			<div class="trackHolder" v-else>
				{{
					$t("triggers.actions.music.fail_reasons." + messageData.failCode, {
...
class ChatTrackAddedToQueue extends AbstractChatMessage {
	...
	public get labelKey(): string {
		if (this.messageData.playlistTarget) {
			return this.messageData.trackAdded
				? "chat.added_to_playlist.title"
				: "chat.added_to_playlist.title_fail";
		} else {
			return this.messageData.trackAdded
				? "chat.added_to_queue.title"
				: "chat.added_to_queue.title_fail";
		}
	}
	...
	public get canBanFromSR(): boolean {
		...
		const trigger = this.$store.triggers.triggerList.find(
			(v) => v.id === this.messageData.triggerIdSource,
		);
	...
	public get isBanned(): boolean {
		...
		return (
			(trigger.permissions.usersRefused || []).findIndex(
				(v) => v.toLowerCase() === this.messageData.user!.login.toLowerCase(),
			) > -1
		);
	}
	...
	public removeTrack(): void {
		...
		SpotifyHelper.instance.skipQueuedTrack(
			this.messageData.user,
			this.messageData.trackAdded.id,
		);
