<template>
	<div class="chatpollresult chatMessage highlight pollResult">
		...
		<div class="content">
			...
			<div class="choices">
				<div
					v-for="o in messageData.poll.choices"
					:key="o.id"
					class="choice"
...
class ChatChatPollResult extends AbstractChatMessage {
	...
	public getChoiceClasses(
		o: TwitchatDataTypes.MessageChatPollData["poll"]["choices"][number],
	): string[] {
		const res = ["outcome"];
	...
	public getChoiceStyles(o: TwitchatDataTypes.MessageChatPollData["poll"]["choices"][number]): {
		[key: string]: string;
	} {
		let totalVotes = 0;
