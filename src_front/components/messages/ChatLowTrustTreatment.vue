<template>
	<div :class="classes" @mouseover="$emit('onOverMessage', messageData, $event)">
		...
		<i18n-t
			scope="global"
			v-if="messageData.restricted"
			keypath="global.moderation_action.user_restricted"
			tag="p"
		>
			<template #USER>
				<a
					class="userlink"
					@click="openUserCard(messageData.user, messageData.channel_id)"
			...
			<template #MODERATOR>
				<a
					class="userlink"
					@click="openUserCard(messageData.moderator, messageData.channel_id)"
		...
		<i18n-t
			scope="global"
			v-else-if="messageData.monitored"
			keypath="global.moderation_action.user_monitored"
			tag="p"
		>
			<template #USER>
				<a
		...
		<i18n-t scope="global" v-else keypath="global.moderation_action.user_unflagged" tag="p">
			<template #USER>
				<a
					class="userlink"
					@click="openUserCard(messageData.user, messageData.channel_id)"
