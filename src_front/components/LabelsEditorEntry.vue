	...
	<div class="label" :class="classes" v-else @click="open($event)">
		...
		<div class="form">
			<ContentEditable
				...
				@focus="
					onEditLabel(false);
					labelValue = labelValue == defaultLabel ? '' : labelValue;
				"
				@blur="onEditLabel()"
			/>
			...
			<div class="sources" v-if="showSources">
				<div v-for="lang in $i18n.availableLocales.filter((v) => v != $i18n.locale)">
					<CountryFlag
						:country="$t('global.lang_flag', 1, { locale: lang })"
						class="flag"
					/>
					<TTButton
						transparent
						...
						@click="copyValue(getUnparsedLabel(lang) || '')"
					></TTButton>
...
class LabelsEditorEntry extends Vue {
	...
	public mounted(): void {
		if (this.path.join(".") == (this.pathToSelect || []).join(".")) {
			...
			gsap.fromTo(
				this.$el,
				{ scaleY: 1.5, filter: "brightness(4)" },
				{
					duration: 0.5,
					scaleY: 1,
					ease: "sine.in",
		...
		for (let i = 0; i < chunks.length; i++) {
			...
			if (i < chunks.length - 1) {
				...
				if (deleteMode) {
					delete messages[key];
					continue;
				} else if (this.labelValue != this.defaultLabel) {
