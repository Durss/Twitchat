<template>
	<component :is="tag" ref="elementRef" :contenteditable="computedContentEditableValue" :class="$attrs['class']"
		@input="update(false)" @blur="update(true)" @keypress="onKeypress"></component>
</template>
...
async function limitLabelSize(): Promise<void> {
	...
	if (sel && sel.rangeCount > 0) {
		...
		if (range.startContainer.firstChild) range.setStart(range.startContainer.firstChild, Math.max(0, Math.min(text.length, caretIndex - 1)));
	}
}
