...
function resetRender(): void {
	...
	if (props.openAnimaton !== false && item.value) {
		...
		gsap.from(item.value as HTMLDivElement[], {
			duration: 0.3,
			x: -50,
			stagger: 0.035,
			ease: "back.out(3)",
...
function mouseEnter(event: MouseEvent): void {
	...
	gsap.from(label, { duration: 0.25, padding: 0, margin: 0, width: 0, clearProps: "all" });
}
...
function mouseLeave(event: MouseEvent): void {
	...
	const label = target.querySelector(".label");
	gsap.killTweensOf(label);
	gsap.to(label, { duration: 0.25, padding: 0, margin: 0, width: 0 });
}
