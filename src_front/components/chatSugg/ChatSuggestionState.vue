<template>
	<div class="chatsuggstate sidePanel">
		<div class="head">
			...
			<h1 class="title">
				<Icon name="chatSugg" />{{ $t("suggestion.state_title") }}
				<mark>{{ poll.command }}</mark>
			</h1>
		</div>
		...
		<div class="content">
			<div class="card-item actions">
				...
				<TTButton
					...
					>{{ $t("suggestion.state_pickBt") }}</TTButton
				>

				<TTButton icon="cross" alert @click="closePoll()">{{
					$t("suggestion.state_closeBt")
				}}</TTButton>
			</div>
			...
			>
				<div
					:class="c.selected ? 'card-item secondary win' : 'card-item'"
				...
				>
					<div class="header">
						...
						<button class="deleteBt" v-else @click="deleteEntery(c.data)">
							<Icon name="trash" />
						</button>
						...
						<a
							...
							@click.prevent="openUserCard(c.data.user)"
							>{{ c.data.user.displayNameOriginal }}</a
						>
					</div>
					<div class="info">
...
class ChatSuggestionState extends AbstractSidePanel {
	...
	public get poll(): TwitchatDataTypes.ChatSuggestionData {
		return this.$store.chatSuggestion.data!;
	}

	public get entries(): {
		data: TwitchatDataTypes.ChatSuggestionDataChoice;
		selected: boolean;
	}[] {
		let list = this.poll.winners.map((v) => {
			return { data: v, selected: true };
		});
		list = list.concat(
			this.poll.choices.map((v) => {
				return { data: v, selected: false };
			}),
		);
		return list;
	}
			...
			if (data.winners && data.winners.length > 0) {
				...
				if (index > -1) {
					...
					if (entry) {
						...
						//Scroll back to top
						const doneList = (
							this.$refs["selected"] as ComponentPublicInstance[] | undefined
						)?.map((v) => v.$el);
	...
	public closePoll(): void {
		this.$confirm(
			this.$t("suggestion.state_close_confirm_title"),
