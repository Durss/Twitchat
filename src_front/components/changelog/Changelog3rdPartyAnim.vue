<template>
	<div class="changelog3rdpartyanim">
		...
		<TTButton primary ref="target" big icon="offline">{{
			$t("params.categories.connexions")
		}}</TTButton>
		<template v-for="(item, index) in categories">
			<Transition name="fadein">
				<TTButton v-if="childIndex > index" class="childItem" :icon="item.icon">{{
					item.label
				}}</TTButton>
			</Transition>
			<TTButton v-if="childIndex <= index" class="childItem hidden" :icon="item.icon">{{
				item.label
			}}</TTButton>
		</template>
	</div>
...
class Changelog3rdPartyAnim extends Vue {
	...
	public mounted(): void {
		const items = (this.$refs.item as ComponentPublicInstance[]).map(
			(i) => i.$el as HTMLElement,
		);
		const target = (this.$refs.target as ComponentPublicInstance).$el as HTMLElement;

		gsap.from(target, { duration: 0.5, scale: 0, ease: "back.out", delay: 0.1 });
		...
		window.setTimeout(async () => {
			for (let i = 0; i < items.length; i++) {
				...
				gsap.fromTo(el, { opacity: 0 }, { duration: 0.1, opacity: 1 });
				gsap.fromTo(
					el,
					{ x, y },
					{
						duration,
						x: tX - bounds.width / 2,
						y: tY - bounds.height / 2,
						ease: "sine.easeIn",
					},
				);
				const delayHide = duration * 0.8;
				gsap.to(el, {
					delay: delayHide,
				...
				gsap.delayedCall(delayHide, () => {
					...
					gsap.fromTo(
						target,
						{ scaleX: 1.5 },
						{ scaleX: 1, ease: "elastic.out", duration: 0.75 },
					);
					gsap.fromTo(
						target,
						{ scaleY: 2 },
						{ scaleY: 1, ease: "elastic.out", duration: 0.75, delay: 0.1 },
					);
				});
			}
		}, 50);
