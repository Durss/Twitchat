<template>
	<div
		class="chatalertmessage"
		@click="$store.main.executeChatAlert(null)"
		v-if="message"
		v-tooltip="$t('global.close')"
	>
		<div class="user">
			{{ $t("global.chat_alert_title", { USER: message.user.displayName }) }}
		</div>
		<div class="message">
...
class ChatAlertMessage extends Vue {
	...
	public get chunks(): TwitchatDataTypes.ParseMessageChunk[] {
		...
		const cmd = this.$store.main.chatAlertParams.chatCmd
			.trim()
			.toLowerCase()
			.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");
	...
	public mounted(): void {
		watch(
			...
			async (message) => {
				if (
					message &&
					this.$store.main.chatAlertParams.message === true &&
