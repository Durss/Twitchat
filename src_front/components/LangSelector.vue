	...
	<div class="langselector">
		...
		>
			<template v-slot:option="option">
				<CountryFlag
					:country="getISOFromLang(option.value[1][0])"
					size="small"
					class="flag"
				/>
...
const languages = computed(() => {
	return Languages.map((v) => {
		return { label: v[0] as string, value: v };
	});
});
...
const subLanguages = computed(() => {
	...
	return sublangs.map((v) => {
		return { label: v[1] as string, value: v };
	});
});
...
function onChange(resetSubList: boolean = false): void {
	...
	if (!langLocal.value) {
		emit("update:lang", "");
	} else if (!sublangLocal.value.label) {
