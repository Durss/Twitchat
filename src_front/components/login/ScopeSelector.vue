...
class ScopeSelector extends Vue {
	...
	public params_all: TwitchatDataTypes.ParameterData<boolean> = {
		type: "boolean",
		value: false,
	...
	public async expandList(): Promise<void> {
		...
		gsap.from(this.$refs.permsList as HTMLDivElement, {
			height: "1.5em",
			duration: 0.5,
			ease: "sine.inOut",
			clearProps: "all",
	...
	private buildList(): void {
		...
		for (const scope of scopes) {
			...
			if (requested.length > 0) {
				...
				const selected =
					forceSelect || disabled ? true : TwitchUtils.hasScopes(localScopes);
