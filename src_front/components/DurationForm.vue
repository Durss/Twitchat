...
function onKeyDown(event: KeyboardEvent, field: "d" | "h" | "m" | "s"): void {
	if (event.key == "ArrowUp" || event.key == "ArrowDown") {
		...
		if (add != 0) {
			switch (field) {
				case "d":
					days.value = (parseInt(days.value) + add).toString();
					break;
				case "h":
					hours.value = (parseInt(hours.value) + add).toString();
					break;
				case "m":
					minutes.value = (parseInt(minutes.value) + add).toString();
					break;
				case "s":
					seconds.value = (f(seconds.value) + add).toString();
					break;
			}
			clamp(field);
			onChange();
	...
	if (event.key == "ArrowRight" || event.key == "ArrowLeft") {
		...
		if (sel && sel.rangeCount > 0) {
			...
			let inputs = [inputD.value, inputH.value, inputM.value, inputS.value].filter(
				(v) => v && v.$el != undefined,
			);
			if (
				(dir == 1 && caretIndex == input.innerText.length) ||
				(dir == -1 && caretIndex == 0)
			) {
...
function clamp(field: "d" | "h" | "m" | "s"): void {
	...
	switch (field) {
		case "d":
			days.value = Utils.toDigits(loop(parseInt(days.value), 999), 1).toString();
			break;
		case "h":
			hours.value = Utils.toDigits(loop(parseInt(hours.value), 24), 2).toString();
			break;
		case "m":
			minutes.value = Utils.toDigits(loop(parseInt(minutes.value), 59), 2).toString();
			break;
		case "s":
			seconds.value = Utils.toDigits(loop(f(seconds.value), 59), 2).toString();
			break;
	}
}
