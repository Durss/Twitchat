(function() {
	//#region node_modules/toygrad/dist/buffer.js
	var Buffer$1 = class extends Float32Array {};
	//#endregion
	//#region node_modules/toygrad/dist/encoder.js
	...
	const Encoder = {
		encode: (buffer, precision) => {
			...
			if (precision === "f32") {
				...
				let bytes = "";
				for (let i = 0, l = buffer.length; i < l; i += 1) bytes += String.fromCharCode(Math.trunc(buffer[i] * scale + 127));
				...
				if (max > 127 || scale < 1) throw new Error("Unsupported encoding, max value out of range");
				let bytes = "";
				for (let i = 0, l = buffer.length; i < l; i += 1) bytes += ALPHABET_86[Math.round(Math.trunc(buffer[i] * scale + 127) / 3)];
				return `1-${scale}|${bytes}`;
			} else throw new Error("Unsupported precision");
		},
		decode: (encoded) => {
			...
			if (precision === 4) {
				...
				for (let i = 0, l = bytesChunkLength; i < l; i += 1) buffer[i] = (bytes[i].charCodeAt(0) - 127) / scale;
			} else if (precision === 1 && isF6) {
				const scale = Number(encoded.slice(2, separatorIndex));
				for (let i = 0, l = bytesChunkLength; i < l; i += 1) buffer[i] = (IALPHABET_86[bytes[i]] * 3 - 127) / scale;
			} else throw new Error("Unsupported precision");
			return buffer;
		}
	};
	//#endregion
	//#region node_modules/toygrad/dist/layers/abstract.js
	var Abstract = class {
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/abstract_hidden.js
	var AbstractHidden = class extends Abstract {};
	//#endregion
	//#region node_modules/toygrad/dist/layers/abstract_activation.js
	var AbstractActivation = class extends AbstractHidden {};
	//#endregion
	//#region node_modules/toygrad/dist/layers/abstract_input.js
	var AbstractInput = class extends Abstract {};
	//#endregion
	//#region node_modules/toygrad/dist/layers/abstract_output.js
	var AbstractOutput = class extends Abstract {};
	//#endregion
	//#region node_modules/toygrad/dist/utils.js
	...
	//#endregion
	//#region node_modules/toygrad/dist/tensor.js
	var Tensor$1 = class Tensor$1 {
		constructor(sx, sy, sz, value, dvalue) {
			...
			if (isUndefined$1(value)) {
				...
				if (value !== 0) this.w.fill(value);
			} else if (isBuffer$1(value)) this.w = value;
			else this.w = new Buffer$1(value);
		...
		clone() {
			const clone = new Tensor$1(this.sx, this.sy, this.sz, this.w);
			clone.w = clone.w.slice();
			return clone;
		}
		cloneWithZeros() {
			return new Tensor$1(this.sx, this.sy, this.sz, 0);
		}
	};
	//#endregion
	//#region node_modules/toygrad/dist/layers/conv.js
	var Conv = class extends AbstractHidden {
		constructor(options, prev) {
			super(options, prev);
			this.sx = options.sx;
		...
		forward(input, isTraining) {
			...
			for (let d = 0; d < this.osz; d++) {
				...
				let x = -this.pad;
				let y = -this.pad;
				for (let ay = 0; ay < this.osy; y += xy_stride, ay++) {
					x = -this.pad;
					for (let ax = 0; ax < this.osx; x += xy_stride, ax++) {
						let a = 0;
						for (let fy = 0; fy < f.sy; fy++) {
							let oy = y + fy;
							for (let fx = 0; fx < f.sx; fx++) {
								let ox = x + fx;
								if (oy >= 0 && oy < V_sy && ox >= 0 && ox < V_sx) for (let fd = 0; fd < f.sz; fd++) a += f.w[(f.sx * fy + fx) * f.sz + fd] * input.w[(V_sx * oy + ox) * input.sz + fd];
							}
						}
						a += this.biases.w[d];
				...
				for (let ay = 0; ay < this.osy; y += xy_stride, ay++) {
					...
					for (let ax = 0; ax < this.osx; x += xy_stride, ax++) {
						...
						for (let fy = 0; fy < f.sy; fy++) {
							...
							for (let fx = 0; fx < f.sx; fx++) {
								let ox = x + fx;
								if (oy >= 0 && oy < V_sy && ox >= 0 && ox < V_sx) for (let fd = 0; fd < f.sz; fd++) {
						...
						if (biased) this.biases.dw[d] += chain_grad;
					}
				}
			}
		}
		getAsOptions(precision) {
			return {
				...this.options,
				_biases: this.biased ? Encoder.encode(this.biases.w, precision) : void 0,
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/dense.js
	var Dense = class extends AbstractHidden {
		constructor(options, prev) {
			super(options, prev);
			this.osx = 1;
			this.osy = 1;
		...
		forward(input, isTraining) {
			...
			for (let i = 0, l = this.osz; i < l; i++) {
				...
				for (let d = 0; d < this.il; d++) a += input.w[d] * wi[d];
				a += this.biases.w[i];
				output.w[i] = a;
			}
			this.ot = output;
		...
		backward() {
			...
			for (let i = 0, l = this.osz; i < l; i++) {
				...
				if (biased) this.biases.dw[i] += chain_grad;
			}
		}
		getAsOptions(precision) {
			return {
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/dropout.js
	var Dropout = class extends AbstractHidden {
		constructor(options, prev) {
			super(options, prev);
			this.probability = options.probability ?? .5;
			this.dropped = new Buffer$1(this.osx * this.osy * this.osz);
		}
		forward(input, isTraining) {
			this.it = input;
			const output = input.clone();
			if (isTraining) for (let i = 0; i < input.length; i++) if (Math.random() < this.probability) {
				output.w[i] = 0;
				this.dropped[i] = 1;
			} else this.dropped[i] = 0;
			else for (let i = 0; i < input.length; i++) output.w[i] *= this.probability;
			this.ot = output;
			return this.ot;
		}
		backward() {
			...
			input.dw = new Buffer$1(input.length);
			for (let i = 0, l = input.length; i < l; i++) if (!this.dropped[i]) input.dw[i] = output.dw[i];
		}
	};
	//#endregion
	//#region node_modules/toygrad/dist/layers/input.js
	var Input = class extends AbstractInput {
		constructor(options, prev) {
			super(options, prev);
			this.osx = options.sx;
			this.osy = options.sy;
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/abstract_activation_elementwise.js
	var AbstractActivationElementwise = class extends AbstractActivation {
		activationForward(x) {
		...
		forward(input, isTraining) {
			this.it = input;
			const output = input.clone();
			for (let i = 0, l = input.length; i < l; i++) output.w[i] = this.activationForward(output.w[i]);
			this.ot = output;
			return this.ot;
		}
		backward() {
			...
			const output = this.ot;
			input.dw = new Buffer$1(input.length);
			for (let i = 0, l = input.length; i < l; i++) input.dw[i] = this.activationBackward(output.w[i], output.dw[i]);
		}
	};
	//#endregion
	//#region node_modules/toygrad/dist/layers/leakyrelu.js
	var LeakyRelu = class extends AbstractActivationElementwise {
		activationForward(x) {
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/maxout.js
	var Maxout = class extends AbstractHidden {
		constructor(options, prev) {
			super(options, prev);
		...
		forward(input, isTraining) {
			...
			const N = this.osz;
			const output = new Tensor$1(this.osx, this.osy, this.osz, 0);
			if (this.osx === 1 && this.osy === 1) for (let i = 0; i < N; i++) {
				let ix = i * this.sx;
				let a = input.w[ix];
				let ai = 0;
				...
				let n = 0;
				for (let x = 0; x < input.sx; x++) for (let y = 0; y < input.sy; y++) for (let i = 0; i < N; i++) {
					let ix = i * this.sx;
		...
		backward() {
			...
			const output = this.ot;
			input.dw = new Buffer$1(input.length);
			if (this.osx === 1 && this.osy === 1) for (let i = 0; i < this.osz; i++) {
				let chain_grad = output.dw[i];
				input.dw[this.switches[i]] = chain_grad;
			}
			else {
				let n = 0;
				for (let x = 0; x < output.sx; x++) for (let y = 0; y < output.sy; y++) for (let i = 0; i < this.osz; i++) {
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/pool.js
	var Pool = class extends AbstractHidden {
		constructor(options, prev) {
			super(options, prev);
			this.sx = options.sx;
			this.sy = options.sy ?? this.sx;
			...
			for (let d = 0; d < this.osz; d++) {
				...
				for (let ax = 0; ax < this.osx; x += this.stride, ax++) {
					...
					for (let ay = 0; ay < this.osy; y += this.stride, ay++) {
						let a = -99999;
						let winx = -1, winy = -1;
						for (let fx = 0; fx < this.sx; fx++) for (let fy = 0; fy < this.sy; fy++) {
							let oy = y + fy;
							let ox = x + fx;
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/regression.js
	var Regression = class extends AbstractOutput {
		constructor(options, prev) {
			super(options, prev);
			this.osx = 1;
			this.osy = 1;
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/relu.js
	var Relu = class extends AbstractActivationElementwise {
		activationForward(x) {
			return x <= 0 ? 0 : x;
		}
		activationBackward(x, dx) {
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/sigmoid.js
	var Sigmoid = class extends AbstractActivationElementwise {
		activationForward(x) {
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/softmax.js
	var Softmax = class extends AbstractOutput {
		constructor(options, prev) {
			super(options, prev);
			this.osx = 1;
			this.osy = 1;
		...
		forward(input, isTraining) {
			...
			let as = input.w;
			let amax = input.w[0];
			for (let i = 1, l = this.osz; i < l; i++) if (as[i] > amax) amax = as[i];
		...
		backward(output) {
			...
			input.dw = new Buffer$1(input.length);
			for (let i = 0, l = this.osz; i < l; i++) {
				const mul = -((i === output ? 1 : 0) - this.es[i]);
				input.dw[i] = mul;
			}
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/tanh.js
	var Tanh = class extends AbstractActivationElementwise {
		activationForward(x) {
			return Math.tanh(x);
		}
		activationBackward(x, dx) {
	...
	//#endregion
	//#region node_modules/toygrad/dist/layers/index.js
	const Layers = {
		Abstract,
		AbstractActivation,
		AbstractHidden,
	...
	//#endregion
	//#region node_modules/toygrad/dist/neural_network.js
	var NeuralNetwork = class {
		constructor(options) {
			this.layers = this.resolve(options.layers);
			this.options = options;
		...
		forward(input, isTraining) {
			...
			for (let i = 1, l = this.layers.length; i < l; i++) result = this.layers[i].forward(result, isTraining);
			return result;
		}
		backward(output) {
			const loss = this.layers[this.layers.length - 1].backward(output);
			for (let i = this.layers.length - 2; i >= 1; i--) this.layers[i].backward(output);
			return loss;
		}
		resolve(layers) {
		...
		getAsOptions(precision = "f32") {
			return { layers: this.layers.map((layer) => layer.getAsOptions(precision)) };
		}
		getParamsAndGrads() {
	...
	//#endregion
	//#region node_modules/lande/standalone/t50-langs.js
	var t50_langs_default = [
		"afr",
		"ara",
		"aze",
		"bel",
	...
	//#endregion
	//#region node_modules/lande/standalone/t50-ngrams.js
	var t50_ngrams_default = {
		"unigrams": [
			" ",
			"ا",
			"a",
	...
	//#endregion
	//#region node_modules/lande/standalone/t50-options.js
	var t50_options_default = { "layers": [
		{
			"type": "input",
			"sx": 1,
			"sy": 1,
		...
		{ "type": "relu" },
		{
			"type": "dense",
			"filters": 50,
			"_biases": "119.2|CBIJGQ4TBgwTFw8KDAcBBwoMAQ0cDAgWAgsJFxEbEAodFw4QFggJBwcSDQkWBwIKDgk=",
				"112.7|EiI7Dy8qP1A+MUcyJzwnPC8qKzA+Ii8xORYAECgbIiwuQyc1PjIiTTgwRS1DOiswNyANLzs6MDoeUhcuTDg4PkFAOTc1NDYiKzolNj0vBgM8GTIgPRxTMiM0Eh81KDImPC4uKyc0NS4qQDU3KypJIDM0PSkfIzo2",
				"111.6|OTE8MS8xQRpIK0k/LDYyHkU9S0Y3LzwyLj9HMz1GRRsnNyosSDBLPDVDRT5INyw5OC5AOTwbLCZFLjcpOE44Q0kuLy87NUE2OTwjNRo+TSEqPDk4HDhBOj47RkIpPTZPUDofTCg0Lkc+Lyw8OyoBQEJCHho3Mz86",
				"113.9|KTIsPDEwCioeLA8RFzUwHhcsFSQoDh4uJwcaNjAUGjYnAxoxNzQPJzA1RCcaLjIpMBgyJi8SIikbIQomNjUoHTAvPxwmJRAxHSg6JDUcEzomJhctGyYCPSooHjY9LCYcBSUeOC4XKS0iEiAjGx83LCQUASYiHiAk",
				"113.8|DjgvGzULEEAcIyQaNTUqGBkpLS0HJyspGh4gNyo1BS0oHCANJCM5UwIcQhwTJQ8rMS8NOTE4OzAeKhgrUhEhIyo5NxYqKgoyMC8tKj8rKCgfJSw3DRIzJCsnLjVHGSoHAS4kRytVKS0jCysyCSoTGSsqRxMvCywh",
				"111.7|OUMgRzQ6RzQkN1oaQTs8Jj06UzdAQjgyQUA1QEMuOz4yOT4iTDcgWCciTi1KLx03JDkqJzYUQT09VDQ5TykuPS43KTMsMTsyPB0xLz06AVI8OzcuRCBVLjYyL0IzNjVCQi82DEA+Nx0/KjE4MzdKJzwkSz88PSow",
				"18.8|T1VFWUJKU05OLT9ULkA5VVBOSUpXUkc1WVNLSk9IUB5CSE9LUkVLOFNNS0ZKQ0BHVFNOUFBXPldOR0ktUk82TD9YTU1KQFJSP1AzLjZES2A3UE5OWD1NLUlMUEpOTkVQSVAAS1c9N1BLR0BRRjdqREVQa0BSUUZD",
				"112.6|MDMtPDspUUIjJDFEQjkuPywpRT42RDkjPEQuOjE8NTYlPSApWCxOSiNPVS9DMTorNCY1MxhCSDwZS0AvVDY9LCJHOxgkLSg2Hi9NGjknT04ANRxBQkJDTyEeUj5KHiQuFTIcSUVSKjFTQCk+LiskTDIxVDs7OxUj"
			]
		},
		{ "type": "softmax" }
	] };
	//#endregion
	//#region node_modules/lande/standalone/utils.js
	...
	var Tensor = class {
		constructor(sx, sy, sz, value, dvalue) {
			...
			if (isUndefined(value)) {
				...
				if (value !== 0) this.w.fill(value);
			} else if (isBuffer(value)) this.w = value;
			else this.w = new buffer_default(value);
	...
	var getNgrams = (str, length) => {
		...
		for (let value in ngrams) ngrams[value].frequency = ngrams[value].count / total;
		return ngrams;
	};
	var getNormalized = (str) => {
		return ` ${str.replace(/-+/g, " ").replace(/[^\p{L}\p{M}\s]/gu, "").replace(/\s{2,}/g, " ").toLowerCase().trim()} `;
	};
	var infer = (text, langs, ngrams, nn) => {
		...
		return Array.from(output).map((probability, index) => [langs[index], probability]).sort((a, b) => b[1] - a[1]);
	};
	//#endregion
	//#region node_modules/lande/standalone/t50.js
	const nn = new NeuralNetwork(t50_options_default);
	const lande = (text) => infer(text, t50_langs_default, t50_ngrams_default, nn);
	//#endregion
	//#region src_lande/lande_worker.ts
