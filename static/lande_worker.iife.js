(function () {
	...
	var Conv = class extends AbstractHidden {
		constructor(options, prev) {
			...
			this.biases = this.biased
				? options._biases
					? new Tensor$1(1, 1, this.osz, Encoder.decode(options._biases))
					: new Tensor$1(1, 1, this.osz, this.bias)
				: new Tensor$1(1, 1, this.osz, 0);
			this.filters = options._filters
				? options._filters.map(
						(filter) =>
							new Tensor$1(this.sx, this.sy, this.isz, Encoder.decode(filter)),
					)
				: range(0, this.osz).map(() => new Tensor$1(this.sx, this.sy, this.isz));
		}
		forward(input, isTraining) {
			this.it = input;
							...
							for (let fx = 0; fx < f.sx; fx++) {
								...
								if (oy >= 0 && oy < V_sy && ox >= 0 && ox < V_sx)
									for (let fd = 0; fd < f.sz; fd++)
										a +=
											f.w[(f.sx * fy + fx) * f.sz + fd] *
											input.w[(V_sx * oy + ox) * input.sz + fd];
							}
						}
						a += this.biases.w[d];
	...
	var Dense = class extends AbstractHidden {
		constructor(options, prev) {
			...
			this.filters = options._filters
				? options._filters.map(
						(filter) => new Tensor$1(1, 1, this.il, Encoder.decode(filter)),
					)
				: range(0, this.osz).map(() => new Tensor$1(1, 1, this.il));
		}
		forward(input, isTraining) {
			this.it = input;
	...
	var Dropout = class extends AbstractHidden {
		...
		backward() {
			...
			for (let i = 0, l = input.length; i < l; i++)
				if (!this.dropped[i]) input.dw[i] = output.dw[i];
		}
	};
	//#endregion
	...
	var NeuralNetwork = class {
		...
		resolve(layers) {
			const resolveLayer = (layer, prev) => {
				...
				switch (layer.type) {
					case "input":
						return new Layers.Input(layer, prev);
					case "conv":
						return new Layers.Conv(layer, prev);
					case "dense":
						return new Layers.Dense(layer, prev);
					case "dropout":
						return new Layers.Dropout(layer, prev);
					case "pool":
						return new Layers.Pool(layer, prev);
					case "leakyrelu":
						return new Layers.LeakyRelu(layer, prev);
					case "maxout":
						return new Layers.Maxout(layer, prev);
					case "relu":
						return new Layers.Relu(layer, prev);
					case "sigmoid":
						return new Layers.Sigmoid(layer, prev);
					case "tanh":
						return new Layers.Tanh(layer, prev);
					case "regression":
						return new Layers.Regression(layer, prev);
					case "softmax":
						return new Layers.Softmax(layer, prev);
					default:
	...
	var t50_options_default = {
		layers: [
			...
			},
			{
				...
				bias: 0.1,
				_biases:
					"154.6|yJFeZ6rIc4iLuoN/dWiYnKWRU5uFlluhTpann9h4qXPTs9KcmsCFW4OAhvyN1oahz92dj220g7e9kLegbZB7q3StxISGfX15eNRmllK/eXZm1IhnmFqRkNV1gZJ0lceGhMyZkr1W27aEc8bbTa95d3CehKm9m3uh",
				_filters: [
			...
			{ type: "relu" },
			{
				...
				filters: 50,
				_biases:
					"119.2|CBIJGQ4TBgwTFw8KDAcBBwoMAQ0cDAgWAgsJFxEbEAodFw4QFggJBwcSDQkWBwIKDgk=",
				_filters: [
	...
	var infer = (text, langs, ngrams, nn) => {
		...
		return Array.from(output)
			.map((probability, index) => [langs[index], probability])
			.sort((a, b) => b[1] - a[1]);
	};
	//#endregion
