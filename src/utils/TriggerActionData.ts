...
export const TriggerEvents:TriggerEventTypes[] = [
	...
	{category:TriggerEventTypeCategories.MOD, icon:"timeout", label:"User timed out", value:TriggerTypes.TIMEOUT, description:"Execute an action when a user is <mark>/timeout</mark>", jsonTest:{ type:"timeout", duration:60, user:"Durss"}},
	{category:TriggerEventTypeCategories.MOD, icon:"ban", label:"User banned", value:TriggerTypes.BAN, description:"Execute an action when a user is <mark>/ban</mark>", jsonTest:{ type:"ban", user:"Durss"}},
	{category:TriggerEventTypeCategories.MOD, icon:"unban", label:"User unbanned", value:TriggerTypes.UNBAN, description:"Execute an action when a user is <mark>/unban</mark>", jsonTest:{ type:"unban", user:"Durss"}},
	{category:TriggerEventTypeCategories.MOD, icon:"vip", label:"User /vip", value:TriggerTypes.VIP, description:"Execute an action when a user is added to your VIPs", jsonTest:{ type:"vip", user:"Durss"}},
	{category:TriggerEventTypeCategories.MOD, icon:"unvip", label:"User /unvip", value:TriggerTypes.UNVIP, description:"Execute an action when a user is removed from your VIPs <i>(only works when using <mark>/unvip</mark> command from twitchat</i>", jsonTest:{ type:"unvip", user:"Durss"}},
	{category:TriggerEventTypeCategories.MOD, icon:"mod", label:"User /mod", value:TriggerTypes.MOD, description:"Execute an action when a user is added to your mods", jsonTest:{ type:"mod", user:"Durss"}},
	{category:TriggerEventTypeCategories.MOD, icon:"unmod", label:"User /unmod", value:TriggerTypes.UNMOD, description:"Execute an action when a user is removed from your mods <i>(only works when using <mark>/unmod</mark> command from twitchat</i>", jsonTest:{ type:"unmod", user:"Durss"}},
	...
	{category:TriggerEventTypeCategories.TIMER, icon:"countdown", label:"Countdown stop", value:TriggerTypes.COUNTDOWN_STOP, description:"Execute an action when a countdown completes or is stoped", jsonTest:{"type":"countdown","started":false,"data":{"timeoutRef":182,"startAt":1661864989574,"duration":600000},"tags":{"id":"00000000-0000-0000-0000-000000000003","tmi-sent-ts":"1661864989574"},"start":"30/08/2022 15h09","start_ms":1661864989574,"duration":"10:00","duration_ms":600000}},
	{category:TriggerEventTypeCategories.TWITCHAT, icon:"shoutout", label:"Shoutout", value:TriggerTypes.SHOUTOUT, description:"Execute an action when doing a shoutout via <mark>/so</mark> command or shoutout button", jsonTest:{"type":"shoutout","user":{"id":"29961813","login":"durss","display_name":"Durss","type":"","broadcaster_type":"affiliate","description":"Je conçois et fabrique des casse-têtes en bois plus ou moins stylés et je code des applications/outils plus ou moins stupides ou utiles. Si tu stream sur Twitch, j'ai entièrement recodé le tchat officiel pour ajouter plein de features, ça devrait t'intéresser:https://twitchat.fr","profile_image_url":"https://static-cdn.jtvnw.net/jtv_user_pictures/1835e681-7306-49b8-a1e2-2775a17424ae-profile_image-300x300.png","offline_image_url":"https://static-cdn.jtvnw.net/jtv_user_pictures/c43305dd-d577-4369-b60b-df0a4acdb7d8-channel_offline_image-1920x1080.jpeg","view_count":15289,"created_at":"2012-04-21T23:01:18Z"},"stream":{"broadcaster_id":"29961813","broadcaster_login":"durss","broadcaster_name":"Durss","broadcaster_language":"fr","game_id":"509673","game_name":"Makers & Crafting","title":"test","delay":0}}},
	{category:TriggerEventTypeCategories.TWITCHAT, icon:"emergency", label:"Emergency start", value:TriggerTypes.EMERGENCY_MODE_START, description:"Execute an action when enabling the emergency mode", jsonTest:{ type: "emergencyMode", enabled: true}},
	{category:TriggerEventTypeCategories.TWITCHAT, icon:"emergency", label:"Emergency stop", value:TriggerTypes.EMERGENCY_MODE_STOP, description:"Execute an action when stopping the emergency mode", jsonTest:{ type: "emergencyMode", enabled: false}},
