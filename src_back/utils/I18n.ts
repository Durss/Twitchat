...
export default class I18n {
	...
	public initialize(): void {
		if (fs.existsSync(path.join(Config.PUBLIC_ROOT, "labels.json"))) {
			this._labels = JSON.parse(
				fs.readFileSync(path.join(Config.PUBLIC_ROOT, "labels.json"), "utf-8"),
			) as Labels;
		} else {
			if (fs.existsSync(path.join("../../static/", "labels.json"))) {
				this._labels = JSON.parse(
					fs.readFileSync(path.join("../../static/", "labels.json"), "utf-8"),
				) as Labels;
			}
		}
	}
	 ...
	 */
	public get(
		lang: string | keyof typeof Labels,
		path: Path<Labels.en>,
	...
	public exists(
		lang: string | keyof typeof Labels,
		path: Path<typeof Labels.en>,
		_replaces?: { [key: string]: string },
	): boolean {
		const keys = path.split(".");
		let root: any = this._labels[lang];
		try {
