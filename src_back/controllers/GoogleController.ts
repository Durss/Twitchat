...
export default class GoogleController extends AbstractController {
	...
	public async initialize(): Promise<void> {
		this.server.get(
			"/api/youtube/oauthURL",
			async (request, response) => await this.getYoutubeOauthURL(request, response),
		);
		this.server.get(
			"/api/google/translate",
			async (request, response) => await this.getTranslation(request, response),
		);
		this.server.post(
			"/api/youtube/authenticate",
			async (request, response) => await this.postYoutubeAuthenticate(request, response),
		);
		this.server.post(
			"/api/youtube/refreshtoken",
			async (request, response) => await this.postYoutubeRefreshToken(request, response),
		);
		...
		if (Config.credentials.google_key) {
			...
			const json = JSON.parse(
				readFileSync(Config.CREDENTIALS_ROOT + Config.credentials.google_key, "utf-8"),
			);
	 ...
	 * @param response
	 */
	private async getYoutubeOauthURL(
		...
		if (!credentials) {
			...
			response.send(
				JSON.stringify({
					success: false,
					error: "youtube credentials are missing",
		...
		if (!credentials.redirect_uris.includes(redirectURI)) {
			...
			response.send(
				JSON.stringify({
					success: false,
					error: "Given redirect URI is invalid",
	 ...
	 * @param response
	 */
	private async postYoutubeAuthenticate(
		request: FastifyRequest,
		response: FastifyReply,
	): Promise<void> {
		if (!this.premiumGuard(request, response)) return;
	 ...
	 * @param response
	 */
	private async postYoutubeRefreshToken(
		request: FastifyRequest,
		response: FastifyReply,
	): Promise<void> {
		if (!this.premiumGuard(request, response)) return;
	...
	private async getTranslation(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		if (params.langSource == params.langTarget) {
			Logger.info(
				"Translation impossible because of same language target and source:",
				params.langSource,
			);
			response.header("Content-Type", "application/json");
			response.status(204);
		...
		if (
			this._allowedLanguagesMap[params.langSource] !== true ||
			this._allowedLanguagesMap[params.langTarget] !== true
		) {
			Logger.info(
				"Translation impossible because of unsupported language " +
					params.langSource +
					" or " +
					params.langTarget,
			);
