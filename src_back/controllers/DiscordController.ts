...
export default class DiscordController extends AbstractController {
	...
	public initialize(): DiscordController {
		this.server.get(
			"/api/discord/link",
			async (request, response) => await this.getLinkedState(request, response),
		);
		this.server.get(
			"/api/discord/code",
			async (request, response) => await this.registerCode(request, response),
		);
		this.server.get(
			"/api/discord/channels",
			async (request, response) => await this.getChannelList(request, response),
		);
		this.server.post(
			"/api/discord/image",
			async (request, response) => await this.postAnImageToDiscord(request, response),
		);
		this.server.post(
			"/api/discord/code",
			async (request, response) => await this.registerCode(request, response),
		...
		this.server.post(
			"/api/discord/answer",
			async (request, response) => await this.postAnswer(request, response),
		);
		this.server.post(
		...
		this.server.post(
			"/api/discord/message",
			async (request, response) => await this.postMessage(request, response),
		);
		this.server.post(
			"/api/discord/thread",
			async (request, response) => await this.postThread(request, response),
		);
		this.server.post(
			"/api/discord/commands",
			async (request, response) => await this.postCommands(request, response),
		);
		this.server.post(
			"/api/discord/ticket",
			async (request, response) => await this.postTicket(request, response),
		);
		this.server.delete(
			"/api/discord/link",
			async (request, response) => await this.deleteLinkState(request, response),
		);
	...
	private async postMessage(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		try {
			...
			const channels = (await this._rest.get(Routes.guildChannels(guard.guild.guildID))) as {
				id: string;
				name: string;
			}[];
			...
			console.log(
				guard.user.login + " misconfigured Discord. Cannot post to " + channel?.name,
			);
			...
			response
				...
				.send(
					JSON.stringify({
						message: "Failed posting message to Discord",
						errorCode,
	...
	private async postThread(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		try {
			...
			messageSent = true;
			const thread = (await this._rest.post(Routes.threads(params.channelId, message.id), {
				body,
			})) as { id: string };
			...
			response
				...
				.send(
					JSON.stringify({
						message: "Failed posting message to Discord",
						errorCode:
							messageSent && !threadCreated ? "CREATE_THREAD_FAILED" : "POST_FAILED",
						channelName: channel ? channel.name : "",
						success: false,
					}),
				);
	...
	private async postCommands(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		try {
			const body = request.body as {
				commands: { name: string; params: { name: string }[] }[];
			};
			...
			await this._rest.put(
				Routes.applicationGuildCommands(
					Config.credentials.discord_client_id,
					guild.guildID,
				),
				{ body: commandList },
			);
			...
			response
				...
				.send(
					JSON.stringify({
						message: "Failed creating Discord guild command",
						errorCode: "GUILD_COMMAND_CREATE_FAILED",
						success: false,
					}),
				);
			return;
	...
	private async postTicket(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		try {
			...
			const messageLink =
				"https://discord.com/channels/" + guard.guild.guildID + "/" + thread.id;
	...
	private async getChannelList(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		const res: GuildChannel[] = (await this._rest.get(
			Routes.guildChannels(guild.guildID),
		)) as GuildChannel[];
	 ...
	 */
	private async postAnImageToDiscord(
...
* **__Platform__**: ${json.messagePlatform}`;
			...
			if (typedError.code == 50001) {
				const channels = (await this._rest.get(Routes.guildChannels(guild.guildID))) as {
					id: string;
					name: string;
				}[];
		...
		if (data.reaction) {
			await this._rest.put(
				Routes.channelMessageOwnReaction(
					data.channelId,
					data.messageId,
					encodeURIComponent(data.reaction),
				),
			);
		} else {
	...
	private async postInteraction(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		const verified = verifyKey(
			...
			signature,
			timestamp,
			Config.credentials.discord_public_key,
		);
		if (!verified) {
		...
		switch (json.type) {
			...
			case InteractionType.APPLICATION_COMMAND: {
				...
				Logger.info(
					command.member.user.username + " executes command " + command.data.name,
				);
	...
	private initDatabase(): void {
		...
		if (!fs.existsSync(Config.discord2Twitch)) {
			...
			try {
				...
				const d = new Date();
				const suffix =
					"_" + d.getFullYear() + "-" + d.getMonth() + "-" + d.getDate() + "_backup";
				fs.copyFileSync(
					Config.discord2Twitch,
					Config.discord2Twitch.replace(".json", suffix + ".json"),
				);
	...
	private async createCommands(): Promise<void> {
		...
		LINK_CMD.setName(cmd)
			...
			.addStringOption((option) => {
				...
				option
					.setDescription(
						I18n.instance.get("en", "server.discord.commands.link.option_channel"),
					)
					.setRequired(true);
				...
				languages.forEach((lang) => {
					option.setDescriptionLocalization(
						...
						I18n.instance.get(
							lang.labels,
							"server.discord.commands.link.option_channel",
						),
					);
				});
				return option;
		...
		languages.forEach((lang) => {
			LINK_CMD.setDescriptionLocalization(
				lang.discord,
				I18n.instance.get(lang.labels, "server.discord.commands.link.description"),
			);
		});
		...
		SAY_CMD.setName(cmd)
			...
			.addStringOption((option) => {
				option
					.setName("message")
					.setDescription(
						I18n.instance.get("en", "server.discord.commands.say.option_message"),
					)
					.setRequired(true);
				...
				languages.forEach((lang) => {
					option.setDescriptionLocalization(
						...
						I18n.instance.get(
							lang.labels,
							"server.discord.commands.say.option_message",
						),
					);
				});
				return option;
			...
			.addStringOption((option) => {
				option
					.setName("style")
					.setDescription(
						I18n.instance.get("en", "server.discord.commands.say.option_style"),
					)
				...
				languages.forEach((lang) => {
					option.setDescriptionLocalization(
						lang.discord,
						I18n.instance.get(lang.labels, "server.discord.commands.say.option_style"),
					);
				});
				return option;
			})
			...
			.setDMPermission(false);
		languages.forEach((lang) => {
			SAY_CMD.setDescriptionLocalization(
				lang.discord,
				I18n.instance.get(lang.labels, "server.discord.commands.say.description"),
			);
		});
		...
		ASK_CMD.setName(cmd)
			...
			.setDMPermission(false);
		languages.forEach((lang) => {
			ASK_CMD.setDescriptionLocalization(
				lang.discord,
				I18n.instance.get(lang.labels, "server.discord.commands.ask.description"),
			);
		});
		...
		if (appCommandMode) {
			existingCmds = (await this._rest.get(
				Routes.applicationCommands(Config.credentials.discord_client_id),
			)) as SlashCommandDefinition[];
		} else {
			existingCmds = (await this._rest.get(
				Routes.applicationGuildCommands(Config.credentials.discord_client_id, debugGuildID),
			)) as SlashCommandDefinition[];
		}
		...
		for (const cmd of existingCmds) {
			if (
				commandList.findIndex(
					(v) => v.name == cmd.name && v.options.length == cmd.options.length,
				) == -1
			) {
			...
			for (const cmd of removedCmds) {
				if (appCommandMode) {
					await this._rest.delete(
						Routes.applicationCommand(Config.credentials.discord_client_id, cmd.id),
					);
				} else {
					await this._rest.delete(
		...
		if (appCommandMode) {
			await this._rest.put(Routes.applicationCommands(Config.credentials.discord_client_id), {
				body: commandList,
			});
		} else {
			await this._rest.put(
		...
		if (appCommandMode) {
			freshCommandList = (await this._rest.get(
				Routes.applicationCommands(Config.credentials.discord_client_id),
			)) as SlashCommandDefinition[];
		} else {
			freshCommandList = (await this._rest.get(
	 ...
	 * @param command
	 */
	private async configureTwitchChannel(
		request: FastifyRequest,
		response: FastifyReply,
		command: SlashCommandPayload,
	): Promise<void> {
		const channel = (
			command.data.options.find((v) => v.name == "twitch_login")?.value || ""
		).trim();
		...
		if (users == false) {
			response.status(200).send({
				...
				data: {
					content: I18n.instance.get(
						command.locale,
						"server.discord.twitch_user_not_found",
						{ CHANNEL: channel },
					),
				},
			});
			...
			try {
				guildDetails = (await this._rest.get(
					Routes.guildPreview(command.guild_id),
				)) as GuildPreview;
			} catch (error) {
				console.log(error);
				response.status(200).send({
					...
					data: {
						content: I18n.instance.get(
							command.locale,
							"server.discord.guild_loading_failed",
			...
			response.status(200).send({
				type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
				data: {
					content: I18n.instance.get(command.locale, "server.discord.code", {
						CODE: code,
					}),
					embeds: [
						{
							title: I18n.instance.get(
								command.locale,
								"server.discord.commands_permissions",
							),
							image: {
	 ...
	 * @param command
	 */
	private async sendMessageToTwitchat(
		request: FastifyRequest,
		response: FastifyReply,
		command: SlashCommandPayload,
	): Promise<void> {
		...
		if (!uid) {
			response.status(200).send({
				...
				data: {
					content: I18n.instance.get(
						command.locale,
						"server.discord.install_instructions",
						{
			...
			if (!COMMAND_NAMES.includes(command.data.name)) {
				...
				SSEController.sendToUser(uid, SSECode.TRIGGER_SLASH_COMMAND, {
				...
				do {
					try {
						...
						params.set("limit", "10");
						let messages = (await this._rest.get(
							Routes.channelMessages(command.channel_id),
							{ query: params },
						)) as any[];
						originalMessage = messages.find(
							(mess) => mess.interaction && mess.interaction.id == command.id,
						);
				...
				const style =
					command.data.options.find((v) => v.name == "style")?.value || "message";
				...
				if (originalMessage) {
					if (confirm) {
						actions.push({
							...
							message:
								":white_check_mark: " +
								I18n.instance.get(guild.locale, "global.yes"),
							icon: "checkmark",
							theme: "primary",
					...
					quote = I18n.instance.get(
						command.locale,
						"server.discord.missing_history_access",
				...
				SSEController.sendToUser(uid, SSECode.NOTIFICATION, {
					messageId: originalMessage?.id,
					col: cols,
					message,
					quote,
	 ...
	 * @returns
	 */
	private async guildGuard(
		request: FastifyRequest,
		response: FastifyReply,
	): Promise<false | { user: TwitchToken; guild: TwitchatGuild2Twitch }> {
		const user = await super.twitchUserGuard(request, response);
			...
			response
				...
				.send(
					JSON.stringify({
						...
						errorCode: "MISSING_GUILD",
						error: "Missing discord guild",
						success: false,
					}),
				);
			return false;
