...
export default class StreamlabsController extends AbstractController {
	...
	public async initialize(): Promise<void> {
		this.server.get(
			"/api/streamlabs/socketToken",
			async (request, response) => await this.getSocketToken(request, response),
		);
		this.server.post(
			"/api/streamlabs/auth",
	...
	private async getSocketToken(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		try {
			...
			response
				...
				.send(
					JSON.stringify({
						success: !!socketJson.socket_token,
	...
	private async postAuth(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		try {
			const json = (await slRes.json()) as {
				access_token: string;
