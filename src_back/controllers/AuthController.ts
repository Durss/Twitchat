...
export default class AuthController extends AbstractController {
	...
	public async initialize(): Promise<void> {
		this.server.get(
			"/api/auth/twitch",
			async (request, response) => await this.twitchAuth(request, response),
		);
		this.server.get(
			"/api/auth/twitch/refreshtoken",
			async (request, response) => await this.refreshToken(request, response),
		);
		this.server.get(
			"/api/auth/CSRFToken",
			async (request, response) => await this.getCSRFToken(request, response),
		);
		this.server.post(
			"/api/auth/CSRFToken",
			...
			if (token !== false && token.uidShare && token.uidShare != user.user_id) {
				...
				if (result === true) {
					...
					response.send(
						JSON.stringify({
							success: false,
							errorCode: "CROSS_LINK",
							error: "Cannot cross link 2 accounts",
						}),
					);
					return;
				}
...
interface CSRFToken extends JwtPayload {
	date: number;
	uidShare?: string;
}
