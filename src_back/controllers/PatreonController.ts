...
export default class PatreonController extends AbstractController {
	...
	public async initialize(): Promise<void> {
		...
		if (Config.credentials.patreon_client_id_server && Config.credentials.patreon_client_secret_server) {
			...
			if (keyUint8.length !== 32) {
				Logger.error('Credential\'s "patreon_cipherKey" key must be 32 bytes (64 chars hexa string). Got', keyUint8.length.toString(), "(" + Config.credentials.patreon_cipherKey + "). Please make sure value is an hexadecimal string");
			}
	...
	public async postUserAuthenticate(request: FastifyRequest, response: FastifyReply): Promise<void> {
		try {
			...
			if (result.status == 200) {
				...
				Logger.error("[PATREON][USER] " + twitchUser.login + " failed authenticating with Patreon with status:", result.status.toString());
	...
	public async getUserIsMember(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		if (result.status == 429) {
			...
			if ("error" in json) {
				...
				//User is premium?
				if (isMember || await this.getUserPremiumState(patreonAuth.twitchUser.user_id) != "no") {
	...
	******************/
	...
	private async getCampaignID(accessToken: string): Promise<{
		id: string, tiers: CampaignResponse["included"]
	} | false> {
	 ...
	 * @param response
	 */
	private async createUserWebhook(request:FastifyRequest, response: FastifyReply, patreonAuth: Awaited<ReturnType<typeof this.getPatreonTokenFromTwitchToken>>, resolveQuery: boolean = true): Promise<boolean> {
		try {
