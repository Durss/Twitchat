...
export default class PatreonController extends AbstractController {
	...
	public async initialize(): Promise<void> {
		this.server.get(
			"/api/patreon/serverauth",
			async (request, response) => await this.getServerAuth(request, response),
		);
		this.server.get(
			"/api/patreon/isApiDown",
			async (request, response) => await this.getApiDown(request, response),
		);
		this.server.post(
			"/api/patreon/webhook",
		...
		this.server.get(
			"/api/patreon/user/memberList",
			async (request, response) => await this.getUserMemberList(request, response),
		);
		this.server.get(
			"/api/patreon/user/isMember",
			async (request, response) => await this.getUserIsMember(request, response),
		);
		this.server.post(
		...
		this.server.post(
			"/api/patreon/user/authenticate",
			async (request, response) => await this.postUserAuthenticate(request, response),
		);
		this.server.post(
			"/api/patreon/user/disconnect",
			async (request, response) => await this.disconnectUser(request, response),
		);

		if (
			Config.credentials.patreon_client_id_server &&
			Config.credentials.patreon_client_secret_server
		) {
			const keyUint8 = new Uint8Array(
				Buffer.from(Config.credentials.patreon_cipherKey, "hex"),
			);
			if (keyUint8.length !== 32) {
				Logger.error(
					'Credential\'s "patreon_cipherKey" key must be 32 bytes (64 chars hexa string). Got',
					keyUint8.length.toString(),
					"(" +
						Config.credentials.patreon_cipherKey +
						"). Please make sure value is an hexadecimal string",
				);
			}
			if (
				!/https:.*\/api\/patreon\/user\/webhook\/\{ID\}/gi.test(
					Config.credentials.patreon_webhook_url,
				)
			) {
				Logger.error(
					'Credential\'s "patreon_webhook_url" must be a valid URL. Must be secured (https) and end with "/api/patreon/user/webhook/{ID}". Got: ' +
						Config.credentials.patreon_webhook_url,
				);
			}
	 ...
	 * @param response
	 */
	public async postUserAuthenticate(
			...
			if (result.status == 200) {
				...
				if (token.error) {
					...
					Logger.error(
						"[PATREON][USER] " +
							twitchUser.login +
							" failed authenticating with Patreon with error:",
						token.error,
					);
				} else {
				...
				Logger.error(
					"[PATREON][USER] " +
						twitchUser.login +
						" failed authenticating with Patreon with status:",
	...
	public async disconnectUser(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		if (patreonAuth.token.scope.includes("w:campaigns.webhook")) {
			...
			if (result.status == 429) {
				...
				) as { [id: string]: { twitchId: string; secret: string } };
				Logger.info(
					"[PATREON][USER] Delete user webhook for",
					json[webhook.campaignID]!.twitchId,
				);
	...
	public async getUserIsMember(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		const url =
			"https://www.patreon.com/api/oauth2/v2/identity?include=memberships&fields%5Buser%5D=about,created,first_name,last_name,image_url,url,vanity&fields%5Bcampaign%5D=summary,is_monthly";
		const options = {
			method: "GET",
			headers: {
				Authorization: "Bearer " + patreonAuth.token.access_token,
		...
		if (result.status == 429) {
			...
			if ("error" in json) {
				...
				const localMemberList: PatreonMember[] = JSON.parse(
					fs.readFileSync(Config.patreonMembers, "utf-8"),
				);
				...
				let isMember = Config.credentials.admin_ids.includes(
					patreonAuth.twitchUser.user_id,
				);
				let memberID = "";
				let memberName =
				...
				if (
					isMember ||
					(await this.getUserPremiumState(patreonAuth.twitchUser.user_id)) != "no"
				) {
					//MemberID is empty if user got premium granted and isn't part of patreon donors
					//Ignore this case
					if (memberID) {
						let linkedCount = 0;
						let twitch2Patreon: Record<string, string> = {};
						if (fs.existsSync(Config.twitch2Patreon)) {
							twitch2Patreon = JSON.parse(
								fs.readFileSync(Config.twitch2Patreon, "utf-8") || "{}",
							);
						}
					...
					if (patreonAuth.token.scope.includes("w:campaigns.webhook")) {
						...
						Logger.warn(
							"[PATREON][USER] User " +
								patreonAuth.twitchUser.login +
								" doesn't have the required scope to create webhooks. User only granted " +
								patreonAuth.token.scope,
						);
					}
				}
	...
	public async getUserMemberList(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		if (!campaign) {
			...
			const campaignMembers = await this.loadCampaignMembers(
				patreonAuth.token.access_token,
				campaign.id,
			);
			if (campaignMembers.members) {
				...
				// );
				memberList = campaignMembers.members.filter((v) => v.attributes.full_name); //Some users are missing name, ignore them
			} else {
		...
		response.status(200);
		response.send(
			JSON.stringify({ success: true, data: { memberList, tierList: campaign.tiers } }),
		);
	}
			...
			if (json.error) {
				...
				try {
					const token = JSON.parse(
						fs.readFileSync(Config.patreonToken, "utf-8"),
					) as PatreonToken;
	...
	public async getServerAuth(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		if (json.error) {
			...
			if (await this.authenticateLocal()) {
				...
				response.status(200);
				response.send(
					'<div style="text-align:center; font-size:3em; width:100%;">Patreon connection Done!<br>You can close this page.</div>',
				);
	 ...
	 * @param response
	 */
	public async postWebhookTrigger(
		request: FastifyRequest,
		response: FastifyReply,
	): Promise<void> {
		...
		const hash = crypto
			.createHmac("md5", Config.credentials.patreon_webhook_secret)
		...
		this.webhookDebounce = setTimeout(async () => {
			await this.refreshPatrons(this.campaignId, event != "members:create");
			...
			if (event == "members:create") {
				...
				setTimeout(() => {
					void this.refreshPatrons(this.campaignId, true);
				}, 10000);
			}
		}, 2000);
		this.patreonApiDown = false;
	 ...
	 * @param response
	 */
	public async postUserWebhookTrigger(
		request: FastifyRequest,
		response: FastifyReply,
	): Promise<void> {
		...
		if ((await this.getUserPremiumState(twitchId)) === "no") {
			Logger.warn(
				'[PATREON][USER] received webhook event "' + event + '" for user ',
				uid,
				"(twitch:" + twitchId + ") but user is not premium, ignore it",
			);
			response.status(200);
			response.send("OK");
			return;
		...
		try {
			...
			} else if (event === "members:update") {
				const message = request.body as WebhookMemberUpdateEvent;
				//Only accept "paid" status
				if (
					message.data.attributes.last_charge_status?.toLowerCase() == "paid" &&
					message.data.attributes.patron_status == "active_patron"
				) {
					const membershipDuration = Math.abs(
						new Date(message.data.attributes.pledge_relationship_start).getTime() -
							new Date(message.data.attributes.last_charge_date).getTime(),
					);
					if (
						this.uidToFirstPayment[message.data.relationships.user.data.id] === true ||
						(membershipDuration < 20 * 24 * 60 * 60 * 1000 &&
							this.uidToFirstPayment[message.data.relationships.user.data.id] !==
								false)
					) {
						...
						const username =
							user?.attributes.full_name || message.data.attributes.full_name;
						Logger.info(
							"[PATREON] User " +
								uid +
								' subscribed with tier "' +
						...
						SSEController.sendToUser(twitchId, "PATREON_MEMBER_CREATE", {
							...
							user: {
								username,
								avatar:
									user?.attributes.image_small_url ||
									user?.attributes.image_url ||
									user?.attributes.thumb_url,
								url: user?.attributes.url,
							},
							tier: {
								amount:
									(tier?.attributes.amount_cents ||
										message.data.attributes.currently_entitled_amount_cents ||
										0) / 100,
			...
			Logger.error(
				"[PATREON][USER] Error while processing webhook event " +
					event +
					" for user " +
					uid,
			);
	...
	public async refreshPatrons(campaignId: string, verbose: boolean = true): Promise<void> {
		...
		if (members) {
			...
			const filteredMembers = members
				...
				.filter(
					(v) =>
						v.attributes.patron_status === "active_patron" &&
				...
				.map((v) => {
					const member: PatreonMember = {
						...
						total:
							Math.max(
								v.attributes.lifetime_support_cents,
								v.attributes.currently_entitled_amount_cents,
							) / 100,
					};
					return member;
				});
			...
			if (verbose)
				Logger.info("[PATREON][SERVICE] " + filteredMembers.length + " active members");
	 ...
	 * @param response
	 */
	private async getCampaignID(accessToken: string): Promise<
		| {
				id: string;
				tiers: CampaignResponse["included"];
		  }
		| false
	> {
		...
		url.searchParams.append(
			"fields[tier]",
			"amount_cents,description,title,image_url,published,published_at",
		);
		const options = {
			method: "GET",
			headers: {
		...
		if (result.status != 200) {
			...
			if (json.errors) {
				...
				const res = {
					id: json.data[0]!.id,
					tiers: json.included.filter((v) => v.type == "tier"),
				};
	 ...
	 * @returns
	 */
	private async loadCampaignMembers(
		accessToken: string,
		campaignId: string,
		offset?: string,
		memberList: any[] = [],
		...
		url.searchParams.append(
			"fields[member]",
			"full_name,lifetime_support_cents,currently_entitled_amount_cents,patron_status",
		);
		url.searchParams.append(
			"fields[tier]",
			"amount_cents,description,patron_count,published,title,url",
		);
	...
	private async loadCampaign(
		campaignId: string,
	): Promise<{ success: boolean; error?: string[]; campaign?: unknown }> {
	 ...
	 * @returns
	 */
	private async getPatreonTokenFromTwitchToken(
		request: FastifyRequest,
		response: FastifyReply,
	): Promise<false | { twitchUser: TwitchToken; token: PatreonToken }> {
		const twitchUser = await this.twitchUserGuard(request, response);
	 ...
	 * @param response
	 */
	private async createUserWebhook(
		request: FastifyRequest,
		response: FastifyReply,
		patreonAuth: Awaited<ReturnType<typeof this.getPatreonTokenFromTwitchToken>>,
		resolveQuery: boolean = true,
	): Promise<boolean> {
		try {
			const webhookRes = await this.getUserWebhook(
				request,
				response,
				patreonAuth,
				resolveQuery,
			...
			if (resultWebhook.status == 429) {
				...
				if (jsonWebhook.errors) {
					...
					json[webhookRes.campaignID] = {
						twitchId: webhookRes.user.user_id,
						secret: jsonWebhook.data.attributes.secret,
					};
					fs.writeFileSync(filePath, JSON.stringify(json), "utf8");
	...
	> {
		...
		if (!campaign) {
			...
			if (resolveQuery) {
				Logger.error(
					"[PATREON][USER] campaigns loading failed for ",
					patreonAuth.twitchUser.login,
				);
				response.header("Content-Type", "application/json");
				response.status(500);
			...
			(json.data || []).forEach((entry) => {
				if (entry.attributes.uri == webhookURL) {
					if (webhookExists) {
						...
						json[campaign.id] = {
							twitchId: patreonAuth.twitchUser.user_id,
	...
	private async refreshUserToken(
		...
		if (json.error) {
			success = false;
			Logger.error(
				"[PATREON][USER] refresh token failed for " +
					twitchUserId +
					". Disconnecting them.",
			);
			console.log(json);
	...
	private async rebuildUserWebhooks(): Promise<void> {
		...
		for (const twitchId in tokens) {
			...
			if (resultWebhook.status == 200) {
					...
					if (entry.attributes.uri == webhookURL) {
						if (webhookExists) {
							...
							if (entry.attributes.paused) {
								void this.resumeWebhook(token, campaign.id, entry.id);
							}
						}
				...
				if (!webhookExists) {
					Logger.warn(
						"[PATREON][USER] Couldn't find webhook for user " +
							twitchId +
					...
					if (resultWebhook.status == 429) {
						Logger.warn(
							"[PATREON][USER] Rate limited when creating user webhook for user " +
								twitchId +
								" :(",
						);
					} else {
	...
	private async resumeWebhook(
		accessToken: PatreonToken,
		campaignID: string,
		webhookID: string,
	): Promise<void> {
