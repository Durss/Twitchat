...
export default class QuizController extends AbstractController {
	...
	public initialize(): QuizController {
		this.server.get(
			"/api/quiz",
			async (request, response) => await this.getQuiz(request, response),
		);
		this.server.put(
			"/api/quiz/broadcast",
			async (request, response) => await this.putBroadcastQuiz(request, response),
		);
