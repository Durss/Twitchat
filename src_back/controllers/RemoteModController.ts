...
export default class RemoteModController extends AbstractController {
	...
	public initialize(): void {
		this.server.get(
			"/api/mod/request",
			async (request, response) => await this.getModParams(request, response),
		);
		this.server.post(
			"/api/mod/qna",
			async (request, response) => await this.postQnaParams(request, response),
		);
		this.server.post(
			"/api/mod/privateMessage",
			async (request, response) => await this.postPrivateMessage(request, response),
		);
		this.server.put(
			"/api/mod/qna/message",
			async (request, response) => await this.putQnaMessage(request, response),
		);
		this.server.put(
			"/api/mod/spoil/message",
			async (request, response) => await this.putSpoilMessage(request, response),
		);
		this.server.put(
			"/api/mod/privateMessage",
			async (request, response) => await this.putPrivateMessage(request, response),
		);
		this.server.delete(
			"/api/mod/qna/message",
			async (request, response) => await this.deleteQnaMessage(request, response),
		);
	}
	...
	private async getModParams(request: FastifyRequest, response: FastifyReply) {
		...
		const channels = await TwitchUtils.getModeratedChannels(
			user.user_id,
	...
	private async postQnaParams(request: FastifyRequest, response: FastifyReply) {
		...
		const moderators = await TwitchUtils.getModerators(
			user.user_id,
			request.headers.authorization!,
		);
	...
	private async postPrivateMessage(request: FastifyRequest, response: FastifyReply) {
		...
		if (!body.to_uid || !body.message || !body.messageId || !body.action) {
			response.header("Content-Type", "application/json");
			response.status(400);
			response.send(
				JSON.stringify({
		...
		if (
			user.user_id !== body.to_uid &&
			moderators.findIndex((v) => v.user_id == body.to_uid) == -1
		) {
			//Check if user is a mod on the given channel
			const moderated = await TwitchUtils.getModeratedChannels(
				user.user_id,
				request.headers.authorization!,
			);
			if (moderated.findIndex((v) => v.broadcaster_id == body.to_uid) == -1) {
				...
				response.send(
					JSON.stringify({
						success: false,
						error: "cannot send private message",
	...
	private async putQnaMessage(request: FastifyRequest, response: FastifyReply) {
		...
		if (
			channels.findIndex((v) => v.broadcaster_id == body.ownerId) == -1 ||
			(await this.getUserPremiumState(body.ownerId)) === "no"
		) {
			response.header("Content-Type", "application/json");
			response.status(400);
			response.send(
				JSON.stringify({
					success: false,
					error: "cannot remotely add a message to a private Q&A session",
					errorCode: "PRIVATE_QNA_SESSION",
				}),
			);
			return;
		}
	...
	private async putSpoilMessage(request: FastifyRequest, response: FastifyReply) {
		...
		if (
			...
			response.send(
				JSON.stringify({
					success: false,
					error: "you are not allowed to remotely spoil messages on this channel",
					errorCode: "SPOIL_NOT_ALLOWED",
				}),
			);
			return;
		}
	...
	private async deleteQnaMessage(request: FastifyRequest, response: FastifyReply) {
		...
		if (!body.messageId || !body.sessionId || !body.ownerId) {
			response.header("Content-Type", "application/json");
			response.status(400);
			response.send(
				JSON.stringify({
