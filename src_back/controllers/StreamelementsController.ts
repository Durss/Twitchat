...
export default class StreamelementsController extends AbstractController {
	...
	private async postAuth(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		body.set("client_id", Config.credentials.streamelements_client_id);
		body.set("client_secret", Config.credentials.streamelements_client_secret);
		body.set("redirect_uri", Config.credentials.streamelements_redirect_uri);
		body.set("code", (request.body as any).code);

		try {
			const slRes = await fetch("https://api.streamelements.com/oauth2/token", {
				method: "POST",
				headers,
				body: body.toString(),
			});
	...
	private async postRefreshToken(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		let text = "";
		try {
			const slRes = await fetch("https://api.streamelements.com/oauth2/token?" + params, {
				method: "POST",
				headers,
			});
