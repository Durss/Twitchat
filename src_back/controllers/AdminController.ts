...
export default class AdminController extends AbstractController {
	...
	public async initialize(): Promise<void> {
		this.server.get(
			"/api/beta/user",
			async (request, response) => await this.getUser(request, response),
		);
		this.server.get(
		...
		this.server.post(
			...
			async (request: FastifyRequest, response: FastifyReply) =>
				this.postLabels(request, response),
		);

		//Forces labels reload from frontend
		this.server.post(
			"/api/admin/labels/reload",
			async (request: FastifyRequest, response: FastifyReply) =>
				this.postLabelsReload(request, response),
		);
		...
		this.server.post(
			...
			async (request: FastifyRequest, response: FastifyReply) =>
				this.postAnnouncement(request, response),
		);
		...
		if (fs.existsSync(Config.ANNOUNCEMENTS_PATH)) {
			AdminController.announcements_cache = fs.readFileSync(
	...
	private async getUserHasData(request: FastifyRequest, response: FastifyReply) {
		...
		const prodFile = path.join(
			Config.PRODUCTION_USER_DATA_PATH_FROM_BETA,
			userInfo.user_id + ".json",
		);
	...
	private async deletePremium(request: FastifyRequest, response: FastifyReply) {
		...
		const params = request.query as any;
		super.ungiftPremium(params.uid);
	...
	private async postAnnouncement(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		if (fs.existsSync(Config.ANNOUNCEMENTS_PATH)) {
			list = JSON.parse(
				fs.readFileSync(Config.ANNOUNCEMENTS_PATH, "utf-8"),
			) as AnnouncementData[];
		}
	...
	public async postLabels(request: FastifyRequest, response: FastifyReply): Promise<void> {
		...
		if (this.savingLabels) {
			Logger.warn(
				"A label update is already in progress, wait for it complete before updateing section",
				section,
			);
		...
		try {
			await new Promise<void>((resolve) => {
				fs.writeFileSync(
					path.join(Config.LABELS_ROOT, lang + "/" + section + ".json"),
				...
				child.on("close", (_code) => {
					if (errored) {
						...
						response.send(
							JSON.stringify({
								success: false,
								message: "failed executing script to compile labels",
								errorCode: "EXEC_SCRIPT_ERROR",
							}),
						);
						resolve();
						return;
